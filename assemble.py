...
def _fa_strip_comments_slow(code):
...
# 정규식 1패스 등가 구현(~10x). 대안 순서 = 상태기계 우선순위: 문자열/char 리터럴이 먼저
# 매치되어 내부의 //, /* 보호(리터럴은 그대로 보존, g1), 주석은 제거(라인주석의 개행은
# [^\n] 이 소비하지 않아 보존 — 상태기계와 동일). 미종결 리터럴/블록주석은 \Z 로 EOF 까지.
# [검증] kartall 14,826개 생성 클래스 + 방출 본문 코퍼스에서 _slow 와 출력 동일 확인.
_FA_STRIP_RE = re.compile(
    r'("(?:[^"\\]|\\.)*(?:"|\Z)'
    r"|'(?:[^'\\]|\\.)*(?:'|\Z))"
    r'|//[^\n]*'
    r'|/\*.*?(?:\*/|\Z)', re.S)
...
def _fa_strip_comments(code):
    ...
    if _KFC_SLOW_STRIP:
        return _fa_strip_comments_slow(code)
    return _FA_STRIP_RE.sub(lambda m: m.group(1) or '', code)

# 문자열 스킵: 내용((비특수|이스케이프쌍)*) 후 닫는따옴표, 미종결이면 트레일링 \ 까지
# (문자 루프의 'i+=2 가 EOF 를 넘는' 소비와 동일하게 끝 위치가 산출됨).
_FA_STR_DQ = re.compile(r'(?:[^"\\]|\\.)*(?:"|\\)?')
_FA_STR_SQ = re.compile(r"(?:[^'\\]|\\.)*(?:'|\\)?")

def _fa_skip_string(s, i):
    m = (_FA_STR_DQ if s[i] == '"' else _FA_STR_SQ).match(s, i + 1)
    return m.end()

_FA_JUMP_PAREN = re.compile(r'[()"\']')
_FA_JUMP_BRACE = re.compile(r'[{}"\']')

def _fa_balanced(s, i):
    """s[i] 가 '(' 또는 '{' 일 때 짝 맞는 닫힘 '다음' 인덱스(문자열 인식).
       [가속] 여닫는 괄호·따옴표만 정규식으로 점프 — 그 외 문자는 상태에 영향 없음(등가)."""
    open_c = s[i]
    rx = _FA_JUMP_PAREN if open_c == '(' else _FA_JUMP_BRACE
    depth = 0; n = len(s); pos = i
    while pos < n:
        m = rx.search(s, pos)
        if not m:
            return n
        c = m.group(0); k = m.start()
        if c == '"' or c == "'":
            pos = _fa_skip_string(s, k)
            continue
        if c == open_c:
            depth += 1
        else:
...
_FA_KW_RE = re.compile(r'(if|for|while|switch)\b')
_FA_DO_RE = re.compile(r'do\b')
_FA_WHILE_RE = re.compile(r'while\b')
_FA_TRY_RE = re.compile(r'try\b')
_FA_TAIL_RE = re.compile(r'[;({"\']')

def _fa_stmt_end(code, i):
    n = len(code)
    m = _FA_KW_RE.match(code, i)
    if m:
        ...
        return j
    if _FA_DO_RE.match(code, i):
        j = _fa_substmt_end(code, i + 2)
        while j < n and code[j] in ' \t\r\n': j += 1
        if _FA_WHILE_RE.match(code, j):
            j += 5
            while j < n and code[j] in ' \t\r\n': j += 1
            if j < n and code[j] == '(':
                j = _fa_balanced(code, j)
    ...
    j = i
    while True:
        m2 = _FA_TAIL_RE.search(code, j)
        if not m2:
            return n
        c = m2.group(0); k = m2.start()
        if c == ';':
            return k + 1
        if c == '"' or c == "'":
            j = _fa_skip_string(code, k)
            continue
        j = _fa_balanced(code, k)   # '{' 또는 '('
    return n
...
# (#13) 임계 실측 보정: kartall 서브셋 컴파일 실측에서 바이트코드/코드문자 비율은
# 클래스 최악 0.76 B/char, 중앙값 0.16 (종전 가정 ~1.2 는 15~7x 과대). 과소 임계는
# 불필요한 분할(조각 호출 체인·프레임 오버헤드·코드 팽창)을 늘리므로 목표를
# 9000자(최악 비율 0.8 가정에도 ~7.2KB < HugeMethodLimit 8KB)로 상향한다.
# 한계(문서화): 분할 경계는 방출 그룹 단위라, 단일 그룹이 거대한 함수(대형 모델 summon
# 등)는 어떤 임계로도 더 쪼갤 수 없다 — 실측에서 17KB 조각 2건이 이 경우.
_SEG_TRIGGER = 12000  # 주석 제외 코드 문자수 — 초과 시에만 분할 발동
_SEG_TARGET  = 9000   # 조각 목표 코드 문자수(실측 최악 0.8 B/char 가정 ~7.2KB 안전권)
_SEG_MARK    = "// __KFC_SEG_BOUNDARY__"   # 조립-렌더 간 조각 경계 전달용(최종 출력서 제거)

_SEG_STR_RE = re.compile(r'"(?:[^"\\]|\\.)*(")?')

def _seg_strip_str(l: str) -> str:
    """문자열 리터럴 '내부' 제거 — 문자열은 상수풀로 빠져 바이트코드에 미포함(ldc 참조뿐).
       [가속] 정규식 1패스 — 문자 상태기계와 등가(미종결 문자열은 여는따옴표만 남음)."""
    return _SEG_STR_RE.sub(lambda m: '""' if m.group(1) else '"', l)
