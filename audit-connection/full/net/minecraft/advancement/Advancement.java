...
public record Advancement(
	...
	public static final Codec<Advancement> CODEC = RecordCodecBuilder.<Advancement>create(
			instance -> instance.group(
					...
					AdvancementRewards.CODEC.optionalFieldOf("rewards", AdvancementRewards.NONE).forGetter(Advancement::rewards),
	...
	private static Text createNameFromDisplay(AdvancementDisplay display) {
		...
		Text text3 = text.copy().styled(style -> style.withHoverEvent(new HoverEvent.ShowText(text2)));
	...
	public static class Builder {
		...
		public Advancement.Builder display(
			...
			return this.display(
				new AdvancementDisplay(icon, title, description, Optional.ofNullable(background).map(AssetInfo::new), frame, showToast, announceToChat, hidden)
			);
		}
		...
		public Advancement.Builder display(
			...
			return this.display(
				new AdvancementDisplay(
					new ItemStack(icon.asItem()), title, description, Optional.ofNullable(background).map(AssetInfo::new), frame, showToast, announceToChat, hidden
				)
			);
		}
