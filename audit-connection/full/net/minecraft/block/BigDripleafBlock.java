...
public class BigDripleafBlock extends HorizontalFacingBlock implements Fertilizable, Waterloggable {
	...
	private Function<BlockState, VoxelShape> createShapeFunction() {
		...
		return this.createShapeFunction(
			state -> VoxelShapes.union((VoxelShape)SHAPES_BY_TILT.get(state.get(TILT)), (VoxelShape)map.get(state.get(FACING))), new Property[]{WATERLOGGED}
		);
	}
	...
	protected static boolean placeDripleafAt(WorldAccess world, BlockPos pos, FluidState fluidState, Direction direction) {
		BlockState blockState = Blocks.BIG_DRIPLEAF.getDefaultState().with(WATERLOGGED, fluidState.isEqualAndStill(Fluids.WATER)).with(FACING, direction);
	...
	private static boolean isEntityAbove(BlockPos pos, Entity entity) {
		return entity.isOnGround() && entity.getPos().y > pos.getY() + 0.6875F;
	}
