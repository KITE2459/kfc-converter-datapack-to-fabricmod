...
public class Block extends AbstractBlock implements ItemConvertible, FabricBlock {
	...
	/**
	 * Skips {@link BlockEntity#onBlockReplaced}.
	 * Used when placing structures.
	 */
	public static final int SKIP_BLOCK_ENTITY_REPLACED_CALLBACK = 256;
	/**
	 * Skips {@link AbstractBlock#onBlockAdded}.
	 * Used when placing structures.
	 */
	...
	/**
	 * Used for 'strict' block placement by commands and structures.
	...
	protected Function<BlockState, VoxelShape> createShapeFunction(Function<BlockState, VoxelShape> stateToShape) {
		return ((ImmutableMap)this.stateManager.getStates().stream().collect(ImmutableMap.toImmutableMap(Function.identity(), stateToShape)))::get;
	}
	...
	protected Function<BlockState, VoxelShape> createShapeFunction(Function<BlockState, VoxelShape> stateToShape, Property<?>... properties) {
		Map<? extends Property<?>, Object> map = (Map<? extends Property<?>, Object>)Arrays.stream(properties)
			.collect(Collectors.toMap(property -> property, property -> property.getValues().getFirst()));
		ImmutableMap<BlockState, VoxelShape> immutableMap = (ImmutableMap<BlockState, VoxelShape>)this.stateManager
			.getStates()
			.stream()
			.filter(state -> map.entrySet().stream().allMatch(entry -> state.get((Property)entry.getKey()) == entry.getValue()))
