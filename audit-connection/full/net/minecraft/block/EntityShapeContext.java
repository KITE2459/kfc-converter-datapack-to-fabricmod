...
public class EntityShapeContext implements ShapeContext {
	protected static final ShapeContext ABSENT = new EntityShapeContext(false, false, -Double.MAX_VALUE, ItemStack.EMPTY, fluidState -> false, null) {
		@Override
	...
	protected EntityShapeContext(
		boolean descending, boolean placement, double minY, ItemStack heldItem, Predicate<FluidState> walkOnFluidPredicate, @Nullable Entity entity
	) {
	...
	protected EntityShapeContext(Entity entity, boolean collidesWithFluid, boolean placement) {
		this(
			...
			placement,
			entity.getY(),
			entity instanceof LivingEntity livingEntityx ? livingEntityx.getMainHandStack() : ItemStack.EMPTY,
			collidesWithFluid ? state -> true : (entity instanceof LivingEntity livingEntity ? state -> livingEntity.canWalkOnFluid(state) : state -> false),
			entity
		);
	}
