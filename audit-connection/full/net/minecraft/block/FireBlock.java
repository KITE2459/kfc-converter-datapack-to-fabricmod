...
public class FireBlock extends AbstractFireBlock {
	...
	public static final Map<Direction, BooleanProperty> DIRECTION_PROPERTIES = (Map<Direction, BooleanProperty>)ConnectingBlock.FACING_PROPERTIES
		.entrySet()
		.stream()
	...
	public FireBlock(AbstractBlock.Settings settings) {
		...
		this.setDefaultState(
			this.stateManager.getDefaultState().with(AGE, 0).with(NORTH, false).with(EAST, false).with(SOUTH, false).with(WEST, false).with(UP, false)
		);
