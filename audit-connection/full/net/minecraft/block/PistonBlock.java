...
public class PistonBlock extends FacingBlock {
	...
	protected VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
		return state.get(EXTENDED) ? (VoxelShape)EXTENDED_SHAPES_BY_DIRECTION.get(state.get(FACING)) : VoxelShapes.fullCube();
	}
	...
	protected boolean onSyncedBlockEvent(BlockState state, World world, BlockPos pos, int type, int data) {
		...
		if (type == 0) {
			...
			world.setBlockState(pos, blockState2, Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK | Block.FORCE_STATE);
			world.addBlockEntity(
				PistonExtensionBlock.createBlockEntityPiston(pos, blockState2, this.getDefaultState().with(FACING, Direction.byIndex(data & 7)), direction, false, true)
			);
	...
	private boolean move(World world, BlockPos pos, Direction dir, boolean extend) {
		...
		if (!extend && world.getBlockState(blockPos).isOf(Blocks.PISTON_HEAD)) {
			world.setBlockState(blockPos, Blocks.AIR.getDefaultState(), Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK | Block.FORCE_STATE);
		}
		...
		if (!pistonHandler.calculatePush()) {
			...
			for (int j = list.size() - 1; j >= 0; j--) {
				...
				world.setBlockState(blockPos3, blockState3, Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK | Block.MOVED);
			...
			if (extend) {
				...
				BlockState blockState2 = Blocks.MOVING_PISTON
					...
					.with(PistonExtensionBlock.TYPE, this.sticky ? PistonType.STICKY : PistonType.DEFAULT);
				map.remove(blockPos);
				world.setBlockState(blockPos, blockState2, Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK | Block.MOVED);
