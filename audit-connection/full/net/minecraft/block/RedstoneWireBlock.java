...
public class RedstoneWireBlock extends Block {
	...
	private Function<BlockState, VoxelShape> createShapeFunction() {
		...
		return this.createShapeFunction(state -> {
			...
			for (Entry<Direction, EnumProperty<WireConnection>> entry : DIRECTION_TO_WIRE_CONNECTION_PROPERTY.entrySet()) {
				voxelShape2 = switch ((WireConnection)state.get((Property)entry.getValue())) {
					case UP -> VoxelShapes.union(voxelShape2, (VoxelShape)map.get(entry.getKey()), (VoxelShape)map2.get(entry.getKey()));
					case SIDE -> VoxelShapes.union(voxelShape2, (VoxelShape)map.get(entry.getKey()));
	...
	private static void addPoweredParticles(
		...
		if (!(random.nextFloat() >= 0.2F * f)) {
			...
			double e = 0.5 + 0.4375F * perpendicular.getOffsetY() + h * direction.getOffsetY();
			double i = 0.5 + 0.4375F * perpendicular.getOffsetZ() + h * direction.getOffsetZ();
			world.addParticleClient(new DustParticleEffect(color, 1.0F), pos.getX() + d, pos.getY() + e, pos.getZ() + i, 0.0, 0.0, 0.0);
		}
	}
