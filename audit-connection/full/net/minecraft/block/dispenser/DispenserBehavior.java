			...
			new ItemDispenserBehavior() {
				...
				public ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
					...
					Consumer<ArmorStandEntity> consumer = EntityType.copier(
						armorStand -> armorStand.setYaw(direction.getPositiveHorizontalDegrees()), serverWorld, stack, null
					);
			...
			new FallibleItemDispenserBehavior() {
				...
				protected ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
					...
					if (world.isAir(blockPos) && WitherSkullBlock.canDispense(world, blockPos, stack)) {
						world.setBlockState(
							blockPos, Blocks.WITHER_SKELETON_SKULL.getDefaultState().with(SkullBlock.ROTATION, RotationPropertyHelper.fromDirection(direction)), Block.NOTIFY_ALL
						);
