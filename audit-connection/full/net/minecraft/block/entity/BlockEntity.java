...
public abstract class BlockEntity implements RenderDataBlockEntity, AttachmentTarget {
	...
	public final void read(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		...
		this.components = (ComponentMap)nbt.decode(BlockEntity.Components.CODEC, registries.getOps(NbtOps.INSTANCE)).orElse(ComponentMap.EMPTY);
	}
	...
	public final NbtCompound createNbt(RegistryWrapper.WrapperLookup registries) {
		...
		nbtCompound.copyFromCodec(BlockEntity.Components.CODEC, registries.getOps(NbtOps.INSTANCE), this.components);
		return nbtCompound;
	}
	...
	@Nullable
	public static Text tryParseCustomName(@Nullable NbtElement nbt, RegistryWrapper.WrapperLookup registries) {
		return nbt == null
			? null
	...
	static class Components {
		public static final MapCodec<ComponentMap> CODEC = ComponentMap.CODEC.optionalFieldOf("components", ComponentMap.EMPTY);
