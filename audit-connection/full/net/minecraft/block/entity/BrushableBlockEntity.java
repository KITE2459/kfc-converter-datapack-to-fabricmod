...
public class BrushableBlockEntity extends BlockEntity {
	...
	public void scheduledTick(ServerWorld world) {
		if (this.brushesCount != 0 && world.getTime() >= this.nextDustTime) {
			...
			int j = this.getDustedLevel();
			if (i != j) {
				world.setBlockState(this.getPos(), this.getCachedState().with(Properties.DUSTED, j), Block.NOTIFY_ALL);
			}
	...
	private boolean readLootTableFromNbt(NbtCompound nbtCompound) {
		this.lootTable = (RegistryKey<LootTable>)nbtCompound.get("LootTable", LootTable.TABLE_KEY).orElse(null);
	...
	protected void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		...
		if (!this.readLootTableFromNbt(nbt)) {
			this.item = (ItemStack)nbt.get("item", ItemStack.CODEC, registryOps).orElse(ItemStack.EMPTY);
		} else {
			this.item = ItemStack.EMPTY;
		}

		this.hitDirection = (Direction)nbt.get("hit_direction", Direction.INDEX_CODEC).orElse(null);
	}
