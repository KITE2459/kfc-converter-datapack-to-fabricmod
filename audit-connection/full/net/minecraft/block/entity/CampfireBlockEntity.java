...
public class CampfireBlockEntity extends BlockEntity implements Clearable {
	...
	protected void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		...
		nbt.getIntArray("CookingTimes")
			.ifPresentOrElse(
				is -> System.arraycopy(is, 0, this.cookingTimes, 0, Math.min(this.cookingTotalTimes.length, is.length)), () -> Arrays.fill(this.cookingTimes, 0)
			);
		nbt.getIntArray("CookingTotalTimes")
			.ifPresentOrElse(
				is -> System.arraycopy(is, 0, this.cookingTotalTimes, 0, Math.min(this.cookingTotalTimes.length, is.length)), () -> Arrays.fill(this.cookingTotalTimes, 0)
			);
	}
