...
public class CrafterBlockEntity extends LootableContainerBlockEntity implements RecipeInputInventory {
	...
	protected void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		...
		this.craftingTicksRemaining = nbt.getInt("crafting_ticks_remaining", 0);
		...
		nbt.getIntArray("disabled_slots").ifPresent(slots -> {
			for (int ix : slots) {
		...
		this.propertyDelegate.set(9, nbt.getInt("triggered", 0));
	}
	...
	private void putDisabledSlots(NbtCompound nbt) {
		...
		nbt.putIntArray("disabled_slots", intList.toIntArray());
	}
