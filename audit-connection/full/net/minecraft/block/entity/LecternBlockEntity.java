...
public class LecternBlockEntity extends BlockEntity implements Clearable, NamedScreenHandlerFactory {
	...
	protected void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		...
		this.book = (ItemStack)nbt.get("Book", ItemStack.CODEC, registryOps).map(itemStack -> this.resolveBook(itemStack, null)).orElse(ItemStack.EMPTY);
		this.pageCount = getPageCount(this.book);
		this.currentPage = MathHelper.clamp(nbt.getInt("Page", 0), 0, this.pageCount - 1);
	}
	...
	protected void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		...
		if (!this.getBook().isEmpty()) {
			RegistryOps<NbtElement> registryOps = registries.getOps(NbtOps.INSTANCE);
			nbt.put("Book", ItemStack.CODEC, registryOps, this.getBook());
