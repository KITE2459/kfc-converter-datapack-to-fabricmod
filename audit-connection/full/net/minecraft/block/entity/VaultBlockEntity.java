...
public class VaultBlockEntity extends BlockEntity {
	...
	protected void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		...
		nbt.put("config", VaultConfig.codec, registryOps, this.config);
		nbt.put("shared_data", VaultSharedData.codec, registryOps, this.sharedData);
		nbt.put("server_data", VaultServerData.codec, registryOps, this.serverData);
	}
	...
	protected void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		...
		nbt.get("server_data", VaultServerData.codec, dynamicOps).ifPresent(this.serverData::copyFrom);
		this.config = (VaultConfig)nbt.get("config", VaultConfig.codec, dynamicOps).orElse(VaultConfig.DEFAULT);
		nbt.get("shared_data", VaultSharedData.codec, dynamicOps).ifPresent(this.sharedData::copyFrom);
	}
	...
	public static final class Client {
		...
		private static Vec3d getConnectedParticlesOrigin(BlockPos pos, Direction direction) {
			return Vec3d.ofBottomCenter(pos).add(direction.getOffsetX() * 0.5, 1.75, direction.getOffsetZ() * 0.5);
		}
	}
