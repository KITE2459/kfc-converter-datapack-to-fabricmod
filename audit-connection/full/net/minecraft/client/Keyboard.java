				...
				case 78:
					if (!this.client.player.hasPermissionLevel(2)) {
						...
						this.client
							.player
							.networkHandler
							.sendCommand("gamemode " + MoreObjects.firstNonNull(this.client.interactionManager.getPreviousGameMode(), GameMode.CREATIVE).getId());
					}
		...
		if (window == this.client.getWindow().getHandle()) {
			...
			if (action != 0) {
				...
				if (bl2) {
					...
					ClientPlayerEntity var17 = this.client.player;
				}
			}
			...
			boolean bl4 = bl3 || this.client.currentScreen instanceof GameMenuScreen gameMenuScreen && !gameMenuScreen.shouldShowMenu();
			if (action == 0) {
