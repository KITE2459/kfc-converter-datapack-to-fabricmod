...
public class WindowFramebuffer extends Framebuffer {
	...
	private GpuTexture createColorAttachment(WindowFramebuffer.Size size) {
		try {
			return RenderSystem.getDevice().createTexture(() -> this.name + " / Color", TextureFormat.RGBA8, size.width, size.height, 1);
	...
	private GpuTexture createDepthAttachment(WindowFramebuffer.Size size) {
		try {
			return RenderSystem.getDevice().createTexture(() -> this.name + " / Depth", TextureFormat.DEPTH32, size.width, size.height, 1);
		} catch (TextureAllocationException var3) {
			return null;
		}
	}
