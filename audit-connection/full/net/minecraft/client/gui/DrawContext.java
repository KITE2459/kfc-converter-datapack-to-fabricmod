...
public class DrawContext {
	...
	private void setScissor(@Nullable ScreenRect rect) {
		...
		if (rect != null) {
			...
			double e = rect.getLeft() * d;
			double f = i - rect.getBottom() * d;
			double g = rect.width() * d;
			double h = rect.height() * d;
	...
	private void drawItem(@Nullable LivingEntity entity, @Nullable World world, ItemStack stack, int x, int y, int seed, int z) {
		if (!stack.isEmpty()) {
			this.client.getItemModelManager().clearAndUpdate(this.itemRenderState, stack, ItemDisplayContext.GUI, world, entity, seed);
			this.matrices.push();
