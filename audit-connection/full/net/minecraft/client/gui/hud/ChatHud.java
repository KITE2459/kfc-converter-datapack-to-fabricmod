...
public class ChatHud {
					...
					if (visible != null) {
						...
						if (t < 200 || focused) {
							...
							if (u > 3) {
								...
								context.drawTextWithShadow(this.client.textRenderer, visible.content(), 0, y, ColorHelper.withAlpha(u, Colors.WHITE));
	...
	public boolean mouseClicked(double mouseX, double mouseY) {
		if (this.isChatFocused() && !this.client.options.hudHidden && !this.isChatHidden()) {
			...
			if (messageHandler.getUnprocessedMessageCount() == 0L) {
				...
				double d = mouseX - 2.0;
				double e = this.client.getWindow().getScaledHeight() - mouseY - 40.0;
				if (d <= MathHelper.floor(this.getWidth() / this.getChatScale()) && e < 0.0 && e > MathHelper.floor(-9.0 * this.getChatScale())) {
	...
	private double toChatLineY(double y) {
		double d = this.client.getWindow().getScaledHeight() - y - 40.0;
		return d / (this.getChatScale() * this.getLineHeight());
	}
	...
	private int getMessageLineIndex(double chatLineX, double chatLineY) {
		if (this.isChatFocused() && !this.isChatHidden()) {
			if (!(chatLineX < -4.0) && !(chatLineX > MathHelper.floor(this.getWidth() / this.getChatScale()))) {
				int i = Math.min(this.getVisibleLineCount(), this.visibleMessages.size());
				if (chatLineY >= 0.0 && chatLineY < i) {
					int j = MathHelper.floor(chatLineY + this.scrolledLines);
