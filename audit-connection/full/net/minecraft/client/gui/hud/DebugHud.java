...
public class DebugHud {
	private static final float field_56577 = 10.0F;
	...
	private static final Map<Heightmap.Type, String> HEIGHT_MAP_TYPES = Maps.newEnumMap(
		Map.of(
	...
	public DebugHud(MinecraftClient client) {
		...
		try (BufferAllocator bufferAllocator = new BufferAllocator(VertexFormats.POSITION_COLOR_NORMAL.getVertexSize() * 12)) {
			...
			bufferBuilder.vertex(10.0F, 0.0F, 0.0F).color(Colors.RED).normal(1.0F, 0.0F, 0.0F);
			bufferBuilder.vertex(0.0F, 0.0F, 0.0F).color(Colors.GREEN).normal(0.0F, 1.0F, 0.0F);
			bufferBuilder.vertex(0.0F, 10.0F, 0.0F).color(Colors.GREEN).normal(0.0F, 1.0F, 0.0F);
			...
			try (BuiltBuffer builtBuffer = bufferBuilder.end()) {
				this.debugCrosshairBuffer = RenderSystem.getDevice()
					.createBuffer(() -> "Crosshair vertex buffer", BufferType.VERTICES, BufferUsage.STATIC_WRITE, builtBuffer.getBuffer());
			}
		}
	}
