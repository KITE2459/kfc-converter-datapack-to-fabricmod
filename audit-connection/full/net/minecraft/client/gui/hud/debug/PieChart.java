			...
			context.draw(vertexConsumers -> {
				...
				for (ProfilerTiming profilerTimingx : list) {
					...
					for (int nxx = kx; nxx >= 0; nxx--) {
						float f = (float)((d + profilerTimingx.parentSectionUsagePercentage * nxx / kx) * (float) (Math.PI * 2) / 100.0);
						float g = MathHelper.sin(f) * 105.0F;
						float h = MathHelper.cos(f) * 105.0F * 0.5F;
						vertexConsumer.vertex(entry, i + g, p - h, 10.0F).color(lx);
					}

					vertexConsumer = vertexConsumers.getBuffer(RenderLayer.getDebugQuads());

					for (int nx = kx; nx > 0; nx--) {
						float f = (float)((d + profilerTimingx.parentSectionUsagePercentage * nx / kx) * (float) (Math.PI * 2) / 100.0);
						float g = MathHelper.sin(f) * 105.0F;
						float h = MathHelper.cos(f) * 105.0F * 0.5F;
						float ox = (float)((d + profilerTimingx.parentSectionUsagePercentage * (nx - 1) / kx) * (float) (Math.PI * 2) / 100.0);
						float px = MathHelper.sin(ox) * 105.0F;
						float qx = MathHelper.cos(ox) * 105.0F * 0.5F;
						if (!((h + qx) / 2.0F > 0.0F)) {
							vertexConsumer.vertex(entry, i + g, p - h, 10.0F).color(mx);
							vertexConsumer.vertex(entry, i + g, p - h + 10.0F, 10.0F).color(mx);
							vertexConsumer.vertex(entry, i + px, p - qx + 10.0F, 10.0F).color(mx);
							vertexConsumer.vertex(entry, i + px, p - qx, 10.0F).color(mx);
						}
					}
