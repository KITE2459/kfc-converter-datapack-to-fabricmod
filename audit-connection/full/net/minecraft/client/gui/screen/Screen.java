...
public abstract class Screen extends AbstractParentElement implements Drawable {
			...
			if (hasShiftDown()) {
				...
				switch (clickEvent) {
					case ClickEvent.OpenUrl(URI var28):
						...
						if (this.client.options.getChatLinksPrompt().getValue()) {
							this.client.setScreen(new ConfirmLinkScreen(confirmed -> {
								if (confirmed) {
									Util.getOperatingSystem().open(var19);
								}
					...
					case ClickEvent.RunCommand(String var24):
						...
						if (!this.client.player.networkHandler.sendCommand(string2)) {
					...
					case ClickEvent.CopyToClipboard(String var14):
						this.client.keyboard.setClipboard(var14);
						break;
					default:
	...
	protected void renderPanoramaBackground(DrawContext context, float deltaTicks) {
		ROTATING_PANORAMA_RENDERER.render(context, this.width, this.height, 1.0F, deltaTicks);
	}
