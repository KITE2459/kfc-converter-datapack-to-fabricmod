...
public class StatsScreen extends Screen {
	...
	class ItemStatsListWidget extends AlwaysSelectedEntryListWidget<StatsScreen.ItemStatsListWidget.Entry> {
		...
		private final Identifier[] headerIconTextures = new Identifier[]{
			Identifier.ofVanilla("statistics/block_mined"),
			Identifier.ofVanilla("statistics/item_broken"),
			Identifier.ofVanilla("statistics/item_crafted"),
			Identifier.ofVanilla("statistics/item_used"),
			Identifier.ofVanilla("statistics/item_picked_up"),
			Identifier.ofVanilla("statistics/item_dropped")
		};
		...
		protected void renderHeader(DrawContext context, int x, int y) {
			...
			for (int i = 0; i < this.headerIconTextures.length; i++) {
				...
				context.drawGuiTexture(RenderLayer::getGuiTextured, identifier, x + this.getIconX(i) - 18, y + 1, 18, 18);
			}

			if (this.selectedStatType != null) {
				int i = this.getIconX(this.getHeaderIndex(this.selectedStatType)) - 36;
			...
			for (int i = 0; i < this.headerIconTextures.length; i++) {
				...
				context.drawGuiTexture(RenderLayer::getGuiTextured, this.headerIconTextures[i], x + this.getIconX(i) - 18 + j, y + 1 + j, 18, 18);
			}
		}
		...
		public boolean mouseClicked(double mouseX, double mouseY, int button) {
			...
			return !bl
					&& this.select((int)(mouseX - (this.getX() + this.width / 2.0 - this.getRowWidth() / 2.0)), (int)(mouseY - this.getY()) + (int)this.getScrollY() - 4)
				? true
				: bl;
		}
			...
			public void render(
				...
				if (StatsScreen.this.itemStats != null) {
					for (int i = 0; i < StatsScreen.this.itemStats.blockStatTypes.size(); i++) {
						...
						this.render(context, stat, x + ItemStatsListWidget.this.getIconX(i), y + entryHeight / 2 - 9 / 2, index % 2 == 0);
					}
					...
					for (int i = 0; i < StatsScreen.this.itemStats.itemStatTypes.size(); i++) {
						this.render(
							...
							x + ItemStatsListWidget.this.getIconX(i + StatsScreen.this.itemStats.blockStatTypes.size()),
							y + entryHeight / 2 - 9 / 2,
							index % 2 == 0
						);
					}
