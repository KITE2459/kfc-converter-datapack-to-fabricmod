...
public class CreativeInventoryScreen extends HandledScreen<CreativeInventoryScreen.CreativeScreenHandler> implements FabricCreativeInventoryScreen {
		...
		if (this.client != null) {
			...
			if (clientPlayerEntity != null) {
				this.updateDisplayParameters(
					clientPlayerEntity.networkHandler.getEnabledFeatures(), this.shouldShowOperatorTab(clientPlayerEntity), clientPlayerEntity.getWorld().getRegistryManager()
				);
	...
	protected void drawBackground(DrawContext context, float deltaTicks, int mouseX, int mouseY) {
		...
		if (selectedTab.hasScrollbar()) {
			...
			context.drawGuiTexture(RenderLayer::getGuiTextured, identifier, i, j + (int)((k - j - 17) * this.scrollPosition), 12, 15);
		}
