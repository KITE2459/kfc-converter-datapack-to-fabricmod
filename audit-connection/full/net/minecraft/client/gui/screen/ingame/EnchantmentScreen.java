...
public class EnchantmentScreen extends HandledScreen<EnchantmentScreenHandler> {
	...
	protected void drawBackground(DrawContext context, float deltaTicks, int mouseX, int mouseY) {
		...
		this.drawBook(context, i, j, deltaTicks);
		...
		for (int l = 0; l < 3; l++) {
			...
			if (o == 0) {
				...
				if ((k < l + 1 || this.client.player.experienceLevel < o) && !this.client.player.isInCreativeMode()) {
					...
					context.drawWrappedText(this.textRenderer, stringVisitable, n, j + 16 + 19 * l, p, (q & 16711422) >> 1, false);
					q = 4226832;
				} else {
					int r = mouseX - (i + 60);
	...
	private void drawBook(DrawContext context, int x, int y, float deltaTicks) {
		float f = MathHelper.lerp(deltaTicks, this.pageTurningSpeed, this.nextPageTurningSpeed);
		float g = MathHelper.lerp(deltaTicks, this.pageAngle, this.nextPageAngle);
		context.draw();
