...
public class StructureBlockScreen extends Screen {
	...
	protected void init() {
		...
		this.strictButton = this.addDrawableChild(
			CyclingButtonWidget.onOffBuilder(this.structureBlock.isStrict())
				.omitKeyText()
				.build(this.width / 2 + 4 + 100, 120, 50, 20, STRICT_TEXT, (cyclingButtonWidget, boolean_) -> this.structureBlock.setStrict(boolean_))
		);
	...
	public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
		...
		if (structureBlockMode == StructureBlockMode.LOAD) {
			...
			context.drawTextWithShadow(this.textRenderer, STRICT_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth(STRICT_TEXT), 110, 10526880);
