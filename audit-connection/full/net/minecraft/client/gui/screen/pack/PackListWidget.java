...
public class PackListWidget extends AlwaysSelectedEntryListWidget<PackListWidget.ResourcePackEntry> {
	...
	protected void renderHeader(DrawContext context, int x, int y) {
		...
		context.drawTextWithShadow(
			this.client.textRenderer, text, x + this.width / 2 - this.client.textRenderer.getWidth(text) / 2, Math.min(this.getY() + 3, y), Colors.WHITE
		);
	}
	...
	public static class ResourcePackEntry extends AlwaysSelectedEntryListWidget.Entry<PackListWidget.ResourcePackEntry> {
		...
		public boolean mouseClicked(double mouseX, double mouseY, int button) {
			double d = mouseX - this.widget.getRowLeft();
			double e = mouseY - this.widget.getRowTop(this.widget.children().indexOf(this));
