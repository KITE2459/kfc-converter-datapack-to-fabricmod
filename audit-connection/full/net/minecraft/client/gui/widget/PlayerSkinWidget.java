...
public class PlayerSkinWidget extends ClickableWidget {
	...
	public PlayerSkinWidget(int width, int height, LoadedEntityModels entityModels, Supplier<SkinTextures> skinSupplier) {
		...
		this.models = PlayerSkinWidget.Models.create(entityModels);
	...
	protected void renderWidget(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
		...
		context.getMatrices().translate(this.getX() + this.getWidth() / 2.0F, (float)(this.getY() + this.getHeight()), 100.0F);
		float f = this.getHeight() / 2.125F;
		...
		context.draw();
		DiffuseLighting.rotateGuiShaderLighting(RotationAxis.POSITIVE_X.rotationDegrees(this.xRotation));
	...
	record Models(PlayerEntityModel wideModel, PlayerEntityModel slimModel) {
		public static PlayerSkinWidget.Models create(LoadedEntityModels entityModels) {
			PlayerEntityModel playerEntityModel = new PlayerEntityModel(entityModels.getModelPart(EntityModelLayers.PLAYER), false);
			PlayerEntityModel playerEntityModel2 = new PlayerEntityModel(entityModels.getModelPart(EntityModelLayers.PLAYER_SLIM), true);
		...
		public void draw(DrawContext context, SkinTextures skinTextures) {
			...
			context.draw(vertexConsumers -> playerEntityModel.render(context.getMatrices(), vertexConsumers.getBuffer(renderLayer), 15728880, OverlayTexture.DEFAULT_UV));
			context.getMatrices().pop();
		}
	}
}
