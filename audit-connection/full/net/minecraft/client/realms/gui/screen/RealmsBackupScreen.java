...
public class RealmsBackupScreen extends RealmsScreen {
	...
	class BackupObjectSelectionListEntry extends ElementListWidget.Entry<RealmsBackupScreen.BackupObjectSelectionListEntry> {
		...
		public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickProgress) {
			...
			context.drawTextWithShadow(
				RealmsBackupScreen.this.textRenderer, Text.translatable("mco.backup.entry", RealmsUtil.convertToAgePresentation(this.mBackup.lastModifiedDate)), x, j, l
			);
			context.drawTextWithShadow(RealmsBackupScreen.this.textRenderer, this.getMediumDatePresentation(this.mBackup.lastModifiedDate), x, k, 5000268);
			int m = 0;
