...
public class RealmsSubscriptionInfoScreen extends RealmsScreen {
	...
	public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
		...
		context.drawTextWithShadow(this.textRenderer, SUBSCRIPTION_START_LABEL_TEXT, i, row(0), Colors.LIGHT_GRAY);
		context.drawTextWithShadow(this.textRenderer, this.startDate, i, row(1), Colors.WHITE);
		if (this.type == Subscription.SubscriptionType.NORMAL) {
			context.drawTextWithShadow(this.textRenderer, TIME_LEFT_LABEL_TEXT, i, row(3), Colors.LIGHT_GRAY);
		} else if (this.type == Subscription.SubscriptionType.RECURRING) {
			context.drawTextWithShadow(this.textRenderer, DAYS_LEFT_LABEL_TEXT, i, row(3), Colors.LIGHT_GRAY);
		}

		context.drawTextWithShadow(this.textRenderer, this.daysLeft, i, row(4), Colors.WHITE);
	}
