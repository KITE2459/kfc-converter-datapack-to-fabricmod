...
public class RealmsUploadScreen extends RealmsScreen implements UploadProgressTracker {
	...
	public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
		...
		if (this.showDots) {
			context.drawTextWithShadow(
				this.textRenderer, DOTS[this.animTick / 10 % DOTS.length], this.width / 2 + this.textRenderer.getWidth(this.status) / 2 + 5, 50, Colors.WHITE
			);
		}
	...
	private void upload() {
		...
		RealmsWorldOptions realmsWorldOptions = RealmsWorldOptions.create(
			this.selectedLevel.getLevelInfo(), true, this.selectedLevel.getVersionInfo().getVersionName()
		);
