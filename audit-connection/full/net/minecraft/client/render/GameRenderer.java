...
public class GameRenderer implements AutoCloseable {
	...
	public void renderBlur() {
		...
		if (!(f < 1.0F)) {
			...
			if (postEffectProcessor != null) {
				postEffectProcessor.render(this.client.getFramebuffer(), this.pool, pass -> pass.setUniform("Radius", f));
			}
		}
	}
	...
	private void bobView(MatrixStack matrices, float tickProgress) {
		if (this.client.getCameraEntity() instanceof AbstractClientPlayerEntity abstractClientPlayerEntity) {
			...
			float g = -(abstractClientPlayerEntity.distanceMoved + var7 * tickProgress);
	...
	private void renderHand(Camera camera, float tickProgress, Matrix4f positionMatrix) {
		if (!this.renderingPanorama) {
			Matrix4f matrix4f = this.getBasicProjectionMatrix(this.getFov(camera, tickProgress, false));
	...
	public float getFarPlaneDistance() {
		return this.viewDistanceBlocks * 4.0F;
	}
		...
		if (!this.client.skipGameRender) {
			...
			if (bl && tick && this.client.world != null) {
				...
				if (this.postProcessorId != null && this.postProcessorEnabled) {
					...
					if (postEffectProcessor != null) {
						postEffectProcessor.render(this.client.getFramebuffer(), this.pool, null);
					}
				}
			}
			...
			if (bl && tick && this.client.world != null) {
				...
				if (!this.client.options.hudHidden) {
					this.renderFloatingItem(drawContext, tickCounter.getTickProgress(false));
				}
	...
	private void updateWorldIcon(Path path) {
		if (this.client.worldRenderer.getCompletedChunkCount() > 10 && this.client.worldRenderer.isTerrainRenderComplete()) {
			ScreenshotRecorder.takeScreenshot(this.client.getFramebuffer(), nativeImage -> Util.getIoWorkerExecutor().execute(() -> {
	...
	public void renderWorld(RenderTickCounter renderTickCounter) {
		...
		if (l > 0.0F) {
			...
			float n = (this.nauseaEffectTime + f * this.nauseaEffectSpeed) * (float) (Math.PI / 180.0);
			matrix4f.rotate(n, vector3f);
			matrix4f.scale(1.0F / m, 1.0F, 1.0F);
			matrix4f.rotate(-n, vector3f);
		}

		float m = Math.max(h, this.client.options.getFov().getValue().intValue());
	...
	private void renderFloatingItem(DrawContext context, float tickProgress) {
		if (this.floatingItem != null && this.floatingItemTimeLeft > 0) {
			...
			context.draw(
				vertexConsumers -> this.client
					.getItemRenderer()
					.renderItem(this.floatingItem, ItemDisplayContext.FIXED, 15728880, OverlayTexture.DEFAULT_UV, matrixStack, vertexConsumers, this.client.world, 0)
			);
			matrixStack.pop();
		}
	}
