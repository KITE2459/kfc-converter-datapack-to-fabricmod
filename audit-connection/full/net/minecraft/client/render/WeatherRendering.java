...
public class WeatherRendering {
	...
	private void buildPrecipitationPieces(
		World world, int ticks, float tickProgress, Vec3d pos, int range, List<WeatherRendering.Piece> rainOut, List<WeatherRendering.Piece> snowOut
	) {
	...
	private void renderPrecipitation(
		...
		if (!snowPieces.isEmpty()) {
			RenderLayer renderLayer = RenderLayer.getWeather(SNOW_TEXTURE, MinecraftClient.isFabulousGraphicsOrBetter());
			...
			for (int j = 0; j < i; j++) {
				...
				if (blockPos3.getY() > world.getBottomY()
					...
					ParticleEffect particleEffect = !fluidState.isIn(FluidTags.LAVA) && !blockState.isOf(Blocks.MAGMA_BLOCK) && !CampfireBlock.isLitCampfire(blockState)
						? ParticleTypes.RAIN
						: ParticleTypes.SMOKE;
					world.addParticleClient(particleEffect, blockPos2.getX() + d, blockPos2.getY() + m, blockPos2.getZ() + e, 0.0, 0.0, 0.0);
				}
			}
