...
public class WorldRenderer implements SynchronousResourceReloader, AutoCloseable {
	...
	public void drawEntityOutlinesFramebuffer() {
		if (this.canDrawEntityOutlines()) {
			this.entityOutlineFramebuffer.drawBlit(this.client.getFramebuffer().getColorAttachment());
		}
	}
	...
	public void render(
			...
			() -> {
				...
				RenderSystem.getDevice()
					...
					.clearColorAndDepthTextures(
						framebuffer.getColorAttachment(), ColorHelper.fromFloats(0.0F, vector4f.x, vector4f.y, vector4f.z), framebuffer.getDepthAttachment(), 1.0
					);
			}
		);
		if (!bl2) {
		...
		if (bl3 && postEffectProcessor2 != null) {
			postEffectProcessor2.render(frameGraphBuilder, i, j, this.framebufferSet, null);
		}
		...
		if (cloudRenderMode != CloudRenderMode.OFF) {
			...
			if (!Float.isNaN(k)) {
				...
				this.renderClouds(frameGraphBuilder, cloudRenderMode, camera.getPos(), l, m, k + 0.33F);
			}
		}

		this.renderWeather(frameGraphBuilder, camera.getPos(), f, fog);
		if (postEffectProcessor != null) {
			postEffectProcessor.render(frameGraphBuilder, i, j, this.framebufferSet, null);
		}
	...
	private void renderMain(
		...
		framePass.setRenderer(() -> {
			...
			float f = renderTickCounter.getTickProgress(false);
			Vec3d vec3d = camera.getPos();
			double d = vec3d.getX();
			double e = vec3d.getY();
			double g = vec3d.getZ();
	...
	private void renderParticles(FrameGraphBuilder frameGraphBuilder, Camera camera, float tickProgress, Fog fog) {
	...
	private void renderClouds(FrameGraphBuilder frameGraphBuilder, CloudRenderMode mode, Vec3d cameraPos, float f, int color, float cloudHeight) {
		...
		framePass.setRenderer(() -> this.cloudRenderer.renderClouds(color, mode, cloudHeight, cameraPos, f));
	}

	private void renderWeather(FrameGraphBuilder frameGraphBuilder, Vec3d cameraPos, float tickProgress, Fog fog) {
		...
		framePass.setRenderer(() -> {
			...
			this.worldBorderRendering.render(this.world.getWorldBorder(), cameraPos, i, f);
			immediate.draw();
		});
	}
	...
	private void renderEntities(
		MatrixStack matrices, VertexConsumerProvider.Immediate vertexConsumers, Camera camera, RenderTickCounter tickCounter, List<Entity> entities
	) {
		Vec3d vec3d = camera.getPos();
		double d = vec3d.getX();
		double e = vec3d.getY();
					...
					if (sortedSet != null && !sortedSet.isEmpty()) {
						...
						if (i >= 0) {
							...
							VertexConsumer vertexConsumer = new OverlayVertexConsumer(
								effectVertexConsumers.getBuffer((RenderLayer)ModelBaker.BLOCK_DESTRUCTION_RENDER_LAYERS.get(i)), entry, 1.0F
							);
	...
	private void renderEntity(
		Entity entity, double cameraX, double cameraY, double cameraZ, float tickProgress, MatrixStack matrices, VertexConsumerProvider vertexConsumers
	) {
		...
		this.entityRenderDispatcher
			.render(entity, d - cameraX, e - cameraY, f - cameraZ, tickProgress, matrices, vertexConsumers, this.entityRenderDispatcher.getLight(entity, tickProgress));
	}
