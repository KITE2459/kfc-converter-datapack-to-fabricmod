...
public class BeaconBlockEntityRenderer<T extends BlockEntity & BeamEmitter> implements BlockEntityRenderer<T> {
	...
	public void render(T entity, float tickProgress, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay, Vec3d cameraPos) {
		...
		for (int j = 0; j < list.size(); j++) {
			BeamEmitter.BeamSegment beamSegment = (BeamEmitter.BeamSegment)list.get(j);
			renderBeam(matrices, vertexConsumers, tickProgress, g, l, i, j == list.size() - 1 ? 2048 : beamSegment.getHeight(), beamSegment.getColor());
			i += beamSegment.getHeight();
		}
	}

	private static void renderBeam(
		MatrixStack matrices, VertexConsumerProvider vertexConsumers, float tickProgress, float scale, long worldTime, int yOffset, int maxY, int color
	) {
