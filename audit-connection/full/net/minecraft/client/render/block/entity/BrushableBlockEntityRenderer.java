...
public class BrushableBlockEntityRenderer implements BlockEntityRenderer<BrushableBlockEntity> {
	...
	public void render(
		BrushableBlockEntity brushableBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j, Vec3d vec3d
	) {
				...
				if (direction != null) {
					...
					if (!itemStack.isEmpty()) {
						...
						int l = WorldRenderer.getLightmapCoordinates(
							...
							brushableBlockEntity.getPos().offset(direction)
						);
						this.itemRenderer
							.renderItem(itemStack, ItemDisplayContext.FIXED, l, OverlayTexture.DEFAULT_UV, matrixStack, vertexConsumerProvider, brushableBlockEntity.getWorld(), 0);
						matrixStack.pop();
					}
				}
			}
