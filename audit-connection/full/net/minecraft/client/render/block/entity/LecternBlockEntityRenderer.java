...
public class LecternBlockEntityRenderer implements BlockEntityRenderer<LecternBlockEntity> {
	...
	public void render(
		LecternBlockEntity lecternBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j, Vec3d vec3d
	) {
		...
		if ((Boolean)blockState.get(LecternBlock.HAS_BOOK)) {
			...
			float g = ((Direction)blockState.get(LecternBlock.FACING)).rotateYClockwise().getPositiveHorizontalDegrees();
