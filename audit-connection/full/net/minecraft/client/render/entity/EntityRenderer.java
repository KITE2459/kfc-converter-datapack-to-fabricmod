...
public abstract class EntityRenderer<T extends Entity, S extends EntityRenderState> {
	...
	public void updateRenderState(T entity, S state, float tickProgress) {
		...
		if (entity2 != null) {
			float g = entity.lerpYaw(tickProgress) * (float) (Math.PI / 180.0);
			Vec3d vec3d = entity.getLeashOffset(tickProgress).rotateY(-g);
			BlockPos blockPos = BlockPos.ofFloored(entity.getCameraPosVec(tickProgress));
			BlockPos blockPos2 = BlockPos.ofFloored(entity2.getCameraPosVec(tickProgress));
			...
			leashData.startPos = entity.getLerpedPos(tickProgress).add(vec3d);
			leashData.endPos = entity2.getLeashPos(tickProgress);
	...
	private EntityHitboxAndView createHitbox(T entity, float tickProgress, boolean green) {
		Builder<EntityHitbox> builder = new Builder<>();
		Box box = entity.getBoundingBox();
		EntityHitbox entityHitbox;
		if (green) {
			entityHitbox = new EntityHitbox(
	...
	protected void appendHitboxes(T entity, Builder<EntityHitbox> builder, float tickProgress) {
	}
