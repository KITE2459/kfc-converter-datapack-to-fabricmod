		...
		if (illusionerEntityRenderState.invisible) {
			...
			for (int j = 0; j < vec3ds.length; j++) {
				...
				matrixStack.translate(
					vec3ds[j].x + MathHelper.cos(j + illusionerEntityRenderState.age * 0.5F) * 0.025,
					vec3ds[j].y + MathHelper.cos(j + illusionerEntityRenderState.age * 0.75F) * 0.0125,
					vec3ds[j].z + MathHelper.cos(j + illusionerEntityRenderState.age * 0.7F) * 0.025
				);
