...
public abstract class LivingEntityRenderer<T extends LivingEntity, S extends LivingEntityRenderState, M extends EntityModel<? super S>>
		...
		if (this.shouldRenderFeatures(livingEntityRenderState)) {
			for (FeatureRenderer<S, M> featureRenderer : this.features) {
				featureRenderer.render(
					matrixStack, vertexConsumerProvider, i, livingEntityRenderState, livingEntityRenderState.relativeHeadYaw, livingEntityRenderState.pitch
				);
			}
		}
	...
	public static boolean shouldFlipUpsideDown(LivingEntity entity) {
		if (entity instanceof PlayerEntity || entity.hasCustomName()) {
			...
			if ("Dinnerbone".equals(string) || "Grumm".equals(string)) {
				return !(entity instanceof PlayerEntity playerEntity && !playerEntity.isPartVisible(PlayerModelPart.CAPE));
			}
		}
	...
	protected void appendHitboxes(T livingEntity, Builder<EntityHitbox> builder, float f) {
		Box box = livingEntity.getBoundingBox();
		float g = 0.01F;
		EntityHitbox entityHitbox = new EntityHitbox(
			box.minX - livingEntity.getX(),
			livingEntity.getStandingEyeHeight() - 0.01F,
			box.minZ - livingEntity.getZ(),
			box.maxX - livingEntity.getX(),
			livingEntity.getStandingEyeHeight() + 0.01F,
			box.maxZ - livingEntity.getZ(),
			1.0F,
			0.0F,
			0.0F
		);
