	...
	public WardenEntityRenderer(EntityRendererFactory.Context context) {
		...
		this.addFeature(
			new EmissiveFeatureRenderer<>(
				this, BIOLUMINESCENT_LAYER_TEXTURE, (state, tickProgress) -> 1.0F, WardenEntityModel::getHeadAndLimbs, RenderLayer::getEntityTranslucentEmissive, false
			)
		);
		this.addFeature(
			new EmissiveFeatureRenderer<>(
		...
		this.addFeature(
			new EmissiveFeatureRenderer<>(
				this, TEXTURE, (state, tickProgress) -> state.tendrilAlpha, WardenEntityModel::getTendrils, RenderLayer::getEntityTranslucentEmissive, false
			)
		);
		this.addFeature(
			new EmissiveFeatureRenderer<>(
				this, HEART_TEXTURE, (state, tickProgress) -> state.heartAlpha, WardenEntityModel::getBody, RenderLayer::getEntityTranslucentEmissive, false
			)
		);
	}
