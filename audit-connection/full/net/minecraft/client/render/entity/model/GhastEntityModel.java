...
public class GhastEntityModel extends EntityModel<GhastEntityRenderState> {
	...
	public static TexturedModelData getTexturedModelData() {
		...
		modelPartData.addChild(
			EntityModelPartNames.BODY, ModelPartBuilder.create().uv(0, 0).cuboid(-8.0F, -8.0F, -8.0F, 16.0F, 16.0F, 16.0F), ModelTransform.origin(0.0F, 17.6F, 0.0F)
		);
		...
		for (int i = 0; i < 9; i++) {
			...
			modelPartData.addChild(getTentacleName(i), ModelPartBuilder.create().uv(0, 0).cuboid(-1.0F, 0.0F, -1.0F, 2.0F, j, 2.0F), ModelTransform.origin(f, 24.6F, g));
		}
	...
	public void setAngles(GhastEntityRenderState ghastEntityRenderState) {
		...
		for (int i = 0; i < this.tentacles.length; i++) {
			this.tentacles[i].pitch = 0.2F * MathHelper.sin(ghastEntityRenderState.age * 0.3F + i) + 0.4F;
		}
	}
}
