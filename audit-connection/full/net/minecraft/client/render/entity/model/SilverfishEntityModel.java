...
public class SilverfishEntityModel extends EntityModel<EntityRenderState> {
	...
	public void setAngles(EntityRenderState state) {
		...
		for (int i = 0; i < this.body.length; i++) {
			this.body[i].yaw = MathHelper.cos(state.age * 0.9F + i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.05F * (1 + Math.abs(i - 2));
			this.body[i].originX = MathHelper.sin(state.age * 0.9F + i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.2F * Math.abs(i - 2);
		}
