...
public class HeldItemRenderer {
	...
	public void renderItem(
		...
		if (!stack.isEmpty()) {
			this.itemRenderer
				.renderItem(entity, stack, renderMode, matrices, vertexConsumer, entity.getWorld(), light, OverlayTexture.DEFAULT_UV, entity.getId() + renderMode.ordinal());
		}
	}
	...
	public void renderItem(float tickProgress, MatrixStack matrices, VertexConsumerProvider.Immediate vertexConsumers, ClientPlayerEntity player, int light) {
		...
		if (handRenderType.renderMainHand) {
			...
			float k = 1.0F - MathHelper.lerp(tickProgress, this.lastEquipProgressMainHand, this.equipProgressMainHand);
		...
		if (handRenderType.renderOffHand) {
			...
			float k = 1.0F - MathHelper.lerp(tickProgress, this.lastEquipProgressOffHand, this.equipProgressOffHand);
	...
	private void renderFirstPersonItem(
		...
		if (!player.isUsingSpyglass()) {
			...
			if (item.isEmpty()) {
				...
				this.renderItem(
					player, item, bl3 ? ItemDisplayContext.FIRST_PERSON_RIGHT_HAND : ItemDisplayContext.FIRST_PERSON_LEFT_HAND, matrices, vertexConsumers, light
				);
			} else {
				...
				if (player.isUsingItem() && player.getItemUseTimeLeft() > 0 && player.getActiveHand() == hand) {
					switch (item.getUseAction()) {
						...
						case BOW:
							...
							float mx = item.getMaxUseTime(player) - (player.getItemUseTimeLeft() - tickProgress + 1.0F);
							float fxx = mx / 20.0F;
				...
				this.renderItem(
					player, item, bl2 ? ItemDisplayContext.FIRST_PERSON_RIGHT_HAND : ItemDisplayContext.FIRST_PERSON_LEFT_HAND, matrices, vertexConsumers, light
				);
			}
