	...
	public void tick() {
		...
		if (biome != this.activeBiome) {
			...
			biome.getLoopSound().ifPresent(sound -> this.soundLoops.compute(biome, (soundx, loop) -> {
				if (loop == null) {
					loop = new BiomeEffectSoundPlayer.MusicLoop((SoundEvent)sound.value());
			...
			.ifPresent(
				sound -> {
					...
					BlockPos blockPos = BlockPos.ofFloored(
						this.player.getX() + this.random.nextInt(i) - sound.getSpawnRange(),
						this.player.getEyeY() + this.random.nextInt(i) - sound.getSpawnRange(),
						this.player.getZ() + this.random.nextInt(i) - sound.getSpawnRange()
					);
					int j = world.getLightLevel(LightType.SKY, blockPos);
					if (j > 0) {
						this.moodPercentage -= j / 15.0F * 0.001F;
					} else {
						this.moodPercentage = this.moodPercentage - (float)(world.getLightLevel(LightType.BLOCK, blockPos) - 1) / sound.getCultivationTicks();
					}
					...
					if (this.moodPercentage >= 1.0F) {
						...
						PositionedSoundInstance positionedSoundInstance = PositionedSoundInstance.ambient(
							sound.getSound().value(), this.random, this.player.getX() + g / l * m, this.player.getEyeY() + h / l * m, this.player.getZ() + k / l * m
						);
