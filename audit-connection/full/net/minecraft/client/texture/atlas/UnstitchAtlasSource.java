...
public record UnstitchAtlasSource(Identifier resource, List<UnstitchAtlasSource.Region> regions, double divisorX, double divisorY) implements AtlasSource {
	...
	public static final MapCodec<UnstitchAtlasSource> CODEC = RecordCodecBuilder.mapCodec(
		instance -> instance.group(
				...
				Codecs.nonEmptyList(UnstitchAtlasSource.Region.CODEC.listOf()).fieldOf("regions").forGetter(UnstitchAtlasSource::regions),
	...
	static class SpriteRegion implements AtlasSource.SpriteRegion {
		...
		public SpriteContents apply(SpriteOpener spriteOpener) {
			try {
				...
				double d = nativeImage.getWidth() / this.divisorX;
				double e = nativeImage.getHeight() / this.divisorY;
