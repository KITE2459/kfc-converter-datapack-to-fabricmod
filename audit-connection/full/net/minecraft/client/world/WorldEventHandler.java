	...
	public void processWorldEvent(int eventId, BlockPos pos, int data) {
		...
		switch (eventId) {
			...
			case 2006:
				for (int m = 0; m < 200; m++) {
					...
					Particle particle2 = this.worldRenderer
						.spawnParticle(ParticleTypes.DRAGON_BREATH, false, pos.getX() + n * 0.1, pos.getY() + 0.3, pos.getZ() + p * 0.1, n, o, p);
					if (particle2 != null) {
						particle2.move(ab);
					}
				}
			...
			case 2013:
				...
				break;
			case 3000:
				this.world.addParticleClient(ParticleTypes.EXPLOSION_EMITTER, true, true, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, 0.0, 0.0, 0.0);
				this.world
					.playSoundAtBlockCenterClient(
						pos,
