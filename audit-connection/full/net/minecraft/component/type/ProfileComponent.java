...
public record ProfileComponent(Optional<String> name, Optional<UUID> id, PropertyMap properties, GameProfile gameProfile) {
	...
	public CompletableFuture<ProfileComponent> getFuture() {
		if (this.isCompleted()) {
			...
			return this.id.isPresent() ? SkullBlockEntity.fetchProfileByUuid((UUID)this.id.get()).thenApply(profile -> {
				GameProfile gameProfile = (GameProfile)profile.orElseGet(() -> new GameProfile((UUID)this.id.get(), (String)this.name.orElse("")));
