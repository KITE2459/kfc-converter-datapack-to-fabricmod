...
public class VanillaHusbandryTabAdvancementGenerator implements AdvancementTabGenerator {
	...
	public void accept(RegistryWrapper.WrapperLookup registries, Consumer<AdvancementEntry> exporter) {
		...
		AdvancementEntry advancementEntry = Advancement.Builder.create()
			.display(
				...
				Identifier.ofVanilla("gui/advancements/backgrounds/husbandry"),
		...
		requireAllCatsTamed(Advancement.Builder.create(), registryWrapper2)
		...
		requireAllWolvesTamed(Advancement.Builder.create(), registryWrapper3)
			.parent(advancementEntry4)
			.display(
				Items.BONE,
		...
		AdvancementEntry advancementEntry11 = requireAllFrogsOnLeads(registryEntryLookup, registryEntryLookup2, registryWrapper, Advancement.Builder.create())
									...
									EntityEquipmentPredicate.Builder.create()
										.body(
											ItemPredicate.Builder.create()
												...
												.components(ComponentsPredicate.Builder.create().exact(ComponentMapPredicate.of(DataComponentTypes.DAMAGE, 0)).build())
										)
								)
						)
					)
	...
	public static AdvancementEntry createBreedAllAnimalsAdvancement(
		...
		Stream<EntityType<?>> eggLayingAnimals
	) {
		return requireListedAnimalsBred(Advancement.Builder.create(), breedableAnimals, entityTypeLookup, eggLayingAnimals)
			.parent(parent)
			.display(
				Items.GOLDEN_CARROT,
	...
	private static Advancement.Builder requireAllFrogsOnLeads(
		...
		RegistryWrapper<FrogVariant> frogVariantRegistry,
		Advancement.Builder builder
	) {
		streamSorted(frogVariantRegistry)
			.forEach(
				entry -> builder.criterion(
					entry.registryKey().getValue().toString(),
					PlayerInteractedWithEntityCriterion.Conditions.create(
						ItemPredicate.Builder.create().items(itemLookup, Items.LEAD),
						Optional.of(
							EntityPredicate.contextPredicateFromEntityPredicate(
								EntityPredicate.Builder.create()
									.type(entityTypeLookup, EntityType.FROG)
									.components(ComponentsPredicate.Builder.create().exact(ComponentMapPredicate.of(DataComponentTypes.FROG_VARIANT, entry)).build())
							)
						)
					)
				)
	...
	private static Advancement.Builder requireFoodItemsEaten(Advancement.Builder builder, RegistryEntryLookup<Item> itemLookup) {
		for (Item item : FOOD_ITEMS) {
			builder.criterion(Registries.ITEM.getId(item).getPath(), ConsumeItemCriterion.Conditions.item(itemLookup, item));
		}
	...
	private static Advancement.Builder requireListedAnimalsBred(
		...
		breedableAnimals.forEach(
			entityType -> builder.criterion(
				EntityType.getId(entityType).toString(), BredAnimalsCriterion.Conditions.create(EntityPredicate.Builder.create().type(entityTypeLookup, entityType))
			)
		);
		eggLayingAnimals.forEach(
			entityType -> builder.criterion(
				...
				BredAnimalsCriterion.Conditions.create(
					Optional.of(EntityPredicate.Builder.create().type(entityTypeLookup, entityType).build()),
					Optional.of(EntityPredicate.Builder.create().type(entityTypeLookup, entityType).build()),
					Optional.empty()
				)
			)
	...
	private static Advancement.Builder requireListedFishBucketsFilled(Advancement.Builder builder, RegistryEntryLookup<Item> itemLookup) {
		for (Item item : FISH_BUCKET_ITEMS) {
			builder.criterion(Registries.ITEM.getId(item).getPath(), FilledBucketCriterion.Conditions.create(ItemPredicate.Builder.create().items(itemLookup, item)));
		}
	...
	private static Advancement.Builder requireListedFishCaught(Advancement.Builder builder, RegistryEntryLookup<Item> itemLookup) {
		for (Item item : FISH_ITEMS) {
			builder.criterion(
				...
				FishingRodHookedCriterion.Conditions.create(Optional.empty(), Optional.empty(), Optional.of(ItemPredicate.Builder.create().items(itemLookup, item).build()))
			);
		}
	...
	private static Advancement.Builder requireAllCatsTamed(Advancement.Builder builder, RegistryWrapper<CatVariant> catVariantRegistry) {
		streamSorted(catVariantRegistry)
			.forEach(
				entry -> builder.criterion(
					entry.registryKey().getValue().toString(),
					TameAnimalCriterion.Conditions.create(
						EntityPredicate.Builder.create()
							.components(ComponentsPredicate.Builder.create().exact(ComponentMapPredicate.of(DataComponentTypes.CAT_VARIANT, entry)).build())
					)
				)
			);
		return builder;
	...
	private static Advancement.Builder requireAllWolvesTamed(Advancement.Builder builder, RegistryWrapper<WolfVariant> wolfVariantRegistry) {
		streamSorted(wolfVariantRegistry)
			.forEach(
				entry -> builder.criterion(
					entry.registryKey().getValue().toString(),
					TameAnimalCriterion.Conditions.create(
						EntityPredicate.Builder.create()
							.components(ComponentsPredicate.Builder.create().exact(ComponentMapPredicate.of(DataComponentTypes.WOLF_VARIANT, entry)).build())
					)
				)
			);
		return builder;
