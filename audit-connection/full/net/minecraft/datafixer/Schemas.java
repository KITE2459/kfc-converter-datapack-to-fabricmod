...
import net.minecraft.datafixer.fix.AreaEffectCloudDurationScaleFix;
...
import net.minecraft.datafixer.fix.ChunkTicketUnpackPosFix;
...
import net.minecraft.datafixer.fix.DisplayNameFix;
import net.minecraft.datafixer.fix.DropChancesFormatFix;
...
import net.minecraft.datafixer.fix.EntityFallDistanceFloatToDoubleFix;
...
import net.minecraft.datafixer.fix.EquipmentFormatFix;
...
import net.minecraft.datafixer.fix.ForcedChunkToTicketFix;
...
import net.minecraft.datafixer.fix.InlineBlockPosFormatFix;
...
import net.minecraft.datafixer.fix.LegacyHoverEventFix;
...
import net.minecraft.datafixer.fix.PlayerEquipmentFix;
...
import net.minecraft.datafixer.fix.RaidRenamesDataFix;
...
import net.minecraft.datafixer.fix.SaddleEquipmentSlotFix;
import net.minecraft.datafixer.fix.SimpleBlockPropertyRenameFix;
...
import net.minecraft.datafixer.fix.ThrownPotionSplitFix;
import net.minecraft.datafixer.fix.TicksInWrongChunkFix;
import net.minecraft.datafixer.fix.TippedArrowPotionToItemFix;
import net.minecraft.datafixer.fix.TooltipDisplayComponentFix;
...
import net.minecraft.datafixer.fix.TypeChangeFix;
import net.minecraft.datafixer.fix.UnflattenTextComponentFix;
...
import net.minecraft.datafixer.schema.Schema1488;
...
import net.minecraft.datafixer.schema.Schema1925;
...
import net.minecraft.datafixer.schema.Schema2511_1;
...
import net.minecraft.datafixer.schema.Schema3813;
...
import net.minecraft.datafixer.schema.Schema4290;
import net.minecraft.datafixer.schema.Schema4292;
import net.minecraft.datafixer.schema.Schema4300;
import net.minecraft.datafixer.schema.Schema4301;
import net.minecraft.datafixer.schema.Schema4302;
import net.minecraft.datafixer.schema.Schema4306;
import net.minecraft.datafixer.schema.Schema4307;
...
public class Schemas {
	...
	public static final int field_38844 = 4295;
	...
	private static void build(DataFixerBuilder builder) {
		...
		builder.addFixer(new TextComponentStrictJsonFix(schema15));
		...
		Schema schema46 = builder.addSchema(1458, Schema1458::new);
		builder.addFixer(new EntityCustomNameToTextFix(schema46));
		builder.addFixer(new ItemCustomNameToComponentFix(schema46));
		builder.addFixer(new BlockEntityCustomNameToTextFix(schema46));
		...
		Schema schema58 = builder.addSchema(1488, Schema1488::new);
		builder.addFixer(
			BlockNameFix.create(
		...
		builder.addFixer(
			new ChoiceWriteReadFix(schema58, true, "Command block block entity custom name fix", TypeReferences.BLOCK_ENTITY, "minecraft:command_block") {
				@Override
		...
		builder.addFixer(
			new DataFix(schema58, false) {
				...
				protected TypeRewriteRule makeRule() {
					...
					OpticFinder<Pair<String, String>> opticFinder3 = DSL.typeFinder((Type<Pair<String, String>>)this.getInputSchema().getType(TypeReferences.TEXT_COMPONENT));
					return this.fixTypeEverywhereTyped(
						...
						typed -> {
							...
							return !"minecraft:commandblock_minecart".equals(string)
								? typed
								: typed.updateTyped(opticFinder2, typedx -> typedx.update(opticFinder3, pair -> pair.mapSecond(TextFixes::text)));
						}
					);
				}
			}
		...
		builder.addFixer(new DisplayNameFix(schema68, "ObjectiveDisplayNameFix", TypeReferences.OBJECTIVE));
		builder.addFixer(new DisplayNameFix(schema68, "TeamDisplayNameFix", TypeReferences.TEAM));
		...
		builder.addFixer(new ItemLoreToTextFix(schema74));
		...
		Schema schema84 = builder.addSchema(1925, Schema1925::new);
		...
		builder.addFixer(new VillagerExperienceRebuildFix(schema92, false));
		...
		Schema schema107 = builder.addSchema(2511, 1, Schema2511_1::new);
		builder.addFixer(new TypeChangeFix(schema107, "SplashPotionItemFieldRenameFix", TypeReferences.ENTITY, "minecraft:potion"));
		...
		Schema schema120 = builder.addSchema(2538, EMPTY_IDENTIFIER_NORMALIZE);
		...
		builder.addFixer(new GameEventRenamesFix(schema123, "Nether biome rename", TypeReferences.BIOME, replacing("minecraft:nether", "minecraft:nether_wastes")));
		Schema schema124 = builder.addSchema(2553, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new GameEventRenamesFix(schema124, "Biomes fix", TypeReferences.BIOME, replacing(FlatteningBiomeMapping.RENAMED_BIOMES)));
		...
		builder.addFixer(BlockNameFix.create(schema129, "Renamed grass path block to dirt path", replacing("minecraft:grass_path", "minecraft:dirt_path")));
		...
		builder.addFixer(BlockNameFix.create(schema133, "Renamed copper blocks to new oxidized terms", replacing(immutableMap)));
		...
		builder.addFixer(BlockNameFix.create(schema134, "Rename copper blocks suffixes", replacing(immutableMap2)));
		...
		builder.addFixer(BlockNameFix.create(schema136, "Renamed grimstone blocks to deepslate", replacing(immutableMap3)));
		...
		builder.addFixer(new GameEventRenamesFix(schema148, "Caves and Cliffs biome renames", TypeReferences.BIOME, replacing(LegacyBiomeMapping.MAP)));
		...
		builder.addFixer(
			new GameEventRenamesFix(schema151, "Remove Deep Warm Ocean", TypeReferences.BIOME, replacing("minecraft:deep_warm_ocean", "minecraft:warm_ocean"))
		);
		...
		Schema schema165 = builder.addSchema(3090, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new RenameEntityNbtKeyFix(schema165, "EntityPaintingFieldsRenameFix", "minecraft:painting", Map.of("Motive", "variant", "Facing", "facing")));
		Schema schema166 = builder.addSchema(3093, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new GoatMissingStateFix(schema166));
		...
		Schema schema169 = builder.addSchema(3108, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new BlendingDataRemoveFromNetherEndFix(schema169));
		Schema schema170 = builder.addSchema(3201, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new OptionsProgrammerArtFix(schema170));
		Schema schema171 = builder.addSchema(3202, Schema3202::new);
		builder.addFixer(new ChoiceTypesFix(schema171, "Added Hanging Sign", TypeReferences.BLOCK_ENTITY));
		Schema schema172 = builder.addSchema(3203, Schema3203::new);
		builder.addFixer(new ChoiceTypesFix(schema172, "Added Camel", TypeReferences.ENTITY));
		Schema schema173 = builder.addSchema(3204, Schema3204::new);
		builder.addFixer(new ChoiceTypesFix(schema173, "Added Chiseled Bookshelf", TypeReferences.BLOCK_ENTITY));
		Schema schema174 = builder.addSchema(3209, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new ItemInstanceSpawnEggFix(schema174, false, "minecraft:pig_spawn_egg"));
		Schema schema175 = builder.addSchema(3214, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new OptionsAmbientOcclusionFix(schema175));
		Schema schema176 = builder.addSchema(3319, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new OptionsAccessibilityOnboardFix(schema176));
		Schema schema177 = builder.addSchema(3322, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new StatusEffectDurationFix(schema177));
		Schema schema178 = builder.addSchema(3325, Schema3325::new);
		builder.addFixer(new ChoiceTypesFix(schema178, "Added displays", TypeReferences.ENTITY));
		Schema schema179 = builder.addSchema(3326, Schema3326::new);
		builder.addFixer(new ChoiceTypesFix(schema179, "Added Sniffer", TypeReferences.ENTITY));
		Schema schema180 = builder.addSchema(3327, Schema3327::new);
		builder.addFixer(new ChoiceTypesFix(schema180, "Archaeology", TypeReferences.BLOCK_ENTITY));
		Schema schema181 = builder.addSchema(3328, Schema3328::new);
		builder.addFixer(new ChoiceTypesFix(schema181, "Added interaction", TypeReferences.ENTITY));
		Schema schema182 = builder.addSchema(3438, Schema3438::new);
		builder.addFixer(
			RenameBlockEntityFix.create(schema182, "Rename Suspicious Sand to Brushable Block", replacing("minecraft:suspicious_sand", "minecraft:brushable_block"))
		);
		builder.addFixer(new EntityBrushableBlockFieldsRenameFix(schema182));
		builder.addFixer(
			ItemNameFix.create(
				schema182,
		...
		builder.addFixer(new ChoiceTypesFix(schema182, "Added calibrated sculk sensor", TypeReferences.BLOCK_ENTITY));
		Schema schema183 = builder.addSchema(3439, Schema3439::new);
		builder.addFixer(new UpdateSignTextFormatFix(schema183, "Updated sign text format for Signs", "minecraft:sign"));
		builder.addFixer(new UpdateSignTextFormatFix(schema183, "Updated sign text format for Hanging Signs", "minecraft:hanging_sign"));
		Schema schema184 = builder.addSchema(3440, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(
			new GameEventRenamesFix(
				schema184,
		...
		builder.addFixer(new RemoveFeatureTogglesFix(schema184, "Remove 1.20 feature toggle", Set.of("minecraft:update_1_20")));
		...
		Schema schema187 = builder.addSchema(3450, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(
		...
		Schema schema188 = builder.addSchema(3451, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new ChunkDeleteLightFix(schema188));
		Schema schema189 = builder.addSchema(3459, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new LegacyDragonFightFix(schema189));
		Schema schema190 = builder.addSchema(3564, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new DropInvalidSignDatafixDataFix(schema190, "minecraft:sign"));
		builder.addFixer(new DropInvalidSignDatafixDataFix(schema190, "minecraft:hanging_sign"));
		Schema schema191 = builder.addSchema(3565, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new RandomSequenceSettingsFix(schema191));
		Schema schema192 = builder.addSchema(3566, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new RenameScoreboardDisplaySlotFix(schema192));
		Schema schema193 = builder.addSchema(3568, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new StatusEffectFix(schema193));
		Schema schema194 = builder.addSchema(3682, Schema3682::new);
		...
		Schema schema199 = builder.addSchema(3799, Schema3799::new);
		...
		Schema schema206 = builder.addSchema(3808, 2, Schema3808_2::new);
		...
		Schema schema208 = builder.addSchema(3812, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new WolfHealthFix(schema208));
		Schema schema209 = builder.addSchema(3813, Schema3813::new);
		builder.addFixer(new BlockPosFormatFix(schema209));
		Schema schema210 = builder.addSchema(3814, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(
			new RenameEntityAttributesFix(schema210, "Rename jump strength attribute", replacing("minecraft:horse.jump_strength", "minecraft:generic.jump_strength"))
		);
		Schema schema211 = builder.addSchema(3816, Schema3816::new);
		builder.addFixer(new ChoiceTypesFix(schema211, "Added Bogged", TypeReferences.ENTITY));
		Schema schema212 = builder.addSchema(3818, Schema3818::new);
		builder.addFixer(new BeehiveFieldRenameFix(schema212));
		builder.addFixer(new EmptyItemInHotbarFix(schema212));
		Schema schema213 = builder.addSchema(3818, 1, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new BannerPatternFormatFix(schema213));
		Schema schema214 = builder.addSchema(3818, 2, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new TippedArrowPotionToItemFix(schema214));
		Schema schema215 = builder.addSchema(3818, 3, Schema3818_3::new);
		builder.addFixer(new WriteAndReadFix(schema215, "Inject data component types", TypeReferences.DATA_COMPONENTS));
		Schema schema216 = builder.addSchema(3818, 4, Schema3818_4::new);
		builder.addFixer(new ParticleUnflatteningFix(schema216));
		Schema schema217 = builder.addSchema(3818, 5, Schema3818_5::new);
		...
		Schema schema219 = builder.addSchema(3820, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new PlayerHeadBlockProfileFix(schema219));
		builder.addFixer(new LodestoneCompassComponentFix(schema219));
		Schema schema220 = builder.addSchema(3825, Schema3825::new);
		builder.addFixer(new ItemStackCustomNameToItemNameFix(schema220));
		builder.addFixer(new BannerCustomNameToItemNameFix(schema220));
		builder.addFixer(new TrialSpawnerConfigTagFix(schema220));
		...
		Schema schema238 = builder.addSchema(4173, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new RenameEntityNbtKeyFix(schema238, "Rename TNT Minecart fuse", "minecraft:tnt_minecart", Map.of("TNTFuse", "fuse")));
		Schema schema239 = builder.addSchema(4175, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new EquippableAssetRenameFix(schema239));
		builder.addFixer(new CustomModelDataExpansionFix(schema239));
		Schema schema240 = builder.addSchema(4176, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new BlockEntityLockToComponentFix(schema240));
		builder.addFixer(new InvalidLockComponentPredicateFix(schema240));
		Schema schema241 = builder.addSchema(4180, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new RemoveFeatureTogglesFix(schema241, "Remove Winter Drop toggle", Set.of("minecraft:winter_drop")));
		Schema schema242 = builder.addSchema(4181, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new BlockEntityFurnaceBurnTimeFix(schema242, "minecraft:furnace"));
		builder.addFixer(new BlockEntityFurnaceBurnTimeFix(schema242, "minecraft:smoker"));
		builder.addFixer(new BlockEntityFurnaceBurnTimeFix(schema242, "minecraft:blast_furnace"));
		Schema schema243 = builder.addSchema(4187, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(
			new EntityAttributeBaseFix(
				schema243, "Villager follow range fix undo", "minecraft:villager", "minecraft:follow_range", followRange -> followRange == 48.0 ? 16.0 : followRange
			)
		);
		builder.addFixer(
			new EntityAttributeBaseFix(
				schema243, "Bee follow range fix", "minecraft:bee", "minecraft:follow_range", followRange -> followRange == 48.0 ? 16.0 : followRange
			)
		);
		builder.addFixer(
			new EntityAttributeBaseFix(
				schema243, "Allay follow range fix", "minecraft:allay", "minecraft:follow_range", followRange -> followRange == 48.0 ? 16.0 : followRange
			)
		);
		builder.addFixer(
			new EntityAttributeBaseFix(
				schema243, "Llama follow range fix", "minecraft:llama", "minecraft:follow_range", followRange -> followRange == 40.0 ? 16.0 : followRange
			)
		);
		builder.addFixer(
			new EntityAttributeBaseFix(
				schema243, "Piglin Brute follow range fix", "minecraft:piglin_brute", "minecraft:follow_range", followRange -> followRange == 16.0 ? 12.0 : followRange
			)
		);
		builder.addFixer(
			new EntityAttributeBaseFix(
				schema243, "Warden follow range fix", "minecraft:warden", "minecraft:follow_range", followRange -> followRange == 16.0 ? 24.0 : followRange
			)
		);
		Schema schema244 = builder.addSchema(4290, Schema4290::new);
		builder.addFixer(new UnflattenTextComponentFix(schema244));
		Schema schema245 = builder.addSchema(4291, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new LegacyHoverEventFix(schema245));
		builder.addFixer(new TextComponentStringyFlagsFix(schema245));
		Schema schema246 = builder.addSchema(4292, Schema4292::new);
		builder.addFixer(new TextComponentHoverAndClickEventFix(schema246));
		Schema schema247 = builder.addSchema(4293, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new DropChancesFormatFix(schema247));
		Schema schema248 = builder.addSchema(4294, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(
			new SimpleBlockPropertyRenameFix(
				schema248, "CreakingHeartBlockStateFix", "minecraft:creaking_heart", "active", "creaking_heart_state", value -> value.equals("true") ? "awake" : "uprooted"
			)
		);
		Schema schema249 = builder.addSchema(4295, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new BlendingDataFix(schema249));
		Schema schema250 = builder.addSchema(4296, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new AreaEffectCloudDurationScaleFix(schema250));
		Schema schema251 = builder.addSchema(4297, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new ForcedChunkToTicketFix(schema251));
		Schema schema252 = builder.addSchema(4299, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new ItemVariantComponentizationFix(schema252));
		Schema schema253 = builder.addSchema(4300, Schema4300::new);
		builder.addFixer(new SaddleEquipmentSlotFix(schema253));
		Schema schema254 = builder.addSchema(4301, Schema4301::new);
		builder.addFixer(new EquipmentFormatFix(schema254));
		Schema schema255 = builder.addSchema(4302, Schema4302::new);
		builder.addFixer(new ChoiceTypesFix(schema255, "Added Test and Test Instance Block Entities", TypeReferences.BLOCK_ENTITY));
		Schema schema256 = builder.addSchema(4303, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new EntityFallDistanceFloatToDoubleFix(schema256, TypeReferences.ENTITY));
		builder.addFixer(new EntityFallDistanceFloatToDoubleFix(schema256, TypeReferences.PLAYER));
		Schema schema257 = builder.addSchema(4305, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new SimpleBlockPropertyRenameFix(schema257, "rename test block mode", "minecraft:test_block", "test_block_mode", "mode", value -> value));
		Schema schema258 = builder.addSchema(4306, Schema4306::new);
		builder.addFixer(new ThrownPotionSplitFix(schema258));
		Schema schema259 = builder.addSchema(4307, Schema4307::new);
		builder.addFixer(new TooltipDisplayComponentFix(schema259));
		Schema schema260 = builder.addSchema(4309, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new RaidRenamesDataFix(schema260));
		builder.addFixer(new ChunkTicketUnpackPosFix(schema260));
		Schema schema261 = builder.addSchema(4311, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(
			new AdvancementRenameFix(schema261, false, "Use lodestone category change", replacing("minecraft:nether/use_lodestone", "minecraft:adventure/use_lodestone"))
		);
		Schema schema262 = builder.addSchema(4312, Schema4312::new);
		builder.addFixer(new PlayerEquipmentFix(schema262));
		Schema schema263 = builder.addSchema(4314, EMPTY_IDENTIFIER_NORMALIZE);
		builder.addFixer(new InlineBlockPosFormatFix(schema263));
	}
