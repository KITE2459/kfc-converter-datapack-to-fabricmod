...
public class AttributeIdFix extends DataFix {
	private static final Map<UUID, String> UUID_TO_ID = ImmutableMap.<UUID, String>builder()
	...
	protected TypeRewriteRule makeRule() {
		...
		return TypeRewriteRule.seq(
			this.fixTypeEverywhereTyped(
				"AttributeIdFix (ItemStack)", type, typed -> typed.updateTyped(opticFinder, typedx -> typedx.update(DSL.remainderFinder(), AttributeIdFix::fixItemStack))
			),
			this.fixTypeEverywhereTyped("AttributeIdFix (Entity)", this.getInputSchema().getType(TypeReferences.ENTITY), AttributeIdFix::fixEntity),
			this.fixTypeEverywhereTyped("AttributeIdFix (Player)", this.getInputSchema().getType(TypeReferences.PLAYER), AttributeIdFix::fixEntity)
		);
	}
	...
	private static Stream<Dynamic<?>> fixModifiers(Stream<Dynamic<?>> dynamicStream) {
		...
		dynamicStream.forEach(modifierDynamic -> {
			UUID uUID = getUuidFromIntArray(modifierDynamic.get("uuid").asIntStream().toArray());
			String string = modifierDynamic.get("name").asString("");
			String string2 = uUID != null ? (String)UUID_TO_ID.get(uUID) : null;
			String string3 = (String)NAME_TO_ID.get(string);
			if (string2 != null) {
	...
	private static Dynamic<?> renameOtherFields(Dynamic<?> attributeDynamic) {
		return attributeDynamic.renameField("UUID", "uuid")
			.renameField("Name", "name")
			.renameField("Amount", "amount")
			.renameAndFixField("Operation", "operation", operationDynamic -> {
	...
	private static Dynamic<?> fixItemStack(Dynamic<?> stackDataDynamic) {
		return stackDataDynamic.update(
			"minecraft:attribute_modifiers",
			attributeModifiersDynamic -> attributeModifiersDynamic.update(
				"modifiers",
				modifiersDynamic -> DataFixUtils.orElse(
					modifiersDynamic.asStreamOpt().result().map(AttributeIdFix::method_60683).map(modifiersDynamic::createList), modifiersDynamic
				)
			)
		);
	}
	...
	private static Dynamic<?> fixAttribute(Dynamic<?> attributeDynamic) {
		return attributeDynamic.renameField("Name", "id")
			...
			.renameAndFixField(
				...
				modifiersDynamic -> DataFixUtils.orElse(
					modifiersDynamic.asStreamOpt()
						.result()
						.map(stream -> stream.map(AttributeIdFix::renameOtherFields))
						.map(AttributeIdFix::method_60683)
						.map(attributeDynamic::createList),
					modifiersDynamic
				)
			);
	}
	...
	private static Typed<?> fixEntity(Typed<?> entityTyped) {
		return entityTyped.update(
			DSL.remainderFinder(),
			remainder -> remainder.renameAndFixField(
				"Attributes",
				"attributes",
				attributesDynamic -> DataFixUtils.orElse(
					attributesDynamic.asStreamOpt().result().map(stream -> stream.map(AttributeIdFix::fixAttribute)).map(attributesDynamic::createList), attributesDynamic
				)
			)
		);
	}
	...
	public static UUID getUuidFromIntArray(int[] uuidArray) {
		return uuidArray.length != 4 ? null : new UUID((long)uuidArray[0] << 32 | uuidArray[1] & 4294967295L, (long)uuidArray[2] << 32 | uuidArray[3] & 4294967295L);
	}
}
