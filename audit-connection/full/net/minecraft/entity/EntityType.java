...
public class EntityType<T extends Entity> implements ToggleableFeature, TypeFilter<Entity, T> {
	...
	public static final EntityType<CreakingEntity> CREAKING = register(
		"creaking", EntityType.Builder.create(CreakingEntity::new, SpawnGroup.MONSTER).dimensions(0.9F, 2.7F).eyeHeight(2.3F).maxTrackingRange(8)
	);
	...
	public static final EntityType<EggEntity> EGG = register(
		"egg",
		EntityType.Builder.<EggEntity>create(EggEntity::new, SpawnGroup.MISC).dropsNothing().dimensions(0.25F, 0.25F).maxTrackingRange(4).trackingTickInterval(10)
	);
	...
	public static final EntityType<InteractionEntity> INTERACTION = register(
		"interaction", EntityType.Builder.create(InteractionEntity::new, SpawnGroup.MISC).dropsNothing().dimensions(0.0F, 0.0F).maxTrackingRange(10)
	);
	...
	public static final EntityType<ShulkerBulletEntity> SHULKER_BULLET = register(
		"shulker_bullet",
		EntityType.Builder.<ShulkerBulletEntity>create(ShulkerBulletEntity::new, SpawnGroup.MISC).dropsNothing().dimensions(0.3125F, 0.3125F).maxTrackingRange(8)
	);
	...
	public T spawnFromItemStack(
		ServerWorld world, @Nullable ItemStack stack, @Nullable LivingEntity spawner, BlockPos pos, SpawnReason spawnReason, boolean alignPosition, boolean invertY
	) {
		Consumer<T> consumer;
		if (stack != null) {
	...
	public static <T extends Entity> Consumer<T> nbtCopier(Consumer<T> chained, World world, ItemStack stack, @Nullable LivingEntity spawner) {
		NbtComponent nbtComponent = stack.getOrDefault(DataComponentTypes.ENTITY_DATA, NbtComponent.DEFAULT);
		return !nbtComponent.isEmpty() ? chained.andThen(entity -> loadFromEntityNbt(world, spawner, entity, nbtComponent)) : chained;
	}
	...
	public T create(ServerWorld world, @Nullable Consumer<T> afterConsumer, BlockPos pos, SpawnReason reason, boolean alignPosition, boolean invertY) {
		...
		if (entity == null) {
			...
			entity.refreshPositionAndAngles(pos.getX() + 0.5, pos.getY() + d, pos.getZ() + 0.5, MathHelper.wrapDegrees(world.random.nextFloat() * 360.0F), 0.0F);
			if (entity instanceof MobEntity mobEntity) {
	...
	public static void loadFromEntityNbt(World world, @Nullable LivingEntity spawner, @Nullable Entity entity, NbtComponent nbt) {
		...
		if (minecraftServer != null && entity != null) {
			EntityType<?> entityType = nbt.getRegistryValueOfId(minecraftServer.getRegistryManager(), RegistryKeys.ENTITY_TYPE);
			if (entity.getType() == entityType) {
				if (world.isClient
					|| !entity.getType().canPotentiallyExecuteCommands()
					|| spawner instanceof PlayerEntity playerEntity && minecraftServer.getPlayerManager().isOperator(playerEntity.getGameProfile())) {
	...
	public static Optional<Entity> getEntityFromNbt(NbtCompound nbt, World world, SpawnReason reason) {
		return Util.ifPresentOrElse(
			fromNbt(nbt).map(type -> type.create(world, reason)),
			entity -> entity.readNbt(nbt),
			() -> LOGGER.warn("Skipping Entity with id {}", nbt.getString("id", "[invalid]"))
		);
	}
	...
	public static Entity loadEntityWithPassengers(NbtCompound nbt, World world, SpawnReason reason, Function<Entity, Entity> entityProcessor) {
		return (Entity)loadEntityFromNbt(nbt, world, reason).map(entityProcessor).map(entity -> {
			NbtList nbtList = nbt.getListOrEmpty("Passengers");

			for (int i = 0; i < nbtList.size(); i++) {
				Entity entity2 = loadEntityWithPassengers(nbtList.getCompoundOrEmpty(i), world, reason, entityProcessor);
				if (entity2 != null) {
	...
	public static Stream<Entity> streamFromNbt(List<? extends NbtElement> entityNbtList, World world, SpawnReason reason) {
		return entityNbtList.stream().flatMap(nbt -> nbt.asCompound().stream()).mapMulti((nbt, callback) -> loadEntityWithPassengers(nbt, world, reason, entity -> {
	...
	public static class Builder<T extends Entity> implements net.fabricmc.fabric.api.object.builder.v1.entity.FabricEntityType.Builder<T> {
		...
		private RegistryKeyedValue<EntityType<?>, Optional<RegistryKey<LootTable>>> lootTable = registryKey -> Optional.of(
