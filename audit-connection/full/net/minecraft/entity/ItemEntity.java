...
public class ItemEntity extends Entity implements Ownable {
	...
	public void tick() {
		if (this.getStack().isEmpty()) {
			...
			boolean bl = MathHelper.floor(this.lastX) != MathHelper.floor(this.getX())
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.putNullable("Thrower", Uuids.INT_STREAM_CODEC, this.throwerUuid);
		nbt.putNullable("Owner", Uuids.INT_STREAM_CODEC, this.owner);
		if (!this.getStack().isEmpty()) {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		this.health = nbt.getShort("Health", (short)5);
		this.itemAge = nbt.getShort("Age", (short)0);
		this.pickupDelay = nbt.getShort("PickupDelay", (short)0);
		this.owner = (UUID)nbt.get("Owner", Uuids.INT_STREAM_CODEC).orElse(null);
		this.throwerUuid = (UUID)nbt.get("Thrower", Uuids.INT_STREAM_CODEC).orElse(null);
		this.thrower = null;
		RegistryOps<NbtElement> registryOps = this.getRegistryManager().getOps(NbtOps.INSTANCE);
		this.setStack((ItemStack)nbt.get("Item", ItemStack.CODEC, registryOps).orElse(ItemStack.EMPTY));
		if (this.getStack().isEmpty()) {
			this.discard();
		}
	}
