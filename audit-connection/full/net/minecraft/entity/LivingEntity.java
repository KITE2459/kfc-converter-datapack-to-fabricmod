...
public abstract class LivingEntity extends Entity implements Attackable {
	...
	public void baseTick() {
		...
		if (this.isAlive() && this.getWorld() instanceof ServerWorld serverWorld2) {
	...
	public void remove(Entity.RemovalReason reason) {
		if ((reason == Entity.RemovalReason.KILLED || reason == Entity.RemovalReason.DISCARDED) && this.getWorld() instanceof ServerWorld serverWorld) {
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		if (!this.activeStatusEffects.isEmpty()) {
			nbt.put("active_effects", StatusEffectInstance.CODEC.listOf(), registryOps, List.copyOf(this.activeStatusEffects.values()));
		}

		nbt.putBoolean("FallFlying", this.isGliding());
		this.getSleepingPosition().ifPresent(pos -> nbt.put("sleeping_pos", BlockPos.CODEC, pos));
		...
		if (this.attackingPlayer != null) {
			this.attackingPlayer.writeNbt(nbt, "last_hurt_by_player");
			nbt.putInt("last_hurt_by_player_memory_time", this.playerHitTimer);
		}

		if (this.attackerReference != null) {
			this.attackerReference.writeNbt(nbt, "last_hurt_by_mob");
			nbt.putInt("ticks_since_last_hurt_by_mob", this.age - this.lastAttackedTime);
		}

		if (!this.equipment.isEmpty()) {
			nbt.put("equipment", EntityEquipment.CODEC, registryOps, this.equipment);
		}
	}
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		this.setAbsorptionAmountUnclamped(nbt.getFloat("AbsorptionAmount", 0.0F));
		if (this.getWorld() != null && !this.getWorld().isClient) {
			nbt.getList("attributes").ifPresent(this.getAttributes()::readNbt);
		}

		RegistryOps<NbtElement> registryOps = this.getRegistryManager().getOps(NbtOps.INSTANCE);
		List<StatusEffectInstance> list = (List<StatusEffectInstance>)nbt.get("active_effects", StatusEffectInstance.CODEC.listOf(), registryOps).orElse(List.of());
		this.activeStatusEffects.clear();
		...
		this.setHealth(nbt.getFloat("Health", this.getMaxHealth()));
		this.hurtTime = nbt.getShort("HurtTime", (short)0);
		this.deathTime = nbt.getShort("DeathTime", (short)0);
		this.lastAttackedTime = nbt.getInt("HurtByTimestamp", 0);
		nbt.getString("Team").ifPresent(team -> {
		...
		this.setFlag(Entity.GLIDING_FLAG_INDEX, nbt.getBoolean("FallFlying", false));
		nbt.get("sleeping_pos", BlockPos.CODEC).ifPresentOrElse(pos -> {
		...
		nbt.getCompound("Brain").ifPresent(brain -> this.brain = this.deserializeBrain(new Dynamic<>(NbtOps.INSTANCE, brain)));
		this.attackingPlayer = LazyEntityReference.fromNbt(nbt, "last_hurt_by_player");
		this.playerHitTimer = nbt.getInt("last_hurt_by_player_memory_time", 0);
		this.attackerReference = LazyEntityReference.fromNbt(nbt, "last_hurt_by_mob");
		this.lastAttackedTime = nbt.getInt("ticks_since_last_hurt_by_mob", 0) + this.age;
		this.equipment.copyFrom((EntityEquipment)nbt.get("equipment", EntityEquipment.CODEC, registryOps).orElseGet(EntityEquipment::new));
	}
		...
		if (this.getWorld() instanceof ServerWorld serverWorld) {
			...
			if (!list.isEmpty()) {
				...
				if (this.random.nextInt(i * j) == 0) {
					this.getWorld().addParticleClient(Util.getRandom(list, this.random), this.getParticleX(0.5), this.getRandomBodyY(), this.getParticleZ(0.5), 1.0, 1.0, 1.0);
				}
			}
		}
	}
	...
	private void playEquipmentBreakEffects(ItemStack stack) {
		if (!stack.isEmpty()) {
			...
			if (registryEntry != null && !this.isSilent()) {
				this.getWorld()
					.playSoundClient(
						this.getX(), this.getY(), this.getZ(), registryEntry.value(), this.getSoundCategory(), 0.8F, 0.8F + this.getWorld().random.nextFloat() * 0.4F, false
					);
			}
	...
	protected void dropExperience(ServerWorld world, @Nullable Entity attacker) {
		if (!this.isExperienceDroppingDisabled()
			&& (this.shouldAlwaysDropExperience() || this.playerHitTimer > 0 && this.shouldDropExperience() && world.getGameRules().getBoolean(GameRules.DO_MOB_LOOT))) {
		...
		switch (status) {
			...
			case 46:
				...
				for (int j = 0; j < 128; j++) {
					...
					this.getWorld().addParticleClient(ParticleTypes.PORTAL, e, k, l, f, g, h);
				}
				break;
			case 47:
	...
	public void sleep(BlockPos pos) {
		...
		if (blockState.getBlock() instanceof BedBlock) {
			this.getWorld().setBlockState(pos, blockState.with(BedBlock.OCCUPIED, true), Block.NOTIFY_ALL);
		}
	...
	public float getWeaponDisableBlockingForSeconds() {
		WeaponComponent weaponComponent = this.getWeaponStack().get(DataComponentTypes.WEAPON);
