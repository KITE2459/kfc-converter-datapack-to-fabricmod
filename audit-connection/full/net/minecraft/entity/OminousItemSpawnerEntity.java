...
public class OminousItemSpawnerEntity extends Entity {
	...
	protected void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.setItem((ItemStack)nbt.get("item", ItemStack.CODEC, registryOps).orElse(ItemStack.EMPTY));
		this.spawnItemAfterTicks = nbt.getLong("spawn_item_after_ticks", 0L);
	}
	...
	protected void writeCustomDataToNbt(NbtCompound nbt) {
		if (!this.getItem().isEmpty()) {
			RegistryOps<NbtElement> registryOps = this.getRegistryManager().getOps(NbtOps.INSTANCE);
			nbt.put("item", ItemStack.CODEC, registryOps, this.getItem());
		}
