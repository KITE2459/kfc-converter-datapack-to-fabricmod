...
public abstract class EntityNavigation {
	...
	protected void checkTimeouts(Vec3d currentPos) {
		...
		if (this.currentPath != null && !this.currentPath.isFinished()) {
			...
			if (vec3i.equals(this.lastNodePosition)) {
				...
				this.currentNodeTimeout = this.entity.getMovementSpeed() > 0.0F ? d / this.entity.getMovementSpeed() * 20.0 : 0.0;
			}
	...
	public boolean shouldRecalculatePath(BlockPos pos) {
		if (this.inRecalculationCooldown) {
			...
			Vec3d vec3d = new Vec3d((pathNode.x + this.entity.getX()) / 2.0, (pathNode.y + this.entity.getY()) / 2.0, (pathNode.z + this.entity.getZ()) / 2.0);
