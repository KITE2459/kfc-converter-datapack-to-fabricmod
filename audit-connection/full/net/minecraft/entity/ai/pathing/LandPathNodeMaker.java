...
public class LandPathNodeMaker extends PathNodeMaker {
	...
	public int getSuccessors(PathNode[] successors, PathNode node) {
		...
		for (Direction directionx : Direction.Type.HORIZONTAL) {
			...
			if (this.isValidDiagonalSuccessor(node, this.successors[directionx.getHorizontalQuarterTurns()], this.successors[direction2.getHorizontalQuarterTurns()])) {
	...
	protected boolean isValidDiagonalSuccessor(PathNode xNode, @Nullable PathNode zNode, @Nullable PathNode xDiagNode) {
		if (xDiagNode == null || zNode == null || xDiagNode.y > xNode.y || zNode.y > xNode.y) {
			...
			boolean bl = xDiagNode.type == PathNodeType.FENCE && zNode.type == PathNodeType.FENCE && this.entity.getWidth() < 0.5;
	...
	private boolean isBlocked(PathNode node) {
		...
		Vec3d vec3d = new Vec3d(
			node.x - this.entity.getX() + box.getLengthX() / 2.0,
			node.y - this.entity.getY() + box.getLengthY() / 2.0,
			node.z - this.entity.getZ() + box.getLengthZ() / 2.0
		);
		int i = MathHelper.ceil(vec3d.length() / box.getAverageSideLength());
		vec3d = vec3d.multiply(1.0F / i);
	...
	protected double getFeetY(BlockPos pos) {
		...
		return (this.canSwim() || this.isAmphibious()) && blockView.getFluidState(pos).isIn(FluidTags.WATER) ? pos.getY() + 0.5 : getFeetY(blockView, pos);
	}
	...
	public static double getFeetY(BlockView world, BlockPos pos) {
		...
		return blockPos.getY() + (voxelShape.isEmpty() ? 0.0 : voxelShape.getMax(Direction.Axis.Y));
	}
	...
	@Nullable
	protected PathNode getPathNode(int x, int y, int z, int maxYStep, double lastFeetY, Direction direction, PathNodeType nodeType) {
	...
	private double getStepHeight() {
		return Math.max(1.125, this.entity.getStepHeight());
	}
	...
	@Nullable
	private PathNode getJumpOnTopNode(int x, int y, int z, int maxYStep, double lastFeetY, Direction direction, PathNodeType nodeType, BlockPos.Mutable mutablePos) {
		...
		if (pathNode == null) {
			...
			double d = x - direction.getOffsetX() + 0.5;
			double e = z - direction.getOffsetZ() + 0.5;
			double f = this.entity.getWidth() / 2.0;
			Box box = new Box(
				d - f,
				this.getFeetY(mutablePos.set(d, (double)(y + 1), e)) + 0.001,
				e - f,
				d + f,
				this.entity.getHeight() + this.getFeetY(mutablePos.set((double)pathNode.x, (double)pathNode.y, (double)pathNode.z)) - 0.002,
				e + f
			);
