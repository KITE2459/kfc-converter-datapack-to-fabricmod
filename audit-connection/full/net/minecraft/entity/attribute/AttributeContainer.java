...
public class AttributeContainer {
	...
	public boolean resetToBaseValue(RegistryEntry<EntityAttribute> attribute) {
		if (!this.fallback.has(attribute)) {
			return false;
		} else {
			EntityAttributeInstance entityAttributeInstance = (EntityAttributeInstance)this.custom.get(attribute);
			if (entityAttributeInstance != null) {
				entityAttributeInstance.setBaseValue(this.fallback.getBaseValue(attribute));
			}
	...
	public void readNbt(NbtList nbt) {
		for (int i = 0; i < nbt.size(); i++) {
			NbtCompound nbtCompound = nbt.getCompoundOrEmpty(i);
			nbtCompound.get("id", EntityAttributeInstance.ENTRY_CODEC).map(this::getCustomInstance).ifPresent(instance -> instance.readNbt(nbtCompound));
		}
	}
}
