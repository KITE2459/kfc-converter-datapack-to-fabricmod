...
public class EntityAttributeInstance {
	...
	public static final Codec<RegistryEntry<EntityAttribute>> ENTRY_CODEC = Registries.ATTRIBUTE.getEntryCodec();
	...
	public NbtCompound toNbt() {
		...
		nbtCompound.put("id", ENTRY_CODEC, this.type);
		nbtCompound.putDouble("base", this.baseValue);
		if (!this.persistentModifiers.isEmpty()) {
			nbtCompound.put("modifiers", EntityAttributeModifier.CODEC.listOf(), List.copyOf(this.persistentModifiers.values()));
		}
	...
	public void readNbt(NbtCompound nbt) {
		this.baseValue = nbt.getDouble("base", 0.0);

		for (EntityAttributeModifier entityAttributeModifier : (List)nbt.get("modifiers", EntityAttributeModifier.CODEC.listOf()).orElse(List.of())) {
