...
public class WitherEntity extends HostileEntity implements RangedAttackMob {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.setInvulTimer(nbt.getInt("Invul", 0));
	...
	public void tickMovement() {
		...
		for (int jx = 0; jx < 3; jx++) {
			...
			this.getWorld()
				.addParticleClient(
					ParticleTypes.SMOKE, p + this.random.nextGaussian() * s, q + this.random.nextGaussian() * s, r + this.random.nextGaussian() * s, 0.0, 0.0, 0.0
				);
	...
	private void shootSkullAt(int headIndex, LivingEntity target) {
		this.shootSkullAt(
			headIndex, target.getX(), target.getY() + target.getStandingEyeHeight() * 0.5, target.getZ(), headIndex == 0 && this.random.nextFloat() < 0.001F
		);
	}
