...
public class EnderDragonEntity extends MobEntity implements Monster {
	...
	public void tickMovement() {
		...
		if (this.isDead()) {
			...
			this.getWorld().addParticleClient(ParticleTypes.EXPLOSION, this.getX() + f, this.getY() + 2.0 + g, this.getZ() + h, 0.0, 0.0, 0.0);
		} else {
	...
	protected void updatePostDeath() {
		...
		if (this.ticksSinceDeath >= 180 && this.ticksSinceDeath <= 200) {
			...
			float g = (this.random.nextFloat() - 0.5F) * 4.0F;
			float h = (this.random.nextFloat() - 0.5F) * 8.0F;
			this.getWorld().addParticleClient(ParticleTypes.EXPLOSION_EMITTER, this.getX() + f, this.getY() + 2.0 + g, this.getZ() + h, 0.0, 0.0, 0.0);
		}
		...
		if (this.getWorld() instanceof ServerWorld serverWorld) {
			if (this.ticksSinceDeath > 150 && this.ticksSinceDeath % 5 == 0 && serverWorld.getGameRules().getBoolean(GameRules.DO_MOB_LOOT)) {
				ExperienceOrbEntity.spawn(serverWorld, this.getPos(), MathHelper.floor(i * 0.08F));
			}
		...
		if (this.ticksSinceDeath == 200 && this.getWorld() instanceof ServerWorld serverWorld2) {
			if (serverWorld2.getGameRules().getBoolean(GameRules.DO_MOB_LOOT)) {
				ExperienceOrbEntity.spawn(serverWorld2, this.getPos(), MathHelper.floor(i * 0.2F));
			}
	...
	public int getNearestPathNodeIndex() {
		if (this.pathNodes[0] == null) {
			for (int i = 0; i < 24; i++) {
				...
				int m;
				if (i < 12) {
					l = MathHelper.floor(60.0F * MathHelper.cos(2.0F * ((float) -Math.PI + (float) (Math.PI / 12) * i)));
					m = MathHelper.floor(60.0F * MathHelper.sin(2.0F * ((float) -Math.PI + (float) (Math.PI / 12) * i)));
				} else if (i < 20) {
					int k = i - 12;
					l = MathHelper.floor(40.0F * MathHelper.cos(2.0F * ((float) -Math.PI + (float) (Math.PI / 8) * k)));
					m = MathHelper.floor(40.0F * MathHelper.sin(2.0F * ((float) -Math.PI + (float) (Math.PI / 8) * k)));
					j += 10;
				} else {
					int var7 = i - 20;
					l = MathHelper.floor(20.0F * MathHelper.cos(2.0F * ((float) -Math.PI + (float) (Math.PI / 4) * var7)));
					m = MathHelper.floor(20.0F * MathHelper.sin(2.0F * ((float) -Math.PI + (float) (Math.PI / 4) * var7)));
				}
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		nbt.getInt("DragonPhase").ifPresent(phase -> this.phaseManager.setPhase(PhaseType.getFromId(phase)));
		this.ticksSinceDeath = nbt.getInt("DragonDeathTime", 0);
	}
	...
	public void crystalDestroyed(ServerWorld world, EndCrystalEntity crystal, BlockPos pos, DamageSource source) {
		...
		if (source.getAttacker() instanceof PlayerEntity playerEntity) {
			...
			playerEntity2 = world.getClosestPlayer(CLOSE_PLAYER_PREDICATE, pos.getX(), pos.getY(), pos.getZ());
		}
