...
public class EnderDragonFight {
	...
	private void generateNewEndGateway() {
		if (!this.gateways.isEmpty()) {
			...
			int j = MathHelper.floor(96.0 * Math.cos(2.0 * (-Math.PI + (Math.PI / 20) * i)));
			int k = MathHelper.floor(96.0 * Math.sin(2.0 * (-Math.PI + (Math.PI / 20) * i)));
	...
	private EnderDragonEntity createDragon() {
		...
		if (enderDragonEntity != null) {
			...
			enderDragonEntity.refreshPositionAndAngles(this.origin.getX(), 128 + this.origin.getY(), this.origin.getZ(), this.world.random.nextFloat() * 360.0F, 0.0F);
	...
	public record Data(
		...
		public static final Codec<EnderDragonFight.Data> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
					...
					Codec.BOOL.lenientOptionalFieldOf("IsRespawning", false).forGetter(EnderDragonFight.Data::isRespawning),
