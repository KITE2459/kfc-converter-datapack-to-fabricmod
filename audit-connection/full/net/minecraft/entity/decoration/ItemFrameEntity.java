...
public class ItemFrameEntity extends AbstractDecorationEntity {
	...
	protected void setFacing(Direction facing) {
		...
		if (facing.getAxis().isHorizontal()) {
			this.setPitch(0.0F);
			this.setYaw(this.facing.getHorizontalQuarterTurns() * 90);
		} else {
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		if (!itemStack.isEmpty()) {
			...
			nbt.put("Item", ItemStack.CODEC, registryOps, itemStack);
		}
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		RegistryOps<NbtElement> registryOps = this.getRegistryManager().getOps(NbtOps.INSTANCE);
		ItemStack itemStack = (ItemStack)nbt.get("Item", ItemStack.CODEC, registryOps).orElse(ItemStack.EMPTY);
		...
		this.setRotation(nbt.getByte("ItemRotation", (byte)0), false);
		this.itemDropChance = nbt.getFloat("ItemDropChance", 1.0F);
		this.setFacing((Direction)nbt.get("Facing", Direction.INDEX_CODEC).orElse(Direction.DOWN));
		this.setInvisible(nbt.getBoolean("Invisible", false));
		this.fixed = nbt.getBoolean("Fixed", false);
	}
	...
	public Packet<ClientPlayPacketListener> createSpawnPacket(EntityTrackerEntry entityTrackerEntry) {
		return new EntitySpawnS2CPacket(this, this.facing.getIndex(), this.getAttachedBlockPos());
	}
