...
public class DrownedEntity extends ZombieEntity implements RangedAttackMob {
	...
	public void updateSwimming() {
		if (!this.getWorld().isClient) {
			if (this.canActVoluntarily() && this.isSubmergedInWater() && this.isTargetingUnderwater()) {
	...
	public void shootAt(LivingEntity target, float pullProgress) {
		...
		if (this.getWorld() instanceof ServerWorld serverWorld) {
			ProjectileEntity.spawnWithVelocity(tridentEntity, serverWorld, itemStack2, d, e + g * 0.2F, f, 1.6F, 14 - this.getWorld().getDifficulty().getId() * 4);
		}
	...
	static class LeaveWaterGoal extends MoveToTargetPosGoal {
		...
		public boolean canStart() {
			return super.canStart()
				...
				&& this.drowned.getY() >= this.drowned.getWorld().getSeaLevel() - 3;
		}
	...
	static class TargetAboveWaterGoal extends Goal {
		...
		@Override
		public boolean canStart() {
			return !this.drowned.getWorld().isDay() && this.drowned.isTouchingWater() && this.drowned.getY() < this.minY - 2;
		}
