	...
	public void tickMovement() {
		...
		if (this.getWorld().isClient && this.isInvisible()) {
			...
			if (this.hurtTime == 1 || this.age % 1200 == 0) {
				...
				this.getWorld()
					.playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_ILLUSIONER_MIRROR_MOVE, this.getSoundCategory(), 1.0F, 1.0F, false);
		...
		public boolean canStart() {
			if (!super.canStart()) {
				...
				return IllusionerEntity.this.getTarget().getId() == this.targetId
					? false
					: IllusionerEntity.this.getWorld().getLocalDifficulty(IllusionerEntity.this.getBlockPos()).isHarderThan(Difficulty.NORMAL.ordinal());
			}
		}
