...
public class PhantomEntity extends FlyingEntity implements Monster {
	...
	public void tick() {
		...
		if (this.getWorld().isClient) {
			float f = MathHelper.cos((this.getWingFlapTickOffset() + this.age) * 7.448451F * (float) (Math.PI / 180.0) + (float) Math.PI);
			float g = MathHelper.cos((this.getWingFlapTickOffset() + this.age + 1) * 7.448451F * (float) (Math.PI / 180.0) + (float) Math.PI);
			if (f > 0.0F && g <= 0.0F) {
				this.getWorld()
					.playSoundClient(
						this.getX(),
			...
			this.getWorld().addParticleClient(ParticleTypes.MYCELIUM, this.getX() + i, this.getY() + k, this.getZ() + j, 0.0, 0.0, 0.0);
			this.getWorld().addParticleClient(ParticleTypes.MYCELIUM, this.getX() - i, this.getY() + k, this.getZ() - j, 0.0, 0.0, 0.0);
		}
	}
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.setPhantomSize(nbt.getInt("size", 0));
	}
	...
	public EntityDimensions getBaseDimensions(EntityPose pose) {
		...
		return entityDimensions.scaled(1.0F + 0.15F * i);
	}
	...
	class PhantomMoveControl extends MoveControl {
		...
		public void tick() {
			...
			if (Math.abs(g) > 1.0E-5F) {
				...
				double p = this.targetSpeed * MathHelper.cos(o * (float) (Math.PI / 180.0)) * Math.abs(d / i);
				double q = this.targetSpeed * MathHelper.sin(o * (float) (Math.PI / 180.0)) * Math.abs(f / i);
				double r = this.targetSpeed * MathHelper.sin(n * (float) (Math.PI / 180.0)) * Math.abs(e / i);
