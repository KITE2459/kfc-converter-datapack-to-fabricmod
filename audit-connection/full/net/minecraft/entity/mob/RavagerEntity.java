...
public class RavagerEntity extends RaiderEntity {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.attackTick = nbt.getInt("AttackTick", 0);
		this.stunTick = nbt.getInt("StunTick", 0);
		this.roarTick = nbt.getInt("RoarTick", 0);
	}
	...
	private void spawnStunnedParticles() {
		if (this.random.nextInt(6) == 0) {
			double d = this.getX() - this.getWidth() * Math.sin(this.bodyYaw * (float) (Math.PI / 180.0)) + (this.random.nextDouble() * 0.6 - 0.3);
			double e = this.getY() + this.getHeight() - 0.3;
			double f = this.getZ() + this.getWidth() * Math.cos(this.bodyYaw * (float) (Math.PI / 180.0)) + (this.random.nextDouble() * 0.6 - 0.3);
			this.getWorld()
				.addParticleClient(EntityEffectParticleEffect.create(ParticleTypes.ENTITY_EFFECT, 0.49803922F, 0.5137255F, 0.57254905F), d, e, f, 0.0, 0.0, 0.0);
		}
	}
