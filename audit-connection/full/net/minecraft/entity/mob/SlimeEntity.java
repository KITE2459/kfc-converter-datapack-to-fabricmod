...
public class SlimeEntity extends MobEntity implements Monster {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		this.setSize(nbt.getInt("Size", 0) + 1, false);
		super.readCustomDataFromNbt(nbt);
		this.onGroundLastTick = nbt.getBoolean("wasOnGround", false);
	}
	...
	public void tick() {
		...
		if (this.isOnGround() && !this.onGroundLastTick) {
			...
			for (int i = 0; i < f * 16.0F; i++) {
				...
				this.getWorld().addParticleClient(this.getParticles(), this.getX() + k, this.getY(), this.getZ() + l, 0.0, 0.0, 0.0);
			}
		...
		if (!this.getWorld().isClient && i > 1 && this.isDead()) {
			...
			for (int l = 0; l < k; l++) {
				...
				this.convertTo(this.getType(), new EntityConversionContext(EntityConversionType.SPLIT_ON_DEATH, false, false, team), SpawnReason.TRIGGERED, newSlime -> {
					...
					newSlime.refreshPositionAndAngles(this.getX() + h, this.getY() + 0.5, this.getZ() + m, this.random.nextFloat() * 360.0F, 0.0F);
				});
			}
		}
