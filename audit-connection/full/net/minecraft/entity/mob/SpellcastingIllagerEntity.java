...
public abstract class SpellcastingIllagerEntity extends IllagerEntity {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.spellTicks = nbt.getInt("SpellTicks", 0);
	}
	...
	public void tick() {
		...
		if (this.getWorld().isClient && this.isSpellcasting()) {
			...
			this.getWorld()
				.addParticleClient(
					EntityEffectParticleEffect.create(ParticleTypes.ENTITY_EFFECT, f, g, h), this.getX() + j * d, this.getY() + e, this.getZ() + k * d, 0.0, 0.0, 0.0
				);
			this.getWorld()
				.addParticleClient(
					EntityEffectParticleEffect.create(ParticleTypes.ENTITY_EFFECT, f, g, h), this.getX() - j * d, this.getY() + e, this.getZ() - k * d, 0.0, 0.0, 0.0
		...
		public void tick() {
			if (SpellcastingIllagerEntity.this.getTarget() != null) {
				SpellcastingIllagerEntity.this.getLookControl()
					.lookAt(
						SpellcastingIllagerEntity.this.getTarget(), SpellcastingIllagerEntity.this.getMaxHeadRotation(), SpellcastingIllagerEntity.this.getMaxLookPitchChange()
					);
			}
		}
	}
