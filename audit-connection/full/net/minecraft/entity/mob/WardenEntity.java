...
public class WardenEntity extends HostileEntity implements Vibrations {
	...
	public void tick() {
		...
		if (this.getWorld().isClient()) {
			if (this.age % this.getHeartRate() == 0) {
				...
				if (!this.isSilent()) {
					this.getWorld()
						.playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_WARDEN_HEARTBEAT, this.getSoundCategory(), 5.0F, this.getSoundPitch(), false);
				}
			}
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.put("anger", WardenAngerManager.createCodec(this::isValidTarget), registryOps, this.angerManager);
		nbt.put("listener", Vibrations.ListenerData.CODEC, registryOps, this.vibrationListenerData);
	}
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		RegistryOps<NbtElement> registryOps = this.getRegistryManager().getOps(NbtOps.INSTANCE);
		this.angerManager = (WardenAngerManager)nbt.get("anger", WardenAngerManager.createCodec(this::isValidTarget), registryOps)
			.orElseGet(() -> new WardenAngerManager(this::isValidTarget, Collections.emptyList()));
		this.updateAnger();
		this.vibrationListenerData = (Vibrations.ListenerData)nbt.get("listener", Vibrations.ListenerData.CODEC, registryOps).orElseGet(Vibrations.ListenerData::new);
	}
	...
	class VibrationCallback implements Vibrations.Callback {
		...
		public boolean accepts(ServerWorld world, BlockPos pos, RegistryEntry<GameEvent> event, GameEvent.Emitter emitter) {
			return !WardenEntity.this.isAiDisabled()
				...
				? !(emitter.sourceEntity() instanceof LivingEntity livingEntity && !WardenEntity.this.isValidTarget(livingEntity))
				: false;
		}
