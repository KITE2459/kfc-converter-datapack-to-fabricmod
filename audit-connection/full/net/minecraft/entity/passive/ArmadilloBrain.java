...
public class ArmadilloBrain {
	...
	private static void addCoreActivities(Brain<ArmadilloEntity> brain) {
		brain.setTaskList(
			Activity.CORE,
			0,
			ImmutableList.of(new StayAboveWaterTask<>(0.8F), new ArmadilloBrain.UnrollAndFleeTask(2.0F), new UpdateLookControlTask(45, 90), new MoveToTargetTask() {
				@Override
				protected boolean shouldRun(ServerWorld serverWorld, MobEntity mobEntity) {
					return mobEntity instanceof ArmadilloEntity armadilloEntity && armadilloEntity.isNotIdle() ? false : super.shouldRun(serverWorld, mobEntity);
				}
			...
			ImmutableList.of(
				...
				Pair.of(
					...
					new RandomTask<>(
						ImmutableList.of(
							...
							Pair.of(WalkTowardsClosestAdultTask.create(WALK_TOWARDS_CLOSEST_ADULT_RANGE, 1.25F), 1)
						)
					)
				),
				...
				Pair.of(
					...
					new RandomTask<>(
						...
						ImmutableList.of(Pair.of(StrollTask.create(1.0F), 1), Pair.of(GoToLookTargetTask.create(1.0F, 3), 1), Pair.of(new WaitTask(30, 60), 1))
					)
				)
			)
		);
		...
		protected void keepRunning(ServerWorld serverWorld, ArmadilloEntity armadilloEntity, long l) {
			...
			if (armadilloEntity.shouldSwitchToScaredState()) {
				...
				if (state == ArmadilloEntity.State.SCARED) {
					...
					if (m < ArmadilloEntity.State.UNROLLING.getLengthInTicks()) {
				...
				} else if (state == ArmadilloEntity.State.UNROLLING && m > ArmadilloEntity.State.UNROLLING.getLengthInTicks()) {
					armadilloEntity.setState(ArmadilloEntity.State.SCARED);
				}
			}
		}
