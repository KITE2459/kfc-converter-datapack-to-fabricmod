...
public class AxolotlEntity extends AnimalEntity implements Bucketable {
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.put("Variant", AxolotlEntity.Variant.INDEX_CODEC, this.getVariant());
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.setVariant((AxolotlEntity.Variant)nbt.get("Variant", AxolotlEntity.Variant.INDEX_CODEC).orElse(AxolotlEntity.Variant.DEFAULT));
	...
	public AxolotlEntity.Variant getVariant() {
		return AxolotlEntity.Variant.byIndex(this.dataTracker.get(VARIANT));
	}
	...
	public <T> T get(ComponentType<? extends T> type) {
		return type == DataComponentTypes.AXOLOTL_VARIANT ? castComponentValue((ComponentType<T>)type, this.getVariant()) : super.get(type);
	}
	...
	protected <T> boolean setApplicableComponent(ComponentType<T> type, T value) {
		if (type == DataComponentTypes.AXOLOTL_VARIANT) {
			this.setVariant(castComponentValue(DataComponentTypes.AXOLOTL_VARIANT, value));
			return true;
		} else {
	...
	public boolean damage(ServerWorld world, DamageSource source, float amount) {
		...
		if (!this.isAiDisabled()
			...
			&& (this.getWorld().random.nextInt(3) < amount || f / this.getMaxHealth() < 0.5F)
			&& amount < f
	...
	public void copyDataFromNbt(NbtCompound nbt) {
		...
		nbt.getLong("HuntingCooldown")
			.ifPresentOrElse(
				huntingCooldown -> this.getBrain().remember(MemoryModuleType.HAS_HUNTING_COOLDOWN, true, nbt.getLong("HuntingCooldown", 0L)),
	...
	public static enum Variant implements StringIdentifiable {
		...
		public static final AxolotlEntity.Variant DEFAULT = LUCY;
		private static final IntFunction<AxolotlEntity.Variant> INDEX_MAPPER = ValueLists.createIndexToValueFunction(
			AxolotlEntity.Variant::getIndex, values(), ValueLists.OutOfBoundsHandling.ZERO
		);
		public static final PacketCodec<ByteBuf, AxolotlEntity.Variant> PACKET_CODEC = PacketCodecs.indexed(INDEX_MAPPER, AxolotlEntity.Variant::getIndex);
		public static final Codec<AxolotlEntity.Variant> CODEC = StringIdentifiable.createCodec(AxolotlEntity.Variant::values);
		@Deprecated
		public static final Codec<AxolotlEntity.Variant> INDEX_CODEC = Codec.INT.xmap(INDEX_MAPPER::apply, AxolotlEntity.Variant::getIndex);
		...
		public static AxolotlEntity.Variant byIndex(int index) {
			return (AxolotlEntity.Variant)INDEX_MAPPER.apply(index);
		}
