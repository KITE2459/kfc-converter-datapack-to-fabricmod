...
public class BeeEntity extends AnimalEntity implements Angerable, Flutterer {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.setHasNectar(nbt.getBoolean("HasNectar", false));
		this.setHasStung(nbt.getBoolean("HasStung", false));
		this.ticksSincePollination = nbt.getInt("TicksSincePollination", 0);
		this.cannotEnterHiveTicks = nbt.getInt("CannotEnterHiveTicks", 0);
		this.cropsGrownSincePollination = nbt.getInt("CropsGrownSincePollination", 0);
		this.hivePos = (BlockPos)nbt.get("hive_pos", BlockPos.CODEC).orElse(null);
	...
	BeehiveBlockEntity getHive() {
		if (this.hivePos == null) {
			...
			return this.isTooFar(this.hivePos) ? null : (BeehiveBlockEntity)this.getWorld().getBlockEntity(this.hivePos, BlockEntityType.BEEHIVE).orElse(null);
		}
	}
	...
	class ValidateFlowerGoal extends BeeEntity.NotAngryGoal {
		...
		@Override
		public boolean canBeeStart() {
			return BeeEntity.this.getWorld().getTime() > this.lastValidateTime + this.ticksUntilNextValidate;
		}
