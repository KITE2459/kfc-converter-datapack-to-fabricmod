...
public class DolphinEntity extends WaterAnimalEntity {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.setHasFish(nbt.getBoolean("GotFish", false));
		this.setMoistness(nbt.getInt("Moistness", 2400));
	}
	...
	public void tick() {
		...
		if (this.isAiDisabled()) {
			...
			if (this.getWorld().isClient && this.isTouchingWater() && this.getVelocity().lengthSquared() > 0.03) {
				...
				for (int i = 0; i < 2; i++) {
					this.getWorld()
						.addParticleClient(ParticleTypes.DOLPHIN, this.getX() - vec3d.x * h + f, this.getY() - vec3d.y, this.getZ() - vec3d.z * h + g, 0.0, 0.0, 0.0);
					this.getWorld()
						.addParticleClient(ParticleTypes.DOLPHIN, this.getX() - vec3d.x * h - f, this.getY() - vec3d.y, this.getZ() - vec3d.z * h - g, 0.0, 0.0, 0.0);
				}
			}
	...
	static class LeadToNearbyTreasureGoal extends Goal {
		...
		public boolean shouldContinue() {
			...
			return blockPos == null
				? false
				: !BlockPos.ofFloored(blockPos.getX(), this.dolphin.getY(), blockPos.getZ()).isWithinDistance(this.dolphin.getPos(), 4.0)
			...
			if (!stack.isEmpty()) {
				...
				itemEntity.setVelocity(
					0.3F
						...
						+ MathHelper.cos(g) * h,
					0.3F * MathHelper.sin(DolphinEntity.this.getPitch() * (float) (Math.PI / 180.0)) * 1.5F,
					0.3F * MathHelper.cos(DolphinEntity.this.getYaw() * (float) (Math.PI / 180.0)) * MathHelper.cos(DolphinEntity.this.getPitch() * (float) (Math.PI / 180.0))
						+ MathHelper.sin(g) * h
				);
