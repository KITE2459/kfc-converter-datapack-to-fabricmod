...
public class FoxEntity extends AnimalEntity {
	private static final TrackedData<Integer> VARIANT = DataTracker.registerData(FoxEntity.class, TrackedDataHandlerRegistry.INTEGER);
	...
	private static final TrackedData<Optional<LazyEntityReference<LivingEntity>>> OWNER = DataTracker.registerData(
		FoxEntity.class, TrackedDataHandlerRegistry.LAZY_ENTITY_REFERENCE
	);
	...
	private static final Codec<List<LazyEntityReference<LivingEntity>>> TRUSTED_ENTITIES_CODEC = LazyEntityReference.<LivingEntity>createCodec().listOf();
	...
	protected void initGoals() {
		...
		this.goalSelector
			.add(
				4,
				new FleeEntityGoal(
					this, PlayerEntity.class, 16.0F, 1.6, 1.4, entity -> NOTICEABLE_PLAYER_FILTER.test(entity) && !this.canTrust(entity) && !this.isAggressive()
				)
			);
		...
		this.targetSelector
			.add(
				3,
				new FoxEntity.DefendFriendGoal(LivingEntity.class, false, false, (entity, world) -> JUST_ATTACKED_SOMETHING_FILTER.test(entity) && !this.canTrust(entity))
			);
	}
	...
	public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
		...
		FoxEntity.Variant variant = FoxEntity.Variant.fromBiome(registryEntry);
		boolean bl = false;
	...
	public FoxEntity.Variant getVariant() {
		return FoxEntity.Variant.byIndex(this.dataTracker.get(VARIANT));
	}
	...
	public <T> T get(ComponentType<? extends T> type) {
		return type == DataComponentTypes.FOX_VARIANT ? castComponentValue((ComponentType<T>)type, this.getVariant()) : super.get(type);
	}
	...
	protected <T> boolean setApplicableComponent(ComponentType<T> type, T value) {
		if (type == DataComponentTypes.FOX_VARIANT) {
			this.setVariant(castComponentValue(DataComponentTypes.FOX_VARIANT, value));
			return true;
		} else {
	...
	Stream<LazyEntityReference<LivingEntity>> getTrustedEntities() {
		return Stream.concat(this.dataTracker.get(OWNER).stream(), this.dataTracker.get(OTHER_TRUSTED).stream());
	}
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.put("Trusted", TRUSTED_ENTITIES_CODEC, this.getTrustedEntities().toList());
		nbt.putBoolean("Sleeping", this.isSleeping());
		nbt.put("Type", FoxEntity.Variant.CODEC, this.getVariant());
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.clearTrusted();
		((List)nbt.get("Trusted", TRUSTED_ENTITIES_CODEC).orElse(List.of())).forEach(this::trust);
		this.setSleeping(nbt.getBoolean("Sleeping", false));
		this.setVariant((FoxEntity.Variant)nbt.get("Type", FoxEntity.Variant.CODEC).orElse(FoxEntity.Variant.DEFAULT));
		this.setSitting(nbt.getBoolean("Sitting", false));
		this.setCrouching(nbt.getBoolean("Crouching", false));
	...
	public float getHeadRoll(float tickProgress) {
		return MathHelper.lerp(tickProgress, this.lastHeadRollProgress, this.headRollProgress) * 0.11F * (float) Math.PI;
	}
	...
	boolean canTrust(LivingEntity entity) {
		return this.getTrustedEntities().anyMatch(trusted -> trusted.uuidEquals(entity));
	}
	...
	public static boolean canJumpChase(FoxEntity fox, LivingEntity chasedEntity) {
		...
		for (int j = 0; j < 6; j++) {
			double g = f == 0.0 ? 0.0 : d * (j / 6.0F);
			double h = f == 0.0 ? e * (j / 6.0F) : g / f;

			for (int k = 1; k < 4; k++) {
				if (!fox.getWorld().getBlockState(BlockPos.ofFloored(fox.getX() + h, fox.getY() + k, fox.getZ() + g)).isReplaceable()) {
					return false;
				}
			}
		}
		...
		public boolean canStart() {
			if (!FoxEntity.this.isSleeping() && this.mob.getTarget() == null) {
				if (FoxEntity.this.getWorld().isThundering() && FoxEntity.this.getWorld().isSkyVisible(this.mob.getBlockPos())) {
					...
					return FoxEntity.this.getWorld().isDay()
						&& FoxEntity.this.getWorld().isSkyVisible(blockPos)
						&& !((ServerWorld)FoxEntity.this.getWorld()).isNearOccupiedPointOfInterest(blockPos)
		...
		public boolean canStart() {
			if (this.reciprocalChance > 0 && this.mob.getRandom().nextInt(this.reciprocalChance) != 0) {
				...
				for (LazyEntityReference<LivingEntity> lazyEntityReference : FoxEntity.this.getTrustedEntities().toList()) {
					LivingEntity livingEntity = (LivingEntity)lazyEntityReference.resolve(serverWorld, LivingEntity.class);
	...
	public class EatBerriesGoal extends MoveToTargetPosGoal {
		...
		private void pickSweetBerries(BlockState state) {
			...
			FoxEntity.this.getWorld().setBlockState(this.targetPos, state.with(SweetBerryBushBlock.AGE, 1), Block.NOTIFY_LISTENERS);
	...
	public class JumpChasingGoal extends DiveJumpingGoal {
		...
		public boolean shouldContinue() {
			...
			if (livingEntity != null && livingEntity.isAlive()) {
				...
				return (!(d * d < 0.05F) || !(Math.abs(FoxEntity.this.getPitch()) < 15.0F) || !FoxEntity.this.isOnGround()) && !FoxEntity.this.isWalking();
			} else {
				return false;
			}
		}
		...
		public void tick() {
			...
			if (livingEntity != null && FoxEntity.this.distanceTo(livingEntity) <= 2.0F) {
				...
				&& FoxEntity.this.getWorld().getBlockState(FoxEntity.this.getBlockPos()).isOf(Blocks.SNOW)) {
	...
	class PickupItemGoal extends Goal {
		...
		public boolean canStart() {
			if (!FoxEntity.this.getEquippedStack(EquipmentSlot.MAINHAND).isEmpty()) {
				...
				List<ItemEntity> list = FoxEntity.this.getWorld()
	...
	public static enum Variant implements StringIdentifiable {
		...
		public static final StringIdentifiable.EnumCodec<FoxEntity.Variant> CODEC = StringIdentifiable.createCodec(FoxEntity.Variant::values);
		private static final IntFunction<FoxEntity.Variant> INDEX_MAPPER = ValueLists.createIndexToValueFunction(
			FoxEntity.Variant::getIndex, values(), ValueLists.OutOfBoundsHandling.ZERO
		);
		public static final PacketCodec<ByteBuf, FoxEntity.Variant> PACKET_CODEC = PacketCodecs.indexed(INDEX_MAPPER, FoxEntity.Variant::getIndex);
		...
		public static FoxEntity.Variant byIndex(int index) {
			return (FoxEntity.Variant)INDEX_MAPPER.apply(index);
		}
	...
	public class WorriableEntityFilter implements TargetPredicate.EntityPredicate {
		...
		public boolean test(LivingEntity livingEntity, ServerWorld serverWorld) {
			...
			} else if (livingEntity instanceof PlayerEntity playerEntity && (playerEntity.isSpectator() || playerEntity.isCreative())) {
				return false;
			} else {
				return FoxEntity.this.canTrust(livingEntity) ? false : !livingEntity.isSleeping() && !livingEntity.isSneaky();
			}
		}
	}
}
