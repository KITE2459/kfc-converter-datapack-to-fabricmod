...
public class FrogEntity extends AnimalEntity {
	...
	protected void initDataTracker(DataTracker.Builder builder) {
		...
		Registry<FrogVariant> registry = this.getRegistryManager().getOrThrow(RegistryKeys.FROG_VARIANT);
	...
	public <T> T get(ComponentType<? extends T> type) {
		return type == DataComponentTypes.FROG_VARIANT ? castComponentValue((ComponentType<T>)type, this.getVariant()) : super.get(type);
	}
	...
	protected <T> boolean setApplicableComponent(ComponentType<T> type, T value) {
		if (type == DataComponentTypes.FROG_VARIANT) {
			this.setVariant(castComponentValue(DataComponentTypes.FROG_VARIANT, value));
			return true;
		} else {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		Variants.readVariantFromNbt(nbt, this.getRegistryManager(), RegistryKeys.FROG_VARIANT).ifPresent(this::setVariant);
	}
	...
	public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
		FrogVariants.select(this.random, this.getRegistryManager(), SpawnContext.of(world, this.getBlockPos())).ifPresent(this::setVariant);
	...
	public static boolean isValidFrogFood(LivingEntity entity) {
		return entity instanceof SlimeEntity slimeEntity && slimeEntity.getSize() != 1 ? false : entity.getType().isIn(EntityTypeTags.FROG_FOOD);
	}
