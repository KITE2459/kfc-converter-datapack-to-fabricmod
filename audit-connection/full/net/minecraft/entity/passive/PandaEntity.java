...
public class PandaEntity extends AnimalEntity {
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.put("MainGene", PandaEntity.Gene.CODEC, this.getMainGene());
		nbt.put("HiddenGene", PandaEntity.Gene.CODEC, this.getHiddenGene());
	}

	@Override
	public void readCustomDataFromNbt(NbtCompound nbt) {
		super.readCustomDataFromNbt(nbt);
		this.setMainGene((PandaEntity.Gene)nbt.get("MainGene", PandaEntity.Gene.CODEC).orElse(PandaEntity.Gene.NORMAL));
		this.setHiddenGene((PandaEntity.Gene)nbt.get("HiddenGene", PandaEntity.Gene.CODEC).orElse(PandaEntity.Gene.NORMAL));
	}
	...
	private void playEatingAnimation() {
		if (this.getEatingTicks() % 5 == 0) {
			...
			for (int i = 0; i < 6; i++) {
				Vec3d vec3d = new Vec3d((this.random.nextFloat() - 0.5) * 0.1, Math.random() * 0.1 + 0.1, (this.random.nextFloat() - 0.5) * 0.1);
				...
				if (this.target == null) {
					...
					if (this.targetType == PlayerEntity.class) {
						...
						this.target = serverWorld.getClosestEntity(
							this.mob.getWorld().getEntitiesByClass(this.targetType, this.mob.getBoundingBox().expand(this.range, 3.0, this.range), livingEntity -> true),
