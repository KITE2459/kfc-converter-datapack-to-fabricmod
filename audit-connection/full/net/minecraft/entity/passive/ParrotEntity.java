...
public class ParrotEntity extends TameableShoulderEntity implements Flutterer {
	...
	public <T> T get(ComponentType<? extends T> type) {
		return type == DataComponentTypes.PARROT_VARIANT ? castComponentValue((ComponentType<T>)type, this.getVariant()) : super.get(type);
	}
	...
	protected <T> boolean setApplicableComponent(ComponentType<T> type, T value) {
		if (type == DataComponentTypes.PARROT_VARIANT) {
			this.setVariant(castComponentValue(DataComponentTypes.PARROT_VARIANT, value));
			return true;
		} else {
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.put("Variant", ParrotEntity.Variant.INDEX_CODEC, this.getVariant());
	}

	@Override
	public void readCustomDataFromNbt(NbtCompound nbt) {
		super.readCustomDataFromNbt(nbt);
		this.setVariant((ParrotEntity.Variant)nbt.get("Variant", ParrotEntity.Variant.INDEX_CODEC).orElse(ParrotEntity.Variant.DEFAULT));
	}
	...
	public static enum Variant implements StringIdentifiable {
		...
		public static final ParrotEntity.Variant DEFAULT = RED_BLUE;
		private static final IntFunction<ParrotEntity.Variant> INDEX_MAPPER = ValueLists.createIndexToValueFunction(
			ParrotEntity.Variant::getIndex, values(), ValueLists.OutOfBoundsHandling.CLAMP
		);
		public static final Codec<ParrotEntity.Variant> CODEC = StringIdentifiable.createCodec(ParrotEntity.Variant::values);
		@Deprecated
		public static final Codec<ParrotEntity.Variant> INDEX_CODEC = Codec.INT.xmap(INDEX_MAPPER::apply, ParrotEntity.Variant::getIndex);
		public static final PacketCodec<ByteBuf, ParrotEntity.Variant> PACKET_CODEC = PacketCodecs.indexed(INDEX_MAPPER, ParrotEntity.Variant::getIndex);
		final int index;
		...
		public static ParrotEntity.Variant byIndex(int index) {
			return (ParrotEntity.Variant)INDEX_MAPPER.apply(index);
		}
