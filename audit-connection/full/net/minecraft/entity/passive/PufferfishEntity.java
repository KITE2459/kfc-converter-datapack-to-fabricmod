...
public class PufferfishEntity extends FishEntity {
	...
	int inflateTicks;
	int deflateTicks;
	private static final TargetPredicate.EntityPredicate BLOW_UP_FILTER = (entity, world) -> entity instanceof PlayerEntity playerEntity
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.setPuffState(Math.min(nbt.getInt("PuffState", 0), 2));
	}
	...
	public void onPlayerCollision(PlayerEntity player) {
		...
		if (player instanceof ServerPlayerEntity serverPlayerEntity
			&& i > 0
			&& player.damage(serverPlayerEntity.getServerWorld(), this.getDamageSources().mobAttack(this), 1 + i)) {
			if (!this.isSilent()) {
