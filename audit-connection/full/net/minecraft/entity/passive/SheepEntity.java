...
public class SheepEntity extends AnimalEntity implements Shearable {
	...
	private static final Map<DyeColor, Integer> COLORS = Util.mapEnum(DyeColor.class, SheepEntity::getDyedColor);
	...
	private static int getDyedColor(DyeColor color) {
		if (color == DyeColor.WHITE) {
			...
			return ColorHelper.getArgb(
				255, MathHelper.floor(ColorHelper.getRed(i) * 0.75F), MathHelper.floor(ColorHelper.getGreen(i) * 0.75F), MathHelper.floor(ColorHelper.getBlue(i) * 0.75F)
			);
		}
	}
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.setColor((DyeColor)nbt.get("Color", DyeColor.INDEX_CODEC).orElse(DEFAULT_COLOR));
	}
