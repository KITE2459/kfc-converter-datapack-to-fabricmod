...
public class StriderEntity extends AnimalEntity implements ItemSteerable {
	...
	protected Vec3d getPassengerAttachmentPos(Entity passenger, EntityDimensions dimensions, float scaleFactor) {
		if (!this.getWorld().isClient()) {
			...
			return super.getPassengerAttachmentPos(passenger, dimensions, scaleFactor).add(0.0, h * scaleFactor, 0.0);
		}
	}
	...
	public Vec3d updatePassengerForDismount(LivingEntity passenger) {
		Vec3d[] vec3ds = new Vec3d[]{
			getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), passenger.getYaw()),
			getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), passenger.getYaw() - 22.5F),
			getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), passenger.getYaw() + 22.5F),
			getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), passenger.getYaw() - 45.0F),
			getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), passenger.getYaw() + 45.0F)
		};
	...
	protected float getSaddledSpeed(PlayerEntity controllingPlayer) {
		return (float)(this.getAttributeValue(EntityAttributes.MOVEMENT_SPEED) * (this.isCold() ? 0.35F : 0.55F) * this.saddledComponent.getMovementSpeedMultiplier());
	}
	...
	public void tick() {
		...
		if (!this.isAiDisabled()) {
			...
			boolean bl2 = this.getVehicle() instanceof StriderEntity striderEntity && striderEntity.isCold();
			this.setCold(!bl || bl2);
		}
