...
public class VillagerEntity extends MerchantEntity implements InteractionObserver, VillagerDataContainer {
	...
	public static final Map<MemoryModuleType<GlobalPos>, BiPredicate<VillagerEntity, RegistryEntry<PointOfInterestType>>> POINTS_OF_INTEREST = ImmutableMap.of(
		...
		MemoryModuleType.JOB_SITE,
		(villager, registryEntry) -> villager.getVillagerData().profession().value().heldWorkstation().test(registryEntry),
		MemoryModuleType.POTENTIAL_JOB_SITE,
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.put("Gossips", VillagerGossips.CODEC, this.gossip);
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.foodLevel = nbt.getByte("FoodLevel", (byte)0);
		this.gossip.clear();
		nbt.get("Gossips", VillagerGossips.CODEC).ifPresent(this.gossip::add);
		this.experience = nbt.getInt("Xp", 0);
		this.lastRestockTime = nbt.getLong("LastRestock", 0L);
		this.lastGossipDecayTime = nbt.getLong("LastGossipDecay", 0L);
		if (this.getWorld() instanceof ServerWorld) {
			this.reinitializeBrain((ServerWorld)this.getWorld());
		}

		this.restocksToday = nbt.getInt("RestocksToday", 0);
		this.natural = nbt.getBoolean("AssignProfessionWhenSpawned", false);
	}
