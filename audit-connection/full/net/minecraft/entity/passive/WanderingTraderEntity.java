...
public class WanderingTraderEntity extends MerchantEntity implements ConsumableComponent.ConsumableSoundProvider {
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.putNullable("wander_target", BlockPos.CODEC, this.wanderTarget);
	}

	@Override
	public void readCustomDataFromNbt(NbtCompound nbt) {
		super.readCustomDataFromNbt(nbt);
		this.despawnDelay = nbt.getInt("DespawnDelay", 0);
		this.wanderTarget = (BlockPos)nbt.get("wander_target", BlockPos.CODEC).orElse(null);
		...
		public void tick() {
			...
			if (blockPos != null && WanderingTraderEntity.this.navigation.isIdle()) {
				if (this.isTooFarFrom(blockPos, 10.0)) {
					Vec3d vec3d = new Vec3d(blockPos.getX() - this.trader.getX(), blockPos.getY() - this.trader.getY(), blockPos.getZ() - this.trader.getZ()).normalize();
