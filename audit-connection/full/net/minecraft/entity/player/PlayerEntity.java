...
public abstract class PlayerEntity extends LivingEntity {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.inventory.setSelectedSlot(nbt.getInt("SelectedItemSlot", 0));
		this.sleepTimer = nbt.getShort("SleepTimer", (short)0);
		this.experienceProgress = nbt.getFloat("XpP", 0.0F);
		this.experienceLevel = nbt.getInt("XpLevel", 0);
		this.totalExperience = nbt.getInt("XpTotal", 0);
		this.enchantingTableSeed = nbt.getInt("XpSeed", 0);
		if (this.enchantingTableSeed == 0) {
			this.enchantingTableSeed = this.random.nextInt();
		}

		this.setScore(nbt.getInt("Score", 0));
		...
		nbt.getList("EnderItems").ifPresent(nbtListx -> this.enderChestInventory.readNbtList(nbtListx, this.getRegistryManager()));
		this.setShoulderEntityLeft(nbt.getCompoundOrEmpty("ShoulderEntityLeft"));
		this.setShoulderEntityRight(nbt.getCompoundOrEmpty("ShoulderEntityRight"));
		this.setLastDeathPos(nbt.get("LastDeathLocation", GlobalPos.CODEC));
		this.currentExplosionImpactPos = (Vec3d)nbt.get("current_explosion_impact_pos", Vec3d.CODEC).orElse(null);
		this.ignoreFallDamageFromCurrentExplosion = nbt.getBoolean("ignore_fall_damage_from_current_explosion", false);
		this.currentExplosionResetGraceTime = nbt.getInt("current_impulse_context_reset_grace_time", 0);
	}
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.putInt("SelectedItemSlot", this.inventory.getSelectedSlot());
		...
		this.lastDeathPos.ifPresent(pos -> nbt.put("LastDeathLocation", GlobalPos.CODEC, pos));
		nbt.putNullable("current_explosion_impact_pos", Vec3d.CODEC, this.currentExplosionImpactPos);
	...
	private boolean isSpaceAroundPlayerEmpty(double offsetX, double offsetZ, double d) {
		...
		return this.getWorld()
			.isSpaceEmpty(
				this,
				new Box(box.minX + 1.0E-7 + offsetX, box.minY - d - 1.0E-7, box.minZ + 1.0E-7 + offsetZ, box.maxX - 1.0E-7 + offsetX, box.minY, box.maxZ - 1.0E-7 + offsetZ)
			);
	}
						...
						if (bl5) {
							...
							if (k > 0.0F) {
								if (target instanceof LivingEntity livingEntity2) {
									...
									target.addVelocity(
										-MathHelper.sin(this.getYaw() * (float) (Math.PI / 180.0)) * k * 0.5F, 0.1, MathHelper.cos(this.getYaw() * (float) (Math.PI / 180.0)) * k * 0.5F
									);
								}
							...
							if (bl4) {
								...
								for (LivingEntity livingEntity3 : this.getWorld().getNonSpectatingEntities(LivingEntity.class, target.getBoundingBox().expand(1.0, 0.25, 1.0))) {
									if (livingEntity3 != this
										...
										&& !(livingEntity3 instanceof ArmorStandEntity armorStandEntity && armorStandEntity.isMarker())
	...
	/**
	 * {@return whether the player is considered "loaded", either because the client
	 * acknowledged loading or because the 60-tick timer elapsed}
	 * 
	 * <p>If {@code false}, the player is invulnerable and cannot perform most actions.
	 */
