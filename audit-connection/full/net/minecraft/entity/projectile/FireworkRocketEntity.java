...
public class FireworkRocketEntity extends ProjectileEntity implements FlyingItemEntity {
	...
	private void explode(ServerWorld world) {
		...
		if (f > 0.0F) {
			if (this.shooter != null) {
				this.shooter.damage(world, this.getDamageSources().fireworks(this, this.getOwner()), 5.0F + list.size() * 2);
			}
			...
			for (LivingEntity livingEntity : this.getWorld().getNonSpectatingEntities(LivingEntity.class, this.getBoundingBox().expand(5.0))) {
				if (livingEntity != this.shooter && !(this.squaredDistanceTo(livingEntity) > 25.0)) {
					...
					for (int i = 0; i < 2; i++) {
						Vec3d vec3d2 = new Vec3d(livingEntity.getX(), livingEntity.getBodyY(0.5 * i), livingEntity.getZ());
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.put("FireworksItem", ItemStack.CODEC, registryOps, this.getStack());
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.life = nbt.getInt("Life", 0);
		this.lifeTime = nbt.getInt("LifeTime", 0);
		RegistryOps<NbtElement> registryOps = this.getRegistryManager().getOps(NbtOps.INSTANCE);
		this.dataTracker.set(ITEM, (ItemStack)nbt.get("FireworksItem", ItemStack.CODEC, registryOps).orElse(getDefaultStack()));
		this.dataTracker.set(SHOT_AT_ANGLE, nbt.getBoolean("ShotAtAngle", false));
	}
