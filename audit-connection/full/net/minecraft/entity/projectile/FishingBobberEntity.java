		...
		if (playerEntity == null) {
			...
			if (this.state == FishingBobberEntity.State.FLYING) {
				...
				if (this.state == FishingBobberEntity.State.BOBBING) {
					...
					this.setVelocity(vec3d.x * 0.9, vec3d.y - d * this.random.nextFloat() * 0.2, vec3d.z * 0.9);
	...
	private void tickFishingLogic(BlockPos pos) {
		...
		if (this.hookCountdown > 0) {
			...
			if (this.fishTravelCountdown > 0) {
				...
				double e = MathHelper.floor(this.getY()) + 1.0F;
			...
			if (this.random.nextFloat() < f) {
				...
				double d = this.getX() + MathHelper.sin(g) * h * 0.1;
				double e = MathHelper.floor(this.getY()) + 1.0F;
				double j = this.getZ() + MathHelper.cos(g) * h * 0.1;
				BlockState blockState = serverWorld.getBlockState(BlockPos.ofFloored(d, e - 1.0, j));
				if (blockState.isOf(Blocks.WATER)) {
