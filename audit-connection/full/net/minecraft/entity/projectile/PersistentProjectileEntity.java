...
public abstract class PersistentProjectileEntity extends ProjectileEntity {
	...
	protected void knockback(LivingEntity target, DamageSource source) {
		double d = this.weapon != null && this.getWorld() instanceof ServerWorld serverWorld
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.putNullable("inBlockState", BlockState.CODEC, registryOps, this.inBlockState);
		nbt.putByte("shake", (byte)this.shake);
		nbt.putBoolean("inGround", this.isInGround());
		nbt.put("pickup", PersistentProjectileEntity.PickupPermission.CODEC, this.pickupType);
		nbt.putDouble("damage", this.damage);
		nbt.putBoolean("crit", this.isCritical());
		nbt.putByte("PierceLevel", this.getPierceLevel());
		nbt.put("SoundEvent", Registries.SOUND_EVENT.getCodec(), this.sound);
		nbt.put("item", ItemStack.CODEC, registryOps, this.stack);
		nbt.putNullable("weapon", ItemStack.CODEC, registryOps, this.weapon);
	}
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		RegistryOps<NbtElement> registryOps = this.getRegistryManager().getOps(NbtOps.INSTANCE);
		this.life = nbt.getShort("life", (short)0);
		this.inBlockState = (BlockState)nbt.get("inBlockState", BlockState.CODEC, registryOps).orElse(null);
		this.shake = nbt.getByte("shake", (byte)0) & 255;
		this.setInGround(nbt.getBoolean("inGround", false));
		this.damage = nbt.getDouble("damage", 2.0);
		this.pickupType = (PersistentProjectileEntity.PickupPermission)nbt.get("pickup", PersistentProjectileEntity.PickupPermission.CODEC)
			.orElse(PersistentProjectileEntity.PickupPermission.DISALLOWED);
		this.setCritical(nbt.getBoolean("crit", false));
		this.setPierceLevel(nbt.getByte("PierceLevel", (byte)0));
		this.sound = (SoundEvent)nbt.get("SoundEvent", Registries.SOUND_EVENT.getCodec()).orElse(this.getHitSound());
		this.setStack((ItemStack)nbt.get("item", ItemStack.CODEC, registryOps).orElse(this.getDefaultItemStack()));
		this.weapon = (ItemStack)nbt.get("weapon", ItemStack.CODEC, registryOps).orElse(null);
	}
	...
	public void applyDamageModifier(float damageModifier) {
		this.setDamage(damageModifier * 2.0F + this.random.nextTriangular(this.getWorld().getDifficulty().getId() * 0.11, 0.57425));
	}
