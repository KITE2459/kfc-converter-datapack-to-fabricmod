...
public class ShulkerBulletEntity extends ProjectileEntity {
	...
	protected void writeCustomDataToNbt(NbtCompound nbt) {
		...
		if (this.target != null) {
			nbt.put("Target", Uuids.INT_STREAM_CODEC, this.target.getUuid());
		}

		nbt.putNullable("Dir", Direction.INDEX_CODEC, this.direction);
	...
	protected void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.stepCount = nbt.getInt("Steps", 0);
		this.targetX = nbt.getDouble("TXD", 0.0);
		this.targetY = nbt.getDouble("TYD", 0.0);
		this.targetZ = nbt.getDouble("TZD", 0.0);
		this.direction = (Direction)nbt.get("Dir", Direction.INDEX_CODEC).orElse(null);
		this.targetUuid = (UUID)nbt.get("Target", Uuids.INT_STREAM_CODEC).orElse(null);
	}
