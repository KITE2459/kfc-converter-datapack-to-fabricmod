...
public abstract class RaiderEntity extends PatrolEntity {
	...
	public void writeCustomDataToNbt(NbtCompound nbt) {
		...
		if (this.raid != null && this.getWorld() instanceof ServerWorld serverWorld) {
			serverWorld.getRaidManager().getRaidId(this.raid).ifPresent(raidId -> nbt.putInt("RaidId", raidId));
		}
	}
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.ableToJoinRaid = nbt.getBoolean("CanJoinRaid", false);
		if (this.getWorld() instanceof ServerWorld serverWorld) {
			nbt.getInt("RaidId").ifPresent(raidId -> {
	...
	static class AttackHomeGoal extends Goal {
		...
		private boolean tryFindHome() {
			...
			Optional<BlockPos> optional = serverWorld.getPointOfInterestStorage()
				.getPosition(
					poi -> poi.matchesKey(PointOfInterestTypes.HOME), this::canLootHome, PointOfInterestStorage.OccupationStatus.ANY, blockPos, 48, this.raider.random
				);
			if (optional.isEmpty()) {
				return false;
			} else {
		...
		public boolean shouldContinue() {
			return this.raider.getNavigation().isIdle()
				? false
				: this.raider.getTarget() == null && !this.home.isWithinDistance(this.raider.getPos(), this.raider.getWidth() + this.distance) && !this.finished;
		}
