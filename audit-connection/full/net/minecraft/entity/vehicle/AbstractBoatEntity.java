...
public abstract class AbstractBoatEntity extends VehicleEntity implements Leashable {
	...
	public void onBubbleColumnSurfaceCollision(boolean drag, BlockPos pos) {
		...
		if (!this.isSubmergedInWater() && this.random.nextInt(100) == 0) {
			this.getWorld()
				.playSoundClient(this.getX(), this.getY(), this.getZ(), this.getSplashSound(), this.getSoundCategory(), 1.0F, 0.8F + 0.4F * this.random.nextFloat(), false);
			this.getWorld()
				.addParticleClient(ParticleTypes.SPLASH, this.getX() + this.random.nextFloat(), this.getY() + 0.7, this.getZ() + this.random.nextFloat(), 0.0, 0.0, 0.0);
	...
	private void handleBubbleColumn() {
		if (this.getWorld().isClient) {
			...
			this.bubbleWobble = 10.0F * (float)Math.sin(0.5 * this.age) * this.bubbleWobbleStrength;
		} else {
