...
public class ExperimentalMinecartController extends MinecartController {
	...
	public Vec3d getLerpedPosition(float tickProgress) {
		ExperimentalMinecartController.InterpolatedStep interpolatedStep = this.getLerpedStep(tickProgress);
		return MathHelper.lerp(interpolatedStep.partialTicksInStep, interpolatedStep.previousStep.position, interpolatedStep.currentStep.position);
	}

	public Vec3d getLerpedVelocity(float tickProgress) {
		ExperimentalMinecartController.InterpolatedStep interpolatedStep = this.getLerpedStep(tickProgress);
		return MathHelper.lerp(interpolatedStep.partialTicksInStep, interpolatedStep.previousStep.movement, interpolatedStep.currentStep.movement);
	}
	...
	record InterpolatedStep(float partialTicksInStep, ExperimentalMinecartController.Step currentStep, ExperimentalMinecartController.Step previousStep) {
	}
