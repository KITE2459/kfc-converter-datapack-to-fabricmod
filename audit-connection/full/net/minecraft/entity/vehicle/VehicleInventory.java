...
public interface VehicleInventory extends Inventory, NamedScreenHandlerFactory {
	...
	default void readInventoryFromNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		this.resetInventory();
		RegistryKey<LootTable> registryKey = (RegistryKey<LootTable>)nbt.get("LootTable", LootTable.TABLE_KEY).orElse(null);
		this.setLootTable(registryKey);
		this.setLootTableSeed(nbt.getLong("LootTableSeed", 0L));
		if (registryKey == null) {
