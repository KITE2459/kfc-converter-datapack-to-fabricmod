	...
	public static boolean writeNbtToBlockEntity(World world, @Nullable PlayerEntity player, BlockPos pos, ItemStack stack) {
		if (world.isClient) {
			...
			if (!nbtComponent.isEmpty()) {
				BlockEntityType<?> blockEntityType = nbtComponent.getRegistryValueOfId(world.getRegistryManager(), RegistryKeys.BLOCK_ENTITY_TYPE);
	...
	public boolean shouldShowOperatorBlockWarnings(ItemStack stack, @Nullable PlayerEntity player) {
		if (player != null && player.getPermissionLevel() >= 2) {
			...
			if (nbtComponent != null) {
				BlockEntityType<?> blockEntityType = nbtComponent.getRegistryValueOfId(player.getWorld().getRegistryManager(), RegistryKeys.BLOCK_ENTITY_TYPE);
