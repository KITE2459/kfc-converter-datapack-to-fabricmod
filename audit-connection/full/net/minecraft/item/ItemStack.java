...
public final class ItemStack implements ComponentHolder, FabricItemStack {
	...
	public void appendTooltip(
		...
		if ((this.isOf(Items.SPAWNER) || this.isOf(Items.TRIAL_SPAWNER)) && displayComponent.shouldDisplay(DataComponentTypes.BLOCK_ENTITY_DATA)) {
			NbtComponent nbtComponent = this.getOrDefault(DataComponentTypes.BLOCK_ENTITY_DATA, NbtComponent.DEFAULT);
		...
		if (blockPredicatesChecker != null && displayComponent.shouldDisplay(DataComponentTypes.CAN_BREAK)) {
			textConsumer.accept(ScreenTexts.EMPTY);
			textConsumer.accept(BlockPredicatesChecker.CAN_BREAK_TEXT);
			blockPredicatesChecker.addTooltips(textConsumer);
		}

		BlockPredicatesChecker blockPredicatesChecker2 = this.get(DataComponentTypes.CAN_PLACE_ON);
		if (blockPredicatesChecker2 != null && displayComponent.shouldDisplay(DataComponentTypes.CAN_PLACE_ON)) {
			textConsumer.accept(ScreenTexts.EMPTY);
			textConsumer.accept(BlockPredicatesChecker.CAN_PLACE_TEXT);
			blockPredicatesChecker2.addTooltips(textConsumer);
		}
