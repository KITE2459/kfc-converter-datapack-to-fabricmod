...
public class RecipeMatcher<T> {
	...
	public boolean match(List<? extends RecipeMatcher.RawIngredient<T>> ingredients, int quantity, @Nullable RecipeMatcher.ItemCallback<T> itemCallback) {
		return new RecipeMatcher.Matcher(ingredients).match(quantity, itemCallback);
	}

	public int countCrafts(List<? extends RecipeMatcher.RawIngredient<T>> ingredients, int max, @Nullable RecipeMatcher.ItemCallback<T> itemCallback) {
	...
	List<T> createItemRequirementList(Iterable<? extends RecipeMatcher.RawIngredient<T>> ingredients) {
		...
		for (Entry<T> entry : Reference2IntMaps.fastIterable(this.available)) {
		...
		private void initItemMatch() {
			for (int i = 0; i < this.totalIngredients; i++) {
				...
				for (int j = 0; j < this.totalRequiredItems; j++) {
					if (rawIngredient.acceptsItem((T)this.requiredItems.get(j))) {
						this.setMatch(j, i);
					}
				}
			}
