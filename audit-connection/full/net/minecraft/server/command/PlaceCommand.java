...
public class PlaceCommand {
							...
							CommandManager.argument("pool", RegistryKeyArgumentType.registryKey(RegistryKeys.TEMPLATE_POOL))
								.then(
									CommandManager.argument("target", IdentifierArgumentType.identifier())
										.then(
											CommandManager.argument("max_depth", IntegerArgumentType.integer(1, 20))
												.executes(
	...
	public static int executePlaceTemplate(
		ServerCommandSource source, Identifier id, BlockPos pos, BlockRotation rotation, BlockMirror mirror, float integrity, int seed, boolean strict
		...
		if (optional.isEmpty()) {
			...
			boolean bl = structureTemplate.place(
				serverWorld,
				pos,
				pos,
				structurePlacementData,
				StructureBlockBlockEntity.createRandom(seed),
				Block.NOTIFY_LISTENERS | (strict ? Block.FORCE_STATE_AND_SKIP_CALLBACKS_AND_DROPS : 0)
			);
			if (!bl) {
