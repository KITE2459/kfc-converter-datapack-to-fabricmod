...
public class EntityTrackerEntry {
	...
	private final Consumer<Packet<?>> watchingSender;
	private final BiConsumer<Packet<?>, List<UUID>> filteredWatchingSender;
	...
	public void tick() {
		...
		if (!list.equals(this.lastPassengers)) {
			List<UUID> list2 = this.streamChangedPassengers(list).map(Entity::getUuid).toList();
		...
		if (this.trackingTick % this.tickInterval == 0 || this.entity.velocityDirty || this.entity.getDataTracker().isDirty()) {
			...
			} else if (this.entity instanceof AbstractMinecartEntity abstractMinecartEntity
	...
	private Stream<Entity> streamChangedPassengers(List<Entity> newPassengers) {
		return Streams.concat(
			this.lastPassengers.stream().filter(passenger -> !newPassengers.contains(passenger)),
			newPassengers.stream().filter(passenger -> !this.lastPassengers.contains(passenger))
		);
	}
