...
public class ServerPlayerInteractionManager {
	...
	private float continueMining(BlockState state, BlockPos pos, int failedStartMiningTime) {
		...
		float f = state.calcBlockBreakingDelta(this.player, this.player.getWorld(), pos) * (i + 1);
		int j = (int)(f * 10.0F);
			...
			if (action == PlayerActionC2SPacket.Action.START_DESTROY_BLOCK) {
				...
				if (pos.equals(this.miningPos)) {
					...
					if (!blockStatex.isAir()) {
						float g = blockStatex.calcBlockBreakingDelta(this.player, this.player.getWorld(), pos) * (j + 1);
						if (g >= 0.7F) {
							this.mining = false;
