...
public class ServerRecipeBook extends RecipeBook {
	...
	private static final Codec<List<RegistryKey<Recipe<?>>>> RECIPES_CODEC = Recipe.KEY_CODEC.listOf();
	...
	public void readNbt(NbtCompound nbt, Predicate<RegistryKey<Recipe<?>>> validPredicate) {
		...
		List<RegistryKey<Recipe<?>>> list = (List<RegistryKey<Recipe<?>>>)nbt.get("recipes", RECIPES_CODEC).orElse(List.of());
		this.handleList(list, this::unlock, validPredicate);
		List<RegistryKey<Recipe<?>>> list2 = (List<RegistryKey<Recipe<?>>>)nbt.get("toBeDisplayed", RECIPES_CODEC).orElse(List.of());
		this.handleList(list2, this::markHighlighted, validPredicate);
	}
