...
public class ServerChunkLoadingManager extends VersionedChunkStorage implements ChunkHolder.PlayersWatchingChunkProvider, ChunkLoadingManager {
	...
	public ServerChunkLoadingManager(
		...
		this.lightingProvider = new ServerLightingProvider(
			chunkProvider, this, this.world.getDimension().hasSkyLight(), simpleConsecutiveExecutor2, this.lightScheduler
		);
		...
		completableFuture.thenRunAsync(() -> {
			...
			if (completableFuture2 != completableFuture) {
				...
				if (this.chunksToUnload.remove(pos, chunk) && chunkx != null) {
					...
					this.worldGenerationProgressListener.setChunkStatus(chunkx.getPos(), null);
	...
	void collectSpawningChunks(List<WorldChunk> chunks) {
		...
		while (longIterator.hasNext()) {
			ChunkHolder chunkHolder = this.chunkHolders.get(longIterator.nextLong());
			if (chunkHolder != null) {
	...
	void forEachBlockTickingChunk(Consumer<WorldChunk> chunkConsumer) {
		this.levelManager.forEachBlockTickingChunk(chunkPos -> {
			ChunkHolder chunkHolder = this.chunkHolders.get(chunkPos);
			if (chunkHolder != null) {
				WorldChunk worldChunk = chunkHolder.getWorldChunk();
				if (worldChunk != null) {
	...
	class EntityTracker {
		...
		public EntityTracker(final Entity entity, final int maxDistance, final int tickInterval, final boolean alwaysUpdateVelocity) {
			this.entry = new EntityTrackerEntry(
				ServerChunkLoadingManager.this.world, entity, tickInterval, alwaysUpdateVelocity, this::sendToOtherNearbyPlayers, this::sendToOtherNearbyPlayers
			);
			this.entity = entity;
		...
		public void sendToOtherNearbyPlayers(Packet<?> packet, List<UUID> except) {
			for (PlayerAssociatedNetworkHandler playerAssociatedNetworkHandler : this.listeners) {
