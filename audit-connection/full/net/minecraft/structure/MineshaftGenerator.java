...
public class MineshaftGenerator {
		...
		protected boolean addChest(StructureWorldAccess world, BlockBox boundingBox, Random random, int x, int y, int z, RegistryKey<LootTable> lootTable) {
			...
			if (boundingBox.contains(blockPos) && world.getBlockState(blockPos).isAir() && !world.getBlockState(blockPos.down()).isAir()) {
				...
				if (chestMinecartEntity != null) {
					chestMinecartEntity.initPosition(blockPos.getX() + 0.5, blockPos.getY() + 0.5, blockPos.getZ() + 0.5);
	...
	public static class MineshaftCrossing extends MineshaftGenerator.MineshaftPart {
		...
		public MineshaftCrossing(NbtCompound nbt) {
			...
			this.direction = (Direction)nbt.get("D", Direction.HORIZONTAL_QUARTER_TURNS_CODEC).orElse(Direction.SOUTH);
		}
	...
	public static class MineshaftRoom extends MineshaftGenerator.MineshaftPart {
		...
		public MineshaftRoom(NbtCompound nbt) {
			...
			this.entrances.addAll((Collection)nbt.get("Entrances", BlockBox.CODEC.listOf()).orElse(List.of()));
		}
