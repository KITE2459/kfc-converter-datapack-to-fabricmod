...
public class StructureTemplate {
				...
				for (StructureTemplate.StructureBlockInfo structureBlockInfo : process(world, pos, pivot, placementData, list)) {
					...
					if (blockBox == null || blockBox.contains(blockPos)) {
						...
						if (structureBlockInfo.nbt != null) {
							world.setBlockState(
								blockPos, Blocks.BARRIER.getDefaultState(), Block.FORCE_STATE_AND_SKIP_CALLBACKS_AND_DROPS | Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK
							);
						}
						...
						if (world.setBlockState(blockPos, blockState, flags)) {
							...
							if (structureBlockInfo.nbt != null) {
								...
								if (blockEntity != null) {
									...
									blockEntity.read(structureBlockInfo.nbt, world.getRegistryManager());
								}
							}
	...
	public void readNbt(RegistryEntryLookup<Block> blockLookup, NbtCompound nbt) {
		...
		Optional<NbtList> optional = nbt.getList("palettes");
		...
		nbt.getListOrEmpty("entities").streamCompounds().forEach(nbtCompound -> {
			NbtList nbtListx = nbtCompound.getListOrEmpty("pos");
			Vec3d vec3d = new Vec3d(nbtListx.getDouble(0, 0.0), nbtListx.getDouble(1, 0.0), nbtListx.getDouble(2, 0.0));
			NbtList nbtList2x = nbtCompound.getListOrEmpty("blockPos");
			BlockPos blockPos = new BlockPos(nbtList2x.getInt(0, 0), nbtList2x.getInt(1, 0), nbtList2x.getInt(2, 0));
			nbtCompound.getCompound("nbt").ifPresent(nbtCompoundx -> this.entities.add(new StructureTemplate.StructureEntityInfo(vec3d, blockPos, nbtCompoundx)));
		});
	}
	...
	private void loadPalettedBlockInfo(RegistryEntryLookup<Block> blockLookup, NbtList palette, NbtList blocks) {
		...
		List<StructureTemplate.StructureBlockInfo> list2 = Lists.<StructureTemplate.StructureBlockInfo>newArrayList();
		List<StructureTemplate.StructureBlockInfo> list3 = Lists.<StructureTemplate.StructureBlockInfo>newArrayList();
		blocks.streamCompounds().forEach(nbtCompound -> {
			...
			BlockState blockState = palette2.getState(nbtCompound.getInt("state", 0));
			NbtCompound nbtCompound2 = (NbtCompound)nbtCompound.getCompound("nbt").orElse(null);
	...
	public static JigsawBlockEntity.Joint readJoint(NbtCompound nbt, BlockState state) {
		return (JigsawBlockEntity.Joint)nbt.get("joint", JigsawBlockEntity.Joint.CODEC).orElseGet(() -> getJointFromFacing(state));
	}
	...
	public record JigsawBlockInfo(
		...
		public static StructureTemplate.JigsawBlockInfo of(StructureTemplate.StructureBlockInfo structureBlockInfo) {
			...
			return new StructureTemplate.JigsawBlockInfo(
				...
				(Identifier)nbtCompound.get("name", Identifier.CODEC).orElse(JigsawBlockEntity.DEFAULT_NAME),
				(RegistryKey<StructurePool>)nbtCompound.get("pool", JigsawBlockEntity.STRUCTURE_POOL_KEY_CODEC).orElse(StructurePools.EMPTY),
				(Identifier)nbtCompound.get("target", Identifier.CODEC).orElse(JigsawBlockEntity.DEFAULT_NAME),
				nbtCompound.getInt("placement_priority", 0),
				nbtCompound.getInt("selection_priority", 0)
			);
		}
