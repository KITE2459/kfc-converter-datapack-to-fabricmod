...
public class StructurePool {
	...
	public StructurePool(
		...
		this.elementWeights = Lists.<Pair<StructurePoolElement, Integer>>newArrayList();
		this.elements = new ObjectArrayList<>();
		...
		for (Pair<Function<StructurePool.Projection, ? extends StructurePoolElement>, Integer> pair : elementWeightsByGetters) {
			...
			this.elementWeights.add(Pair.of(structurePoolElement, pair.getSecond()));
	...
	public static enum Projection implements StringIdentifiable {
		...
		public static final StringIdentifiable.EnumCodec<StructurePool.Projection> CODEC = StringIdentifiable.createCodec(StructurePool.Projection::values);
