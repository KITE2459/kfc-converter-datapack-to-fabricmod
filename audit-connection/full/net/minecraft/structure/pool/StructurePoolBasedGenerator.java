	...
	public static Optional<Structure.StructurePosition> generate(
		...
		if (structurePoolElement == EmptyPoolElement.INSTANCE) {
			...
			int k = projectStartToHeightmap.isEmpty()
				? blockPos2.getY()
				: pos.getY() + chunkGenerator.getHeightOnGround(i, j, (Heightmap.Type)projectStartToHeightmap.get(), heightLimitView, context.noiseConfig());
			...
			if (method_65173(heightLimitView, dimensionPadding, poolStructurePiece.getBoundingBox())) {
				LOGGER.debug(
					"Center piece {} with bounding box {} does not fit dimension padding {}", structurePoolElement, poolStructurePiece.getBoundingBox(), dimensionPadding
				);
				return Optional.empty();
			} else {
				int m = k + vec3i.getY();
	...
	private static void generate(
		...
		while (structurePoolGenerator.structurePieces.hasNext()) {
			...
			structurePoolGenerator.generatePiece(
				...
				shapedPoolStructurePiece.depth,
				modifyBoundingBox,
				heightLimitView,
				noiseConfig,
				aliasLookup,
									...
									if (modifyBoundingBox && blockBox2.getBlockCountY() <= 16) {
										m = list2.stream().mapToInt(jigsawInfo -> {
											...
											if (!blockBox2.contains(structureBlockInfox.pos().offset(JigsawBlock.getFacing(structureBlockInfox.state())))) {
												...
												RegistryKey<StructurePool> registryKeyx = aliasLookup.lookup(jigsawInfo.pool());
											...
											if (!VoxelShapes.matchesAnywhere(mutableObject2.getValue(), VoxelShapes.cuboid(Box.from(blockBox4).contract(0.25)), BooleanBiFunction.ONLY_SECOND)) {
												...
												if (depth + 1 <= this.maxSize) {
													StructurePoolBasedGenerator.ShapedPoolStructurePiece shapedPoolStructurePiece = new StructurePoolBasedGenerator.ShapedPoolStructurePiece(
														poolStructurePiece, mutableObject2, depth + 1
													);
