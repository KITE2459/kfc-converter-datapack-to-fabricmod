...
public class JfrJsonReport {
	...
	private JsonElement collectStructureGenSection(List<StructureGenerationSample> structureGenerationSamples) {
		JsonObject jsonObject = new JsonObject();
		LongRunningSampleStatistics<StructureGenerationSample> longRunningSampleStatistics = LongRunningSampleStatistics.fromSamples(structureGenerationSamples);
		...
		((Map)structureGenerationSamples.stream().collect(Collectors.groupingBy(StructureGenerationSample::structureName)))
			.forEach(
				(name, samples) -> {
					...
					LongRunningSampleStatistics<StructureGenerationSample> longRunningSampleStatistics2 = LongRunningSampleStatistics.fromSamples(samples);
					...
					longRunningSampleStatistics2.quantiles().forEach((integer, double_) -> jsonObject3.addProperty("p" + integer, double_));
