...
public interface BlockView extends HeightLimitView, FabricBlockView {
	...
	static void collectCollisionsBetween(Vec3d from, Vec3d to, Box box, BlockView.CollisionVisitor visitor) {
		...
		if (!(vec3d.lengthSquared() < MathHelper.square(0.99999F))) {
			...
			for (BlockPos blockPos2 : BlockPos.iterate(box)) {
				if (!longSet.contains(blockPos2.asLong())) {
					visitor.visit(blockPos2, i + 1);
				}
			}
		} else {
			for (BlockPos blockPos : BlockPos.iterate(box)) {
				visitor.visit(blockPos, 0);
			}
		}
	}

	private static int collectCollisionsBetween(LongSet visited, Vec3d oldPos, Vec3d newPos, Box boundingBox, BlockView.CollisionVisitor visitor) {
		...
		while (g <= 1.0 || h <= 1.0 || o <= 1.0) {
			...
			if (!optional.isEmpty()) {
				...
				for (int w = i; w <= t; w++) {
					for (int x = j; x <= u; x++) {
						for (int y = k; y <= v; y++) {
							if (visited.add(BlockPos.asLong(w, x, y))) {
								visitor.visit(mutable.set(w, x, y), p);
							}
						}
					}
				}
	...
	public interface CollisionVisitor {
		void visit(BlockPos pos, int version);
	}
}
