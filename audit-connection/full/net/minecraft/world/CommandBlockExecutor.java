...
public abstract class CommandBlockExecutor implements CommandOutput {
	...
	public NbtCompound writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		...
		nbt.putNullable("CustomName", TextCodecs.CODEC, registryOps, this.customName);
		nbt.putBoolean("TrackOutput", this.trackOutput);
		if (this.trackOutput) {
			nbt.putNullable("LastOutput", TextCodecs.CODEC, registryOps, this.lastOutput);
		}
	...
	public void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
		...
		this.setCustomName(BlockEntity.tryParseCustomName(nbt.get("CustomName"), registries));
		this.trackOutput = nbt.getBoolean("TrackOutput", true);
		if (this.trackOutput) {
			this.lastOutput = BlockEntity.tryParseCustomName(nbt.get("LastOutput"), registries);
		} else {
			this.lastOutput = null;
		}

		this.updateLastExecution = nbt.getBoolean("UpdateLastExecution", true);
		if (this.updateLastExecution) {
			this.lastExecution = nbt.getLong("LastExecution", -1L);
		} else {
