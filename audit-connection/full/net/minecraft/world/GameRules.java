...
public class GameRules {
	...
	public static final GameRules.Key<GameRules.BooleanRule> ALLOW_FIRE_TICKS_AWAY_FROM_PLAYER = register(
		"allowFireTicksAwayFromPlayer", GameRules.Category.UPDATES, GameRules.BooleanRule.create(false)
	);
	/**
	...
	public static final GameRules.Key<GameRules.BooleanRule> TNT_EXPLODES = register("tntExplodes", GameRules.Category.MISC, GameRules.BooleanRule.create(true));
	private final Map<GameRules.Key<?>, GameRules.Rule<?>> rules;
	private final FeatureSet enabledFeatures;

	public static <T extends GameRules.Rule<T>> GameRules.Type<T> getRuleType(GameRules.Key<T> key) {
		return (GameRules.Type<T>)RULE_TYPES.get(key);
	}

	public static <T extends GameRules.Rule<T>> Codec<GameRules.Key<T>> createKeyCodec(Class<T> ruleClass) {
		return Codec.STRING
			.comapFlatMap(
				string -> (DataResult)RULE_TYPES.entrySet()
					.stream()
					.filter(ruleType -> ((GameRules.Type)ruleType.getValue()).ruleClass == ruleClass)
					.map(Entry::getKey)
					.filter(key -> key.getName().equals(string))
					.map(key -> key)
					.findFirst()
	...
	public static class BooleanRule extends GameRules.Rule<GameRules.BooleanRule> {
		...
		static GameRules.Type<GameRules.BooleanRule> create(boolean initialValue, BiConsumer<MinecraftServer, GameRules.BooleanRule> changeCallback) {
			return new GameRules.Type<>(
				...
				type -> new GameRules.BooleanRule(type, initialValue),
				changeCallback,
				GameRules.Visitor::visitBoolean,
				GameRules.BooleanRule.class,
				FeatureSet.empty()
			);
		}
