...
public class Heightmap {
	...
	public static enum Type implements StringIdentifiable {
		...
		MOTION_BLOCKING(4, "MOTION_BLOCKING", Heightmap.Purpose.CLIENT, state -> state.blocksMovement() || !state.getFluidState().isEmpty()),
		...
		private static final IntFunction<Heightmap.Type> INDEX_MAPPER = ValueLists.createIndexToValueFunction(
			type -> type.index, values(), ValueLists.OutOfBoundsHandling.ZERO
		);
		public static final PacketCodec<ByteBuf, Heightmap.Type> PACKET_CODEC = PacketCodecs.indexed(INDEX_MAPPER, type -> type.index);
		private final int index;
		private final String id;
		private final Heightmap.Purpose purpose;
		private final Predicate<BlockState> blockPredicate;

		private Type(final int index, final String id, final Heightmap.Purpose purpose, final Predicate<BlockState> blockPredicate) {
			this.index = index;
			this.id = id;
