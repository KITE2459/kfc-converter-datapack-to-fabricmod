...
public abstract class World implements WorldAccess, EntityQueriable<Entity>, AutoCloseable, AttachmentTarget {
	...
	/**
	 * Plays a sound caused by a source. On the client, the sound will only play if the source is the same as the client's player.
	 * On the server, sound packets will be sent to players around the source, excluding the source itself.
	 * 
	 * @param source the player that caused this sound, or {@code null}
	 */
	 ...
	 * On the server, sound packets will be sent to players around the source, excluding the source itself.
	 * 
	 * @param source the entity that caused this sound, or {@code null}
	 */
	...
	/**
	 * Plays a sound caused by a source at the provided entity's position. On the client, the sound will only play if the source is the same as the client's player.
	 * On the server, sound packets will be sent to players around the source, excluding the source itself.
	 * 
	 * @param source the player that caused this sound, or {@code null}
	 */
	...
	public void playSound(
		@Nullable Entity source, double x, double y, double z, RegistryEntry<SoundEvent> sound, SoundCategory category, float volume, float pitch
	) {
		this.playSound(source, x, y, z, sound, category, volume, pitch, this.threadSafeRandom.nextLong());
	}
	...
	public void playSoundAtBlockCenterClient(BlockPos pos, SoundEvent sound, SoundCategory category, float volume, float pitch, boolean useDistance) {
		this.playSoundClient(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, sound, category, volume, pitch, useDistance);
	}
	...
	/**
	 * Adds an important particle to the client's world renderer. Important particles get added even if the Particles graphics option is set to Minimal
	 * Does nothing on the server.
	 * 
	 * @see net.minecraft.server.world.ServerWorld#spawnParticles
	 */
	...
	/**
	 * {@return whether {@code entity} can modify blocks at {@code pos}}
	 * 
	...
	public CrashReportSection addDetailsToCrashReport(CrashReport report) {
		...
		crashReportSection.add("All players", (CrashCallable<String>)(() -> {
			List<? extends PlayerEntity> list = this.getPlayers();
			return list.size() + " total; " + (String)list.stream().map(PlayerEntity::asString).collect(Collectors.joining(", "));
		}));
