...
public class BiomeEffects {
	public static final Codec<BiomeEffects> CODEC = RecordCodecBuilder.create(
		instance -> instance.group(
				...
				Codec.INT.optionalFieldOf("dry_foliage_color").forGetter(effects -> effects.dryFoliageColor),
				...
				Pool.createCodec(MusicSound.CODEC).optionalFieldOf("music").forGetter(effects -> effects.music),
				Codec.FLOAT.fieldOf("music_volume").orElse(1.0F).forGetter(effects -> effects.musicVolume)
			)
	...
	private final Optional<Pool<MusicSound>> music;
	...
	BiomeEffects(
		...
		Optional<Pool<MusicSound>> music,
		float musicVolume
	) {
	...
	public Optional<Integer> getDryFoliageColor() {
	...
	public static class Builder {
		...
		private Optional<Pool<MusicSound>> music = Optional.empty();
		...
		public BiomeEffects.Builder music(@Nullable MusicSound music) {
			if (music == null) {
				...
				this.music = Optional.of(Pool.of(music));
				return this;
			}
		}

		public BiomeEffects.Builder noMusic() {
			return this.music(Pool.empty()).musicVolume(0.0F);
		}
