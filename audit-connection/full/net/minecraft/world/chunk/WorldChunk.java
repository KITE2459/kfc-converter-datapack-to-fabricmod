	...
	public BlockState setBlockState(BlockPos pos, BlockState state, int flags) {
		...
		if (bl && state.isAir()) {
			...
			if (blockState == state) {
				...
				if ((bl3 || block instanceof AbstractRailBlock) && this.world instanceof ServerWorld serverWorld && ((flags & 1) != 0 || bl4)) {
				...
				for (Short short_ : this.postProcessingLists[i]) {
					...
					if (!(blockState.getBlock() instanceof FluidBlock)) {
						...
						if (blockState2 != blockState) {
							world.setBlockState(blockPos, blockState2, Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK | Block.FORCE_STATE);
						}
					}
				}
