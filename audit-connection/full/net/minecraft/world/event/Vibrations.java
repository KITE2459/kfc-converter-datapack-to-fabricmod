...
public interface Vibrations {
	...
	static int getSignalStrength(float distance, int range) {
		double d = 15.0 / range;
		return Math.max(1, 15 - MathHelper.floor(d * distance));
	}
	...
	public static class VibrationListener implements GameEventListener {
		...
		private static boolean isOccluded(World world, Vec3d emitterPos, Vec3d listenerPos) {
			Vec3d vec3d = new Vec3d(MathHelper.floor(emitterPos.x) + 0.5, MathHelper.floor(emitterPos.y) + 0.5, MathHelper.floor(emitterPos.z) + 0.5);
			Vec3d vec3d2 = new Vec3d(MathHelper.floor(listenerPos.x) + 0.5, MathHelper.floor(listenerPos.y) + 0.5, MathHelper.floor(listenerPos.z) + 0.5);
