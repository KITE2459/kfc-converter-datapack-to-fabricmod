...
public final class DensityFunctionTypes {
	...
	protected record Clamp(DensityFunction input, double minValue, double maxValue) implements DensityFunctionTypes.Unary {
	...
	protected static final class EndIslands implements DensityFunction.Base {
		...
		@Override
		public double sample(DensityFunction.NoisePos pos) {
			return (sample(this.sampler, pos.blockX() / 8, pos.blockZ() / 8) - 8.0) / 128.0;
		}
	...
	protected record ShiftedNoise(
		...
		@Override
		public double sample(DensityFunction.NoisePos pos) {
			double d = pos.blockX() * this.xzScale + this.shiftX.sample(pos);
			double e = pos.blockY() * this.yScale + this.shiftY.sample(pos);
			double f = pos.blockZ() * this.xzScale + this.shiftZ.sample(pos);
	...
	protected record UnaryOperation(DensityFunctionTypes.UnaryOperation.Type type, DensityFunction input, double minValue, double maxValue)
	...
	protected record Wrapping(DensityFunctionTypes.Wrapping.Type type, DensityFunction wrapped) implements DensityFunctionTypes.Wrapper {
		@Override
		public double sample(DensityFunction.NoisePos pos) {
