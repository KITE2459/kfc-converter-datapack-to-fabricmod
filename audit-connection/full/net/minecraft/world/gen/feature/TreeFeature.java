...
public class TreeFeature extends Feature<TreeFeatureConfig> {
	...
	public final boolean generate(FeatureContext<TreeFeatureConfig> context) {
		...
		if (bl && (!set2.isEmpty() || !set3.isEmpty())) {
			...
			return (Boolean)BlockBox.encompassPositions(Iterables.concat(set, set2, set3, set4)).map(box -> {
				...
				StructureTemplate.updateCorner(structureWorldAccess, Block.NOTIFY_ALL, voxelSet, box.getMinX(), box.getMinY(), box.getMinZ());
				return true;
			}).orElse(false);
		} else {
			return false;
	...
	public static List<BlockPos> getLeafLitterPositions(TreeDecorator.Generator decoratorGenerator) {
		...
		List<BlockPos> list2 = decoratorGenerator.getRootPositions();
		List<BlockPos> list3 = decoratorGenerator.getLogPositions();
		if (list2.isEmpty()) {
			list.addAll(list3);
