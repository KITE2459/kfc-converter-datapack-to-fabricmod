		...
		FEATURES.asMap()
			.forEach(
				(world, features) -> {
					...
					Registry<PlacedFeature> registry = world.getRegistryManager().getOrThrow(RegistryKeys.PLACED_FEATURE);
						...
						.forEach(
							(featureData, count) -> LOGGER.debug(
								string2
									...
									+ String.format(Locale.ROOT, "%10f ", (double)count.intValue() / integer.intValue())
