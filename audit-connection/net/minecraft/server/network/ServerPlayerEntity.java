...
public class ServerPlayerEntity extends PlayerEntity {
	...
	public BlockPos getWorldSpawnPos(ServerWorld world, BlockPos basePos) {
		...
		if (world.getDimension().hasSkyLight() && world.getServer().getSaveProperties().getGameMode() != GameMode.ADVENTURE) {
			...
			int j = MathHelper.floor(world.getWorldBorder().getDistanceInsideBorder(basePos.getX(), basePos.getZ()));
			if (j < i) {
				i = j;
			}
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.sculkShriekerWarningManager = (SculkShriekerWarningManager)nbt.get("warden_spawn_tracker", SculkShriekerWarningManager.CODEC)
			.orElseGet(SculkShriekerWarningManager::new);
		this.enteredNetherPos = (Vec3d)nbt.get("entered_nether_pos", Vec3d.CODEC).orElse(null);
		this.seenCredits = nbt.getBoolean("seenCredits", false);
		this.recipeBook.readNbt(nbt.getCompoundOrEmpty("recipeBook"), recipeKey -> this.server.getRecipeManager().get(recipeKey).isPresent());
		if (this.isSleeping()) {
			this.wakeUp();
		}

		this.respawn = (ServerPlayerEntity.Respawn)nbt.get("respawn", ServerPlayerEntity.Respawn.CODEC).orElse(null);
		this.spawnExtraParticlesOnFall = nbt.getBoolean("spawn_extra_particles_on_fall", false);
		this.startRaidPos = (BlockPos)nbt.get("raid_omen_position", BlockPos.CODEC).orElse(null);
	}

	@Override
	public void writeCustomDataToNbt(NbtCompound nbt) {
		super.writeCustomDataToNbt(nbt);
		nbt.put("warden_spawn_tracker", SculkShriekerWarningManager.CODEC, this.sculkShriekerWarningManager);
		this.writeGameModeNbt(nbt);
		nbt.putBoolean("seenCredits", this.seenCredits);
		nbt.putNullable("entered_nether_pos", Vec3d.CODEC, this.enteredNetherPos);
		...
		nbt.putNullable("respawn", ServerPlayerEntity.Respawn.CODEC, this.respawn);
		nbt.putBoolean("spawn_extra_particles_on_fall", this.spawnExtraParticlesOnFall);
		nbt.putNullable("raid_omen_position", BlockPos.CODEC, this.startRaidPos);
	...
	private void writeRootVehicle(NbtCompound nbt) {
		...
		if (entity2 != null && entity != this && entity.hasPlayerRider()) {
			...
			nbtCompound.put("Attach", Uuids.INT_STREAM_CODEC, entity2.getUuid());
			nbtCompound.put("Entity", nbtCompound2);
			nbt.put("RootVehicle", nbtCompound);
		}
	}

	public void readRootVehicle(NbtCompound nbt) {
		Optional<NbtCompound> optional = nbt.getCompound("RootVehicle");
		if (!optional.isEmpty()) {
			ServerWorld serverWorld = this.getServerWorld();
			Entity entity = EntityType.loadEntityWithPassengers(
				((NbtCompound)optional.get()).getCompoundOrEmpty("Entity"), serverWorld, SpawnReason.LOAD, entityx -> !serverWorld.tryLoadEntity(entityx) ? null : entityx
			);
			if (entity != null) {
				UUID uUID = (UUID)((NbtCompound)optional.get()).get("Attach", Uuids.INT_STREAM_CODEC).orElse(null);
		...
		if (!this.enderPearls.isEmpty()) {
			...
			for (EnderPearlEntity enderPearlEntity : this.enderPearls) {
				if (enderPearlEntity.isRemoved()) {
					...
					nbtCompound.put("ender_pearl_dimension", World.CODEC, enderPearlEntity.getWorld().getRegistryKey());
	...
	public void readEnderPearls(NbtCompound nbt) {
		nbt.getList("ender_pearls").ifPresent(enderPearls -> enderPearls.streamCompounds().forEach(this::readEnderPearl));
	}

	private void readEnderPearl(NbtCompound nbt) {
		Optional<RegistryKey<World>> optional = nbt.get("ender_pearl_dimension", World.CODEC);
		if (!optional.isEmpty()) {
			ServerWorld serverWorld = this.getServerWorld().getServer().getWorld((RegistryKey<World>)optional.get());
			if (serverWorld != null) {
				Entity entity = EntityType.loadEntityWithPassengers(
					nbt, serverWorld, SpawnReason.LOAD, enderPearl -> !serverWorld.tryLoadEntity(enderPearl) ? null : enderPearl
				);
				if (entity != null) {
	...
	public void setExperiencePoints(int points) {
		...
		float g = (f - 1.0F) / f;
		this.experienceProgress = MathHelper.clamp(points / f, 0.0F, g);
	...
	private static Optional<ServerPlayerEntity.RespawnPos> findRespawnPosition(ServerWorld world, ServerPlayerEntity.Respawn respawn, boolean bl) {
		...
		if (block instanceof RespawnAnchorBlock && (bl2 || (Integer)blockState.get(RespawnAnchorBlock.CHARGES) > 0) && RespawnAnchorBlock.isNether(world)) {
			...
			return optional.map(respawnPos -> ServerPlayerEntity.RespawnPos.fromCurrentPos(respawnPos, blockPos));
			...
			return BedBlock.findWakeUpPosition(EntityType.PLAYER, world, blockPos, blockState.get(BedBlock.FACING), f)
				.map(respawnPos -> ServerPlayerEntity.RespawnPos.fromCurrentPos(respawnPos, blockPos));
		} else if (!bl2) {
			...
			return bl3 && bl4
				? Optional.of(new ServerPlayerEntity.RespawnPos(new Vec3d(blockPos.getX() + 0.5, blockPos.getY() + 0.1, blockPos.getZ() + 0.5), f))
				: Optional.empty();
		}
	}
	...
	public Either<PlayerEntity.SleepFailureReason, Unit> trySleep(BlockPos pos) {
		...
		if (this.isSleeping() || !this.isAlive()) {
			...
			this.setSpawnPoint(new ServerPlayerEntity.Respawn(this.getWorld().getRegistryKey(), pos, this.getYaw(), false), true);
	...
	protected void fall(double heightDifference, boolean onGround, BlockState state, BlockPos landedPosition) {
		if (this.spawnExtraParticlesOnFall && onGround && this.fallDistance > 0.0) {
			...
			this.getServerWorld().spawnParticles(new BlockStateParticleEffect(ParticleTypes.BLOCK, state), vec3d.x, vec3d.y, vec3d.z, i, 0.3F, 0.3F, 0.3F, 0.15F);
	...
	private static GameMode gameModeFromNbt(@Nullable NbtCompound nbt, String key) {
		return nbt != null ? (GameMode)nbt.get(key, GameMode.INDEX_CODEC).orElse(null) : null;
	}
	...
	private void writeGameModeNbt(NbtCompound nbt) {
		nbt.put("playerGameType", GameMode.INDEX_CODEC, this.interactionManager.getGameMode());
		GameMode gameMode = this.interactionManager.getPreviousGameMode();
		nbt.putNullable("previousPlayerGameType", GameMode.INDEX_CODEC, gameMode);
	}
	...
	public record Respawn(RegistryKey<World> dimension, BlockPos pos, float angle, boolean forced) {
		public static final Codec<ServerPlayerEntity.Respawn> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
					World.CODEC.optionalFieldOf("dimension", World.OVERWORLD).forGetter(ServerPlayerEntity.Respawn::dimension),
					BlockPos.CODEC.fieldOf("pos").forGetter(ServerPlayerEntity.Respawn::pos),
					Codec.FLOAT.optionalFieldOf("angle", 0.0F).forGetter(ServerPlayerEntity.Respawn::angle),
