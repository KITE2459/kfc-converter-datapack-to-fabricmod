...
public class Texts {
	...
	private static Style parseStyle(@Nullable ServerCommandSource source, Style style, @Nullable Entity sender, int depth) throws CommandSyntaxException {
		if (style.getHoverEvent() instanceof HoverEvent.ShowText(Text hoverEvent2)) {
			HoverEvent hoverEvent2x = new HoverEvent.ShowText(parse(source, hoverEvent2, sender, depth + 1));
