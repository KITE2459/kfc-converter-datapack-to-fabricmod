...
execute as @e[distance=..200,tag=check-point,type=marker] if score @s check-num = @p[tag=check-player-temp,distance=..5] check-num run tag @s add checkpoint-passed-temp
...
#2lap-early-finish
execute as @e[distance=..200,tag=check-2lap-last,tag=check-point,type=marker] if score @s check-num = @p[tag=check-player-temp,distance=..5] check-num as @p[tag=check-player-temp,distance=..5] run function checkpoint:system/2lap-early-finish/main

#위치 기록해두고 R키 위치로 활용
function checkpoint:system/memory-pass-position

#멀티 플레이일 경우 순위 감지 실행
scoreboard players set #from-check-pass-function check-prev 1
scoreboard players set #from-check-pass-function check-prev 0

tag @e[tag=checkpoint-passed-temp] remove checkpoint-passed-temp