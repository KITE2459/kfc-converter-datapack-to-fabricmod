...
#체크포인트로 이동
execute on vehicle on vehicle if data entity @s data.pos run data modify storage kartmain posx set from entity @s data.pos[0]
execute on vehicle on vehicle if data entity @s data.pos run data modify storage kartmain posy set from entity @s data.pos[1]
execute on vehicle on vehicle if data entity @s data.pos run data modify storage kartmain posz set from entity @s data.pos[2]
execute on vehicle on vehicle if data entity @s data.pos at @s run function gamemain:rkey-tp with storage kartmain
...
#효과
scoreboard players set @e[tag=mykart,limit=1,type=#kartmobil:kartmobil] kartcollisiontime 40
