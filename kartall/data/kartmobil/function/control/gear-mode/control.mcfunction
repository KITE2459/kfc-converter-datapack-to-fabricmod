function kartmobil:control/boost-gauge/gear-main
...
#페달조작
execute if entity @p[tag=kartpassenger,predicate=kartmobil:w] run function kartmobil:control/gear-mode/accel
execute if entity @p[tag=kartpassenger,predicate=kartmobil:s] run function kartmobil:control/brake

#기어조작
execute unless score @s[tag=kart-gear-press] kartboostcount matches 6.. if entity @p[tag=kartpassenger,predicate=kartmobil:d] run scoreboard players add @s kartboostcount 1
