...
#tellraw @p [{text:"[DEBUG] allowed-angle: ",color:"gold",extra:[{score:{name:"#allowed-angle",objective:"kartmain"}},{text:" "},{text:"allowed-angle-minus: ",color:"gold"},{score:{name:"#allowed-angle-minus",objective:"kartmain"}}]}]

execute if score #allowed-angle-minus kartmain > #kartangle kartmain run function kartmobil:move/steering/drift/drift-start
execute if score #kartangle kartmain > #allowed-angle kartmain run function kartmobil:move/steering/drift/drift-start