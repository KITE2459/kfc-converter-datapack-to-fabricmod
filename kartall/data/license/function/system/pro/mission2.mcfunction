...
#11초(220) 안에 완주
bossbar set minecraft:license max 133700

bossbar set minecraft:license name {translate:"02:13.700 내로 완주","color":"aqua"}

execute store result bossbar minecraft:license value run scoreboard players get time timermain

execute if score time timermain matches 133701.. run function license:system/retirefail
#완주 못하면 리타이어
...
execute if score @s trackselect-lap >= #max-lap trackselect-lap at @s run function license:system/pro/mission2complete