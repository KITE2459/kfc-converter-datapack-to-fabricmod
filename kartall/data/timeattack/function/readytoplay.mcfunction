...
execute as @n[tag=trackselect-timeattack-marker,type=marker] at @s run function timerpack:show-track-on-sidebar/main
data modify entity @e[tag=timeattack-state-text,limit=1] text set value [{translate:"타임어택 진행 중\n","color":"green"},{translate:"관전하기","color":"aqua"}]
...
#게임 시작
execute store result score @p trackselect-map-id run data get entity @n[tag=trackselect-timeattack-marker] data.track.number
scoreboard players operation #map trackselect-map-id = @p trackselect-map-id
scoreboard players set @p timecount 1
