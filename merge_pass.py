...
# A/B 토글: 정규식 가속 대신 레퍼런스 문자 상태기계 사용 (회귀 이분탐색용)
_KFC_SLOW_STRIP = os.environ.get("KFC_SLOW_STRIP") == "1"
...
def _strip_noncode_slow(text: str) -> str:
    """(레퍼런스 구현 — KFC_SLOW_STRIP=1 A/B용) 문자 단위 상태기계. 시맨틱 정의는 여기."""
...
# 정규식 1패스 등가 구현 (~10x): 대안 순서가 상태기계 우선순위를 재현한다 —
# 문자열이 먼저 매치되므로 문자열 속 //, /* 는 주석으로 오인되지 않고,
# 주석이 먼저 매치되므로 주석 속 " 는 문자열 시작으로 오인되지 않는다.
# 미종결 문자열/블록주석은 \Z 대안으로 EOF 까지 소비(상태기계와 동일).
# [검증] kartall 14,826개 생성 클래스 전수에서 _strip_noncode_slow 와 출력 동일 확인.
_STRIP_NONCODE_RE = re.compile(
    r'"(?:[^"\\]|\\.)*(")?'             # 문자열: g1=닫는따옴표(미종결=None)
    r'|//[^\n]*'                        # 라인주석(개행은 보존)
    r'|/\*.*?(?:\*/|\Z)',               # 블록주석(미종결시 EOF까지)
    re.S)

def _strip_noncode_repl(m):
    if m.group(0)[0] != '"':
        return ''                       # 주석 → 제거
    return '""' if m.group(1) else '"'  # 문자열 → 껍데기(닫힘 여부 보존)
...
def _strip_noncode(text: str) -> str:
    ...
    if _KFC_SLOW_STRIP:
        return _strip_noncode_slow(text)
    return _STRIP_NONCODE_RE.sub(_strip_noncode_repl, text)
...
def _oversized(text: str) -> bool:
    ...
    _total = len(text.encode("utf-8"))
    if LEGACY_SIZE_BRIDGE and _total > METHOD_CAP:
        return True
    if _total < METHOD_BC_CAP:
        # [지름길 — 판정 동일] (a) stripped ≤ 원본 바이트 < 56KB → 어떤 메서드도 초과 불가.
        # (b) 상수풀 60,000 '고유' 리터럴은 최소 2바이트/개 = 120KB+ 소스 필요 → 도달 불가.
        # 평균 ~2KB 레코드가 대부분이라 전체 스캔(_bc_proxy_max_method/_strip_noncode)을 생략.
        return False
...
def _bridge_oversized_text(text: str, fid: str, is_macro: bool) -> str:
    """executeReturn 본문을 instantExecuteFunction 브릿지로 치환한 새 소스 반환(텍스트 기반)."""
    ret = _find_method(text, r'public static int executeReturn\(')
    if not ret:
        return text
    s, e, mtext = ret
    # 시그니처 줄과 닫는 '}' 만 남기고 본문 교체
    head = mtext[:mtext.index("{") + 1]
    new_method = head + "\n" + _bridge_body(fid, is_macro) + "\n    }"
    return text[:s] + new_method + text[e:]

def bridge_oversized(c: Cls) -> str:
    return _bridge_oversized_text(c.text, c.fid, c.is_macro)


# ── bucketize 레코드 전처리 워커 (오버사이즈 판정 + 브릿지 치환 + 메서드 추출) ──
# 레코드별 완전 독립 · 순수 텍스트 연산이라 병렬화가 시맨틱에 영향 없다.
# 종전엔 pass-3 메인 직렬로 레코드당 전체 텍스트 문자 스캔 4~5회
# (_oversized→_bc_proxy_max_method/_strip_noncode ×2 + _extract_two_methods→_find_method ×2)
# 가 돌았다 — 대형 팩(수십만 함수·수백MB 코드)에서 pass-3 의 지배적 비용.
def _prep_record(args):
    text, fid, is_macro = args
    over = _oversized(text)
    if over:
        text = _bridge_oversized_text(text, fid, is_macro)
    mang = _mangle(fid)
    two = _extract_two_methods(text, mang)
    return (over, text if over else None, mang, two, _imports_of(text))

# ── 버킷 조립 워커 (rewrite_calls + 템플릿 조립 + 기록) ──
# 버킷별 완전 독립(remap 은 읽기 전용 전역) — 산출 바이트는 직렬 조립과 동일.
# kartall 실측에서 pass-3 의 지배 비용(42.3s)이 이 조립·기록 루프였다.
_BB_REMAP = None
_BB_GROUP = None
...
def _build_bucket(task):
    ...
    code = (f"package {bucket_pkg};\n\n{import_block}\n\n"
            f"/** Auto-bucketed: {n_members} datapack functions. */\n"
            f"public final class {cls_name} {{\n"
            f"    private {cls_name}() {{ throw new UnsupportedOperationException(); }}\n\n"
            + "\n\n".join(body_methods) + "\n}\n")
    # pass-4 가 디스크에서 되읽어 하던 상수 호이스팅을 여기서(메모리) 수행 — 동일 함수라 산출 동일.
    code, _hk = hoist_constants_text(code)
    _write_if_changed(Path(path_str), code)
    return _hk

def _mp_jobs() -> int:
    """병렬 워커 수: KFC_JOBS 명시 > 실제 가용 코어 (convert.py 와 동일 규칙)."""
    try:
        j = int(os.environ.get("KFC_JOBS", "0"))
        if j > 0:
            return j
    except ValueError:
        pass
...
def bucketize_records(records, src_root: Path, group: str,
    ...
    import time as _bt
    _bt0 = _bt.time(); _btl = [_bt0]
    def _btlog(label):
        if verbose:
            _n = _bt.time()
            print(f"  [bucketize:t] {label}: +{_n - _btl[0]:.1f}s", flush=True)
            _btl[0] = _n
    src_root = Path(src_root)
    ...
    # 4) fqcn → (bucketFQCN, mang) 전역 맵 + 버킷 멤버 메서드 추출 (1)의 전처리 결과 재사용)
    bucket_pkg = f"{group}.buckets"
    remap = {}
    bucket_of = {}
    for bi, group_cls in enumerate(buckets):
    ...
    if jobs > 1 and len(tasks) >= 32:
        ...
        try:
            with _mpc.get_context("spawn").Pool(processes=jobs,
                ...
                hoisted = sum(pool.imap_unordered(_build_bucket, tasks, chunksize=4))
        except Exception as _pe:
            print(f"[bucketize:mem][warn] parallel bucket build failed ({_pe}) — serial fallback")
            hoisted = None
    if hoisted is None:
        _bb_init(remap, group)
        hoisted = sum(_build_bucket(task) for task in tasks)
    written = len(tasks)
    if verbose:
...
def _hoist_one_file(pstr: str):
    """워커: 파일 하나 읽어 호이스팅, 변경 시 기록. 파일별 완전 독립(순서 무관·결정적)."""
    p = Path(pstr)
    try:
...
def hoist_constants_tree(src_root: Path, verbose: bool = False, skip_dir: Path | None = None) -> dict:
    """src_root 아래 모든 생성 .java 에 호이스팅 적용(KfcGen.java 제외).
       파일별 독립 연산이므로 파일 수가 많으면 병렬(결과·산출 바이트는 직렬과 동일).
       skip_dir: 이미 호이스팅된 트리(버킷 — pass-3 워커 내 처리) 제외."""
    paths = [str(p) for p in sorted(Path(src_root).rglob("*.java"))
             if p.name != "KfcGen.java"
             and (skip_dir is None or skip_dir not in p.parents)]
    files = 0; repl = 0
    jobs = _mp_jobs()
    results = None
    if jobs > 1 and len(paths) >= 64:
        import multiprocessing as _mpc
        try:
            with _mpc.get_context("spawn").Pool(processes=jobs) as pool:
                results = pool.map(_hoist_one_file, paths, chunksize=8)
        except Exception as _pe:
            print(f"[hoist][warn] parallel failed ({_pe}) — serial fallback")
            results = None
    if results is None:
        results = [_hoist_one_file(p) for p in paths]
    for pstr, k in results:
        if k:
            files += 1; repl += k
            if verbose:
                print(f"  [hoist] {Path(pstr).name}: {k}")
